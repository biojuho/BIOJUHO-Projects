...
| 스레드 | 담당 | 상태 | 다음 액션 |
|--------|------|------|-----------|
| 0004 교차 확산 감지 | **Codex** | 진행 중 | 0002 병합(`f3de282`) 확인 후 재개. `fast_viral_collector.py` 독점. **2026-08-07 화면 중복 처리를 추가 배정** — 클러스터 키가 같은데 두 자리를 먹는 건(`_select_diverse_community_items`가 소스별 라운드로빈만 하고 클러스터를 안 봄)과, 표기 차이로 키가 갈리는 건(여자친구/여친) 둘 다. 지우지 말고 합쳐서 "2곳 동시"를 강조하는 방향. **핸드오프 파일은 아직 OPEN·실행자 공란** |
...
| 0011 "이유"·"근황" 좁히기 | **Qwen** | 진행 중 | `handoffs/0011-two-words-make-two-thirds.md` — 사는 축 26건 중 두 어휘가 17건(64%)이고 08:5x 화면에서는 **상위 6건 전부**가 이 둘이었다. X 정답지에서는 각 1건뿐 |
