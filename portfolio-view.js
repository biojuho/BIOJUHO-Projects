(function (root) {
  ...
  function createPortfolioView(deps) {
    ...
    function portfolioViewModel(input) {
      ...
      const kpis = [
        { title: "프로젝트", value: String(stats.total), unit: "개", color: "var(--blue)", badge: "▦", delta: "" },
        { title: "평균 진행률", value: String(stats.avg), unit: "%", color: "var(--green)", badge: "✺", delta: "▲ 4%p" },
        { title: "지연", value: String(stats.delayed), unit: "건", color: "var(--red)", badge: "△", delta: stats.delayed ? "조치 필요" : "없음", trendDown: stats.delayed > 0 },
        { title: "위험 프로젝트", value: String(stats.risky), unit: "건", color: "var(--amber)", badge: "⬡", delta: "Amber 이상" },
      ];
