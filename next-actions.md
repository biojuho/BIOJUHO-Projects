...
> 2026-06-04 AutoResearch cycle: Dev-server controller now supports grouped teardown with `stop --include-dependencies`. Live dashboard proof started `dashboard-frontend` with `dashboard-api`, then stopped in order `dashboard-frontend`, `dashboard-api` with `stopped=2/2`; final status returned `0/2 ready`, direct port check showed no listeners on `8080` or `5173`, focused dev-server tests `20 passed`, and workspace smoke `8/8 PASS` via `var/workspace-smoke-workspace-dev-server-group-stop-2026-06-04.json`. Report: `docs/reports/2026-06/AUTO_RESEARCH_DEV_SERVER_GROUP_STOP_2026-06-04.md`.
