...
4. 감성(sentiment) 및 안전 여부(safety_flag)
5. 왜 지금 이 트렌드가 뜨는지 원인 추론 (why_trending)
6. 트렌드의 현재 단계 (peak_status)
7. X에서 활발히 논의하기 적합한 정도 (relevance_score)

[규칙]
- 각 트렌드의 "왜 지금 뜨는지" 원인을 반드시 추론할 것
- X에서 활발히 논의되기 적합한 트렌드만 높은 relevance_score 부여
- 단순 연예/가십보다 인사이트를 줄 수 있는 주제를 우선
- 이미 하락세인 트렌드는 peak_status=하락중으로 표시
...
safety_flag=true 조건: 재난/사망/폭력/혐오/자살 등 유해 콘텐츠 생성이 부적절한 트렌드."""
...
[분석 규칙]
- 각 트렌드의 "왜 지금 뜨는지" 원인을 반드시 추론할 것
- X에서 활발히 논의되기 적합한 트렌드만 높은 relevance_score 부여 (단순 연예/가십 낮게)
- 이미 하락세인 트렌드는 peak_status=하락중으로 표시
...
- safety_flag=true: 재난/사망/폭력/혐오/자살 관련 트렌드
