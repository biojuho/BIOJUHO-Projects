...
위 데이터를 참고하여 3가지 유형의 트윗 시안을 JSON 형식으로만 작성해주세요.
...
[출력 형식 - JSON만]
{
  "posts": [
    ...
    {"type": "핵심 정보", "content": "핵심 팩트/인사이트 (500자 이내)"},
...
하나의 트렌드 주제를 깊이 있게 다루는 정확히 3개 트윗으로 구성된 쓰레드를 작성합니다.

[쓰레드 구조 - 반드시 3개]
1. 🪝 훅 트윗 (최대 2,500자): 타임라인에서 "더보기"를 누르게 만드는 강렬한 도입부. 핵심 주장 + 데이터 + 호기심 유발. 충분히 길게 작성해도 됩니다.
2. 💡 핵심 인사이트 (500~1,000자): 남들이 놓치는 핵심 포인트 + 데이터 기반 분석
3. 🎯 마무리 트윗 (500~1,000자): CTA + 핵심 요약 + 리트윗/의견 요청
...
[출력 형식 - JSON만]
{
  ...
  "tweets": ["첫 번째 트윗 (훅)", "두 번째 트윗 (핵심 인사이트)", "세 번째 트윗 (마무리)"]
}"""
...
위 데이터를 기반으로 정확히 3개 트윗의 바이럴 쓰레드를 JSON 형식으로 작성해주세요.
