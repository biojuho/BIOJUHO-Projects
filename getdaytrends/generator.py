...
컨텍스트 기반 트윗 5종 + X Premium+ 장문 + Meta Threads + 강화 쓰레드 생성.
"""
...
[출력 형식 - JSON만]
{
  "posts": [
    ...
    {"type": "캐러셀 1", "content": "슬라이드 1 (500자 이내)"},
    {"type": "캐러셀 2", "content": "슬라이드 2 (500자 이내)"},
    {"type": "캐러셀 3", "content": "슬라이드 3 (500자 이내)"},
...
2) 캐러셀 연속 포스트 3개: 슬라이드 넘기듯 이어지는 정보 전달
3) 참여형 포스트: 공감 + 질문으로 댓글 유도
...
하나의 트렌드 주제를 깊이 있게 다루는 5~8개 트윗으로 구성된 쓰레드를 작성합니다.

[쓰레드 구조]
1. 🪝 훅 트윗 (최대 2,500자): 타임라인에서 "더보기"를 누르게 만드는 강렬한 도입부. 핵심 주장 + 데이터 + 호기심 유발. 충분히 길게 작성해도 됩니다.
2. 📊 데이터 트윗 (500~1,000자): 뉴스/통계/데이터 기반 핵심 팩트
3. 💡 인사이트 트윗 (500~1,000자): 남들이 놓치는 핵심 포인트
4. 🔍 분석 트윗 (500~1,000자): 독자적 시각의 심층 분석
5. 🌍 맥락 트윗 (500~1,000자): 글로벌/역사적 맥락
6. 🔮 전망 트윗 (500~1,000자): 미래 예측과 시나리오
7. 🎯 마무리 트윗 (500~1,000자): CTA + 핵심 요약 + 리트윗/의견 요청
...
위 데이터를 기반으로 5~8개 트윗의 바이럴 쓰레드를 JSON 형식으로 작성해주세요.
