...
@pytest.mark.asyncio
async def test_llm_client_wrapper_accumulates_sidecar_records_across_calls(monkeypatch, tmp_path):
    from antigravity_mcp.integrations.llm import client_wrapper
    ...
    await wrapper._complete_text(prompt=("system", "first prompt"), max_tokens=100, cache_scope="unit-sidecar-a")
    await wrapper._complete_text(prompt=("system", "second prompt"), max_tokens=100, cache_scope="unit-sidecar-b")

    lines = [line for line in usage_path.read_text(encoding="utf-8").splitlines() if line.strip()]
    assert len(lines) == 2
    for line in lines:
        record = json.loads(line)
        assert record["usage"] == {"input_tokens": 10, "output_tokens": 5, "total_tokens": 15}
