...
def test_biolinker_warnings():
    ...
    try:
        ...
        rfp = RFPDocument(id="test", title="Test RFP", body_text="Test Body", source="Test Source", deadline=None, keywords=[])
        # Fix: current_trl must be string
        profile = UserProfile(company_name="TestCorp", tech_keywords=["Bio"], tech_description="Bio Tech", company_size="Small", current_trl="3")
...
def test_notion_server_env():
    ...
    try:
        with open(r"d:\AI 프로젝트\MCP_notion-antigravity\server.py", 'r', encoding='utf-8') as f:
            content = f.read()
