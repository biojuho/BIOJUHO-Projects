...
**이 20건은 X 캡션이지 우리 입력이 아니다(2026-08-07 확인).** 레이더가 `screen_material`에
넣는 것은 커뮤니티 게시판 제목이고, 같은 소재라도 문장이 전혀 다르다 — X의 "통닭아냐?"(82만)는
더쿠에서 "임산부덕 오늘 병원갔다가 놀람"이라는 제목으로 올라온 글이다. 앞의 문장은 우리 화면에
절대 나타나지 않는다. **그래서 이 파일의 재현율은 우리가 고칠 수 있는 것을 재지 못한다.**
커뮤니티 원문으로 재는 쪽은 `test_kernel_recall_community_origin.py`에 따로 있고, 규칙을 바꿀
근거로는 그쪽을 쓴다. 이 파일은 "X에서 실제로 퍼진 문장들이 어떻게 생겼는지"의 참고 표본으로
남긴다.
...
# 0011 좁히기 후: 약한 어휘("이유"·"근황") 단독은 unknown. 서사 이유 패턴 1건 + 대상 전환 1건.
_BASELINE_LIVE = 2

# 0011 이후에도 잡히는 대표 건. "식당"은 서사 이유(⑥)로, "에어컨→냉장고"는 대상 전환(⑤)으로 잡힘.
_KNOWN_HITS = (
    "어느 식당이 여름 휴가를 간 이유",
)
