...
async def test_step_save_records_false_external_save_result(memory_db):
    ...
    with patch("core.steps_save.save_to_notion", return_value=False) as save_to_notion:
...
async def test_step_save_skips_content_hub_when_disabled(memory_db):
    ...
    with patch("core.steps_save.save_to_notion", return_value=True) as save_to_notion:
        with patch("core.steps_save.save_to_content_hub", return_value=True) as save_to_content_hub:
            success = await _step_save(
                [_make_trend("single-db-topic")],
                [make_batch("single-db-topic")],
                config,
