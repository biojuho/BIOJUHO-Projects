...
def test_format_news_age_recent():
    ...
    # 2시간 전 날짜 만들기
    two_hours_ago = _dt.now(UTC) - timedelta(hours=2)
...
async def test_record_content_feedback(conn):
    ...
    cursor = await conn.execute("SELECT keyword, category, qa_score, regenerated, reason FROM content_feedback")
...
async def test_record_content_feedback_regenerated(conn):
    ...
    cursor = await conn.execute("SELECT keyword, regenerated FROM content_feedback WHERE keyword = '재생성키워드'")
    row = await cursor.fetchone()
    assert row is not None
