...
class TestHealthCheck:
    ...
    @pytest.mark.asyncio
    async def test_healthy_when_authenticated_and_cli_reachable(self, notebooklm_home):
        auth_status = {
            "authenticated": True,
            "needs_refresh": False,
            "age_hours": 1.0,
        ...
        with (
            patch("notebooklm_automation.health.check_auth_status", return_value=auth_status),
            patch("notebooklm_automation.health._probe_notebooklm_cli", return_value=(True, 2)),
        ):
            result = await nlm_health.health_check()
    ...
    @pytest.mark.asyncio
    async def test_degraded_when_authenticated_session_needs_refresh(self, notebooklm_home):
        auth_status = {
            "authenticated": True,
            "needs_refresh": True,
            "age_hours": 22.0,
        ...
        with (
            patch("notebooklm_automation.health.check_auth_status", return_value=auth_status),
            patch("notebooklm_automation.health._probe_notebooklm_cli", return_value=(True, 1)),
        ):
            result = await nlm_health.health_check()
