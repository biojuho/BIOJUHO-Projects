...
class AppConfig:
    ...
    # [shortform-only] 단문 트윗 5종만 생성. 장문/Threads/X쓰레드/블로그는 기본 비활성.
    enable_long_form: bool = False
    enable_threads: bool = False
    smart_schedule: bool = True
    night_mode: bool = True
    long_form_min_score: int = 999  # 비활성 (enable_long_form=False면 무시)
    thread_min_score: int = 999  # X 멀티 쓰레드 비활성
    threads_min_score: int = 999  # Meta Threads 비활성

    # [shortform-only] 단문 트윗 길이 범위(한국어 공백 포함)
    tweet_min_chars: int = 160
    tweet_max_chars: int = 240
    min_viral_score: int = (
        60  # ???源녿뼥 ??ш낄援?? 雅?퍔瑗띰㎖?뱁맪??????熬곣뫕????????獄쏅똻??癲꾧퀗????? [??れ삀???55 ??60]
    )
    ...
    min_article_count: int = (
        3  # ??????ш끽維곲??癲ル슔?됭짆????れ삀?節놁쒜????怨뚮옖???[v13.0: 5??, ?????源녿뼥 ??좊즴甕??????袁⑸젻泳?]
    )
    ...
    embedding_cluster_threshold: float = (
        0.75  # ?熬곣뫀毓쇌벧?????モ?????ш낄猷?嚥♂쇱씀?(0.0~1.0, ?亦껋꼨援?キ??嚥?흮 ??ш낄猷볣걡?
    )
    qa_skip_cached: bool = True  # 癲??????雅???熬곣뫕??????QA ???袁⑤툞
    qa_skip_high_score: int = 85  # ??????????⑤?彛??嶺뚮ㅎ????QA ???袁⑤툞
    qa_skip_categories: list[str] = field(
        default_factory=lambda: ["\ub0a0\uc528", "\uc74c\uc2dd", "\uc2a4\ud3ec\uce20"]
    )
    ...
    enable_content_diversity: bool = (
        True  # ???⑤챷????獄쏅똻??????猿????ш끽維???ш낄援θキ????낆뒩????濚욌꼬?댄꺇???袁⑸젻泳?
    )
    ...
    blog_min_score: int = 999  # [shortform-only] disabled. Comment legacy:??????????⑤?彛쒙┼????源낇꼧????怨?繞벿삳쎖????れ꽔?????獄쏅똻??
    ...
    # ===================================================
    enable_zero_content_prevention: bool = (
        True  # 癲ル슢?꾤땟????嶺뚮ㅎ????? ??筌믨퀡???怨멸텭??沃섅뀙??關履?????癲ル슔?됭짆??1???怨뚮옖???
    )
    ...
    early_signal_boost_threshold: float = (
        2.0  # ?縕?猿녿뎨?ER????????N?????⑤?彛???????ш끽維뽫댆??熬곣뫕???????嶺뚮ㅎ遊뉔걡酉멥??
    )
    early_signal_suppress_threshold: float = (
        0.3  # ?縕?猿녿뎨?ER????????N????熬곣뫀????????? ??좊읈?濚욌꼬?댄꺍?????얜Ŋ源?
    )
    ...
    fact_check_min_accuracy: float = (
        0.6  # ???됰씭肄?癲ル슪???띿물?癲ル슔?됭짆???嶺뚮쮳?곌섈??(0~1, 雅?퍔瑗띰㎖?뱁맪???????濚??
    )
