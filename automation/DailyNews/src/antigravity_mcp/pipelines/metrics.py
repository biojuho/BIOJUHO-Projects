...
async def collect_recent_metrics(
    *,
    ...
    if not tweet_ids:
        store.record_job_finish(run_id, status="success", summary={"tweets_updated": 0, "reason": "no_recent_tweets"})
        return run_id, 0, []

    adapter = XMetricsAdapter(state_store=store)
    if not adapter.is_available:
        warnings.append("X_BEARER_TOKEN not configured; skipping metrics collection.")
        store.record_job_finish(run_id, status="partial", summary={"tweets_updated": 0}, error_text="no_bearer_token")
