    ...
    def load_checkpoint(self, job_id: str) -> tuple[str | None, dict[str, Any]]:
        ...
        with self.db.get_connection() as conn:
            if self.db.use_postgres:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT current_step, state_json FROM pipeline_checkpoints WHERE job_id = %s", (job_id,)
                    )
                    row = cur.fetchone()
            else:
                row = conn.execute(
                    "SELECT current_step, state_json FROM pipeline_checkpoints WHERE job_id = ?", (job_id,)
                ).fetchone()
    ...
    def mark_completed(self, job_id: str) -> None:
        ...
        with self.db.get_connection() as conn:
            if self.db.use_postgres:
                with conn.cursor() as cur:
                    cur.execute(
                        "UPDATE pipeline_checkpoints SET status = 'completed', updated_at = NOW() WHERE job_id = %s",
                        (job_id,),
                    )
                conn.commit()
            else:
                conn.execute(
                    "UPDATE pipeline_checkpoints SET status = 'completed', updated_at = CURRENT_TIMESTAMP WHERE job_id = ?",
                    (job_id,),
                )
                conn.commit()
