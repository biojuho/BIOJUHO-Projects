...
async def log_update(title, goal, achievement, content, type_val, priority):
    ...
    for line in content.split("\\n"):  # 줄바꿈 문자 처리
        ...
        if line.startswith("# "):
            ...
            children_blocks.append(
                {"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"text": {"content": line[3:]}}]}}
            )
        elif line.startswith("- "):
            children_blocks.append(
                {
    ...
    try:
        ...
        print("[SUCCESS] Log saved to Notion!")
...
if __name__ == "__main__":
    ...
    asyncio.run(log_update(args.title, args.goal, args.achievement, args.content, args.type, args.priority))
