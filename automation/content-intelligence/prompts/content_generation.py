...
[도입부 구조 — 아래 5가지 opener 패턴 중 1가지를 랜덤 선택해 작성]
Opener-1 (공감 상황형): 독자가 겪는 구체적 상황 묘사로 시작.
  예: "새벽 2시, 또 GPT한테 같은 질문을 던지고 있었다."
Opener-2 (통계 충격형): 놀라운 숫자/데이터로 시작.
  예: "지난 달 AI 관련 특허 출원이 전월 대비 340% 증가했다."
Opener-3 (반직관 선언형): 통념을 뒤집는 주장으로 시작.
  예: "LLM은 생각보다 똑똑하지 않다. 대신 우리가 질문을 못 한다."
Opener-4 (1인칭 경험형): 필자의 실제 경험으로 시작.
  예: "자동화 파이프라인을 처음 돌렸을 때, 결과가 0건이었다."
Opener-5 (미래 시나리오형): 가까운 미래를 묘사하며 시작.
  예: "6개월 뒤, 이 기술을 안 쓰는 팀은 없을 것이다."

[본문 소제목 구조 — 아래 4가지 heading 패턴 중 1가지를 선택]
Heading-A (문제→원인→해법→전망):
  ## 무엇이 문제인가 / ## 왜 이렇게 됐나 / ## 어떻게 해결하나 / ## 앞으로의 방향
Heading-B (What→Why→How→So-what):
  ## 핵심 개념 정리 / ## 왜 지금 중요한가 / ## 실전 적용법 / ## 결론: 이것만 기억하세요
Heading-C (Before→Turning-point→After→Action):
  ## 이전까지의 상식 / ## 게임 체인저 등장 / ## 달라진 풍경 / ## 지금 해야 할 것
Heading-D (현상→데이터→분석→제언):
  ## 최근 동향 / ## 숫자로 보는 현실 / ## 전문가 시각 / ## 실무자를 위한 제안

선택한 opener 패턴과 heading 패턴을 본문에 자연스럽게 적용하되,
선택 결과를 JSON의 blog_structure 필드로 보고:
  "blog_structure": {"opener": "Opener-N", "heading": "Heading-X"}

[본문 구조]
  1. 도입 (선택된 opener 패턴 적용, 3~4문장) — 독자가 "나 얘기네" 느끼게
  2. 목차 박스 (선택된 heading 패턴의 소제목 4개 미리 보여주기, 체류시간 확보)
  3. 본론 (소제목 ## 4개, 각 300~500자, 핵심 키워드 자연 삽입)
