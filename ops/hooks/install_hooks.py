...
def _install_hook_file(source: Path, destination: Path) -> None:
    destination.write_text(_hook_install_content(source), encoding="utf-8", newline="\n")
...
def _installed_hook_matches(source: Path, destination: Path) -> bool:
    if source.resolve() == destination.resolve():
        return True
    return _normalized_hook_content(destination) == _hook_install_content(source)


def _hook_install_content(source: Path) -> str:
    """Return a stable shim so linked worktrees do not overwrite long hooks mid-run."""
    hook_name = source.name
    return (
        f"{SHIM_HEADER}"
        "# Generated by ops/hooks/install_hooks.py.\n"
        "# Delegates to the tracked hook in the active worktree.\n"
        "hook_name=${0##*/}\n"
        f'if [ "$hook_name" != "{hook_name}" ]; then\n'
        f'    hook_name="{hook_name}"\n'
        "fi\n"
        'hook_path="ops/hooks/$hook_name"\n'
        'if [ ! -f "$hook_path" ]; then\n'
        '    echo "Git hook source not found: $hook_path" >&2\n'
        "    exit 1\n"
        "fi\n"
        'exec sh "$hook_path" "$@"\n'
    )
