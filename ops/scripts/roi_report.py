...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument("--gdt-db", default=str(_GDT_DB), help="GetDayTrends SQLite DB 경로")
    parser.add_argument("--llm-db", default=str(_LLM_DB), help="LLM 비용 SQLite DB 경로")
...
def _fetch_calibration_trend(gdt_db: str, limit: int = 8) -> list[dict]:
    ...
    conn = _conn(gdt_db)
    has_table = bool(
        conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='calibration_insights'").fetchone()
    )
    if not has_table:
        conn.close()
        return []
...
def run(
    ...
    best_hour = max(heatmap, key=lambda h: heatmap[h]["avg_eng"]) if heatmap else -1
...
def _print_summary(report: dict) -> None:
    ...
    if cal:
        latest = cal[0]
        print(
            f"  [최신 Calibration] r={latest['pearson_r']:.4f}  MAE={latest['mae']:.2f}"
