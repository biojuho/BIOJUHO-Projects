...
일일 비용 대시보드 + 예산 경고 (Daemon)

사용법:
  python ops/scripts/cost_dashboard.py               # 콘솔 출력만
  python ops/scripts/cost_dashboard.py --notify       # 출력 + Discord/Telegram 알림
...
def run_cost_dashboard(*, days: int = 1, notify: bool = False):
    ...
    print(f"총 LLM API 호출: {summary['total_calls']}건")
    ...
    if budget_exceeded:
        ...
        print(f"예산 정상 (한도: ${DAILY_BUDGET_USD:.2f})")
        if RATE_LIMIT_LOCK_FILE.exists():
            RATE_LIMIT_LOCK_FILE.unlink()
            print("예산 아래로 복귀, 차단 해제 완료")
    ...
    if notify:
        try:
            ...
            if not notifier.has_channels:
                print("알림 채널 미설정 (DISCORD_WEBHOOK_URL / TELEGRAM_BOT_TOKEN)")
                return
