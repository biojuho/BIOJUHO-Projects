...
- Gap: MCP smoke trace metrics now derive deterministic offline timing/path-depth signals plus tracked Markdown and HTML handoff reports from existing smoke artifacts; full OpenTelemetry span trees, token/cost metrics, and cross-step causality remain future scoped work.
