...
```bash
python parse_markdown.py <markdown_file> [--output json|html] [--html-only] [--dry-run] [--validate]
```
Returns JSON with: title, cover_image, content_images, **dividers** (with block_index for positioning), html, total_blocks, validation

### validate_article.py
Run a deterministic dry-run before browser automation:
```bash
python validate_article.py <markdown_file> [--no-global-image-search]
```
Returns JSON with validation errors/warnings, selector manifest status, and the ordered workflow plan.
...
Selector manifest for the X Articles editor. Load this file before automation so button labels and fallback selectors stay centralized.
...
2. Run `validate_article.py` and stop if it returns any errors
3. Parse Markdown with Python script → get title, images, **dividers** with block_index, HTML
...
9. **Insert dividers at positions specified by block_index** (via Insert > Divider menu)
10. Save as draft (NEVER auto-publish)
