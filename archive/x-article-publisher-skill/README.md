...
**v1.2.1** — Now with dry-run validation, selector manifest support, divider support, table-to-image, Mermaid support, and cross-platform clipboard
...
Structured Data (title, images with block_index, HTML)
     ??Dry-run validation + selector manifest
     ↓ Playwright MCP
...
Before opening the browser, run a dry-run validation:

```bash
python skills/x-article-publisher/scripts/validate_article.py /path/to/article.md
```

This checks title/body integrity, image existence, block index ranges, and selector manifest availability.
...
│       ├── config/
│       │   └── selectors.json   # Centralized editor selector manifest
│       └── scripts/
│           ├── parse_markdown.py    # Extracts block_index + dividers
│           ├── validate_article.py  # Dry-run validation + workflow plan
...
### v1.2.1 (2026-03)
- **Dry-run validation**: New `validate_article.py` script checks title/body integrity, image existence, block ranges, and draft workflow order
- **Selector manifest**: New `skills/x-article-publisher/config/selectors.json` centralizes editor labels and fallback selectors
- **Safer image resolution**: Parser now records resolution strategy and warns when global fallback search is used
- **Structured parse validation**: `parse_markdown.py` now returns validation metadata and optional dry-run workflow plans
