...
const publicProjectRoot = "project-root";
...
function redactLocalPathText(value) {
  return String(value || "")
    .replaceAll(root, publicProjectRoot)
...
function redactAuditPayload(value) {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, redactAuditPayload(item)]));
  }
  return value;
}
...
function hasStageEquivalentTerm(relPath, text, term) {
  ...
  const equivalents = {
    ...
    "data/output-quality-audit.json": {
      ...
      "status=waiting_for_pasted_proof": ["status=all_fields_detected"],
      "detected=0/0": ["detected=6/6"],
...
function refreshPackagedBrowserEvidenceSources() {
  const sourceRefreshTimeoutMs = positiveMsOption(process.env.JOOPARK_AUDIT_SOURCE_REFRESH_TIMEOUT_MS, 90000);
  const commands = [
    {
...
function buildChecklist() {
  ...
  const outputQualityAuditTerms = [
    ...
    { file: "data/output-quality-audit.json", terms: ["launchPostAuthCheckpoint", "Launch post-auth checkpoint: pass (5 commands, expected=6, blocked=4, recheck=5, sources=4, dispatchApproval=false, verificationOnly=true", "\"recheckSequenceCount\": 5", "\"sourceArtifactCount\": 4", "\"verificationOnly\": true", "\"dispatchApproval\": false", "\"recheckKeys\"", "\"sourceArtifacts\"", "confirm_scope", "verify_remote_parity", "verify_actions_visibility", "verify_handoff_guard", "data/remote-workflow-file-check.json", "data/publish-dispatch-plan.json", "data/launch-handoff-verification.json", "verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown", "guard=Do not run gh workflow run until every action_required post-auth checkpoint item has passed and verify-launch-handoff reports safeToDispatch=true."] },
					    { file: "data/output-quality-audit.json", terms: ["workflowUiInstallReceipt", "Workflow UI paste packet: pass (workflowUiInstallPastePacketCoverage=1,", "commands, checklist=", "guard=Do not run gh workflow run until every post-install evidence field has been filled", "verify-launch-handoff reports safeToDispatch=true.", "workflowUiInstallPastePacketCoverage", "workflowUiInstallPastePacketCopy", "workflowUiInstallPastePacketReady", "workflowUiInstallReceiptCoverage", "workflowUiInstallReceiptCommandCount", "workflowUiInstallReceiptChecklistCount", "postInstallEvidenceIntake", "postInstallEvidenceIntakeFields", "postInstallEvidenceIntakeFieldCoverage", "Post-install evidence intake: pass (6 fields, coverage=1)", "Post-install proof parser: pass (6 fields, coverage=1)", "status=all_fields_detected", "detected=6/6", "falsePositiveGuard=true", "not dispatch approval", "Post-install quick proof: pass (4 steps, coverage=1)", "Post-install quick proof field mapping: pass (1/4 mapped fields complete, coverage=1)"] },
			    { file: "data/output-quality-audit.json", terms: ["postInstallEvidenceIntake", "\"source\": \"generated_from_launch_execution_packet\"", "\"status\": \"collect_post_install_proof\"", "\"completedFieldCount\": 2", "\"proofComplete\": false", "\"quickProofStepCount\": 4", "\"quickProofCoverage\": 1", "\"quickProofFieldMappingCoverage\": 1", "\"quickProofMappedFieldCount\": 4", "\"commandCount\": 4", "\"signalCount\": 8", "Post-install evidence intake: pass (6 fields, coverage=1) - status=collect_post_install_proof, completed=2/6, proofComplete=false, commands=4, signals=8", "Post-install quick proof: pass (4 steps, coverage=1)", "Post-install quick proof field mapping: pass (1/4 mapped fields complete, coverage=1)", "quickProofReady=true; steps=4; coverage=1", "quickProofFieldMappingReady=true; mapped=4; completed=1/4; coverage=1", "quick proof field 1 remote_file_parity -> remote_parity_proof", "quick proof field 4 handoff_verifier -> handoff_verifier_proof", "source=generated_from_launch_execution_packet; status=collect_post_install_proof; proofComplete=false; completed=2/6; commands=4; signals=8", "remote_parity_proof: evidence_required", "handoff_verifier_proof: evidence_required"] },
  ...
  const pipelineViewModuleTerms = [
    { file: "pipeline-view.js", terms: ["JooParkPipelineView", "joopark-pipeline-view/v1", "function createPipelineView", "function pipelineViewModel", "function cellButton", "function renderPipelineHTML", "data-search-result=\"pm-pipeline\"", "searchEmptyState(\"pm-pipeline\"", "role=\"table\""] },
    { file: "app.js", terms: ["pipelineViewHelpers", "function pipelineViewCall", "function renderPipeline", "pipelineViewCall(\"renderPipelineHTML\"", "function openPipelineCellSheet"] },
    { file: "index.html", terms: ["./search-empty-state.js", "./team-view.js", "./pipeline-view.js", "./app.js"] },
    { file: "scripts/package-release.mjs", terms: ["runtimeAssets", "pipeline-view.js"] },
    { file: "scripts/verify-release.mjs", terms: ["pipeline-view.js"] },
    { file: "scripts/smoke-release.mjs", terms: ["pipeline-view.js", "pipeline_view_cache_no_cache"] },
    { file: "scripts/smoke-a11y.mjs", terms: ["pipeline_view_module_loaded", "pipeline_matrix_table_semantics"] },
    { file: "scripts/smoke-interactions.mjs", terms: ["pipelineViewModule", "joopark-pipeline-view/v1", "pipeline view runtime module was not loaded"] },
    { file: "scripts/check-app-structure.mjs", terms: ["pipelineViewText", "pipeline-view.js", "joopark-pipeline-view/v1"] },
    { file: "README.md", terms: ["pipeline-view.js", "자산 × 워크스트림 파이프라인 보드", "정적 런타임 헬퍼"] },
  ].map((item) => ({ file: item.file, missingTerms: hasTerms(item.file, item.terms).missing }));
  checklist.push({
    id: "pipeline_view_runtime_module",
    requirement: "Pipeline asset-by-workstream matrix, cell drill-down sheet, search hooks, and matrix table semantics are extracted into a packaged runtime helper while pipeline state and sheet actions remain in app.js.",
    status: pipelineViewModuleTerms.every((item) => item.missingTerms.length === 0) ? "pass" : "fail",
    evidence: pipelineViewModuleTerms,
  });
  ...
  const commandPaletteModuleTerms = [
	    { file: "command-palette.js", terms: ["JooParkCommandPalette", "joopark-command-palette/v1", "function createCommandPalette", "function buildItems", "function render", "function setIndex", "function runIndex", "aria-activedescendant", "function issueSourceTone", "const kanbanSourceCount", "const personalSourceCount", "commandCountSub"] },
    ...
    { file: "scripts/smoke-a11y.mjs", terms: ["command_palette_module_loaded", "joopark-command-palette/v1"] },
	    { file: "scripts/smoke-interactions.mjs", terms: ["commandPaletteModule", "joopark-command-palette/v1", "command palette runtime module was not loaded", "commandPalettePersonalSourceCounts", "commandPaletteKanbanSourceCounts", "source filter palette command did not expose result count", "kanban source filter palette command did not expose result count"] },
  ...
  const mobilePaletteLayoutTerms = [
    ...
    { file: "scripts/smoke-mobile.mjs", terms: ["paletteMobileReport", "smoke-palette-mobile", "palette_body_not_locked", "palette_body_lock_not_cleared", "palette_panel_outside_viewport", "palette_quick_capture_outside_viewport", "palette_quick_capture_touch_target_too_short", "palette_mobile_issue_count"] },
  ].map((item) => ({ file: item.file, missingTerms: hasTerms(item.file, item.terms).missing }));
  checklist.push({
    id: "mobile_palette_layout_stability",
    requirement: "The command palette fits inside narrow mobile viewports, locks background scrolling, keeps its input and option rows touchable, scrolls only the results region, and checks quick-capture fallback placement in mobile smoke.",
  ...
  const homeFirstRunEmptyGuidanceTerms = [
    { file: "app.js", terms: ["function renderHome", "firstRunSteps", "firstRunGuidedStartItems", "firstRunGuidedStartCoverage", "projectFollowThroughSteps", "defaultMilestone", "first_issue", "first_milestone", "first_owner", "무엇을 관리하나", "다음 행동", "안전망 확인", "오늘 업무 캡처", "프로젝트 구조화", "⌘K 팔레트 검색", "백업/복구 준비", "data-home-empty=\"${key}\""] },
  ...
  const paletteComboboxStatusTerms = [
    ...
    { file: "scripts/smoke-a11y.mjs", terms: ["command_palette_module_loaded", "palette_input_describedby_hint_status", "palette_body_lock", "palette_body_lock_cleared", "palette_quick_capture_option_count", "palette_quick_capture_active_descendant", "palette_quick_capture_status_announces", "palette_close_clears_status"] },
  ].map((item) => ({ file: item.file, missingTerms: hasTerms(item.file, item.terms).missing }));
  checklist.push({
    id: "palette_combobox_status_recovery",
    requirement: "The command palette combobox links keyboard help and live result status, falls back to quick-capture options on no-match queries, keeps the active descendant in sync, and has a11y smoke coverage.",
  ...
  const vendorTerms = [
    ...
    { file: "vendor/purify.min.js", terms: ["DOMPurify 3.4.11"] },
    { file: "vendor/LICENSES.md", terms: ["marked | 18.0.5", "DOMPurify | 3.4.11", "Fuse.js는 7.x부터 UMD"] },
    { file: "README.md", terms: ["marked](https://github.com/markedjs/marked) | 18.0.5", "DOMPurify](https://github.com/cure53/DOMPurify) | 3.4.11"] },
...
try {
  ...
  const payload = redactAuditPayload(audit());
