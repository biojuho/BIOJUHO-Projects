(function (root) {
  ...
  function createKeyboardShortcuts(deps) {
    ...
    function handleKeydown(event) {
      ...
      // The project picker is a focusable overlay whose options are plain buttons,
      // so single-key shortcuts must be suppressed over it too (else Tab into it
      // then `n`/`/`/`g h` fires app actions behind the open picker).
      const dialogOpen = isPaletteOpen() || callbacks.projectPickerIsOpen() || callbacks.isModalOpen() || callbacks.isSheetOpen();
      ...
      const gChordPending = lastGTime && (Date.now() - lastGTime) < 1200;
      // Calendar m/w/d mode switch must yield to a pending g-chord, otherwise
      // `g m`/`g w`/`g d` (navigate to notes/wiki/db) is dead on the calendar view.
      if (!gChordPending && currentView === "cal" && ["m", "w", "d"].includes(key) && !event.metaKey && !event.ctrlKey && !event.altKey) {
        event.preventDefault();
