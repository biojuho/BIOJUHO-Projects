...
const _mdCacheMax = 100;
function renderMarkdown(src) {
  const text = src == null ? "" : String(src).trim();
  if (!text) return "";
  if (_mdCache.has(text)) {
    // True LRU: re-insert so a frequently-reused note isn't FIFO-evicted while hot.
...
function restoreDeletedRecord(entry) {
  ...
  switch (entry.kind) {
    ...
    case "issue": {
      // Don't resurrect an orphan: if the parent project was deleted while this
      // issue sat in the recycle bin, restoring would create an invisible record
      // (kanban filters by project) that still inflates counts. Refuse clearly.
      if (record.project && !recordById(dashboard.projects, record.project)) {
        showToast("이 이슈의 프로젝트가 삭제되어 복구할 수 없습니다", "warn");
        return false;
      }
      // Drop a dangling assignee whose member was deleted (issue stays valid).
      if (record.assignee && !recordById(dashboard.team, record.assignee)) record.assignee = "";
    ...
    case "task": {
      ...
      if (record.project && !recordById(dashboard.projects, record.project)) {
        showToast("이 작업의 프로젝트가 삭제되어 복구할 수 없습니다", "warn");
        return false;
      }
      if (record.owner && !recordById(dashboard.team, record.owner)) record.owner = "";
      // Scrub dependencies that point at tasks no longer present.
      if (Array.isArray(record.deps)) record.deps = record.deps.filter((depId) => taskById(depId));
...
/* Deep-link into the Settings backup panel (same retry pattern as the
 * recently-deleted recovery deep link below). */
function openBackupPanel() {
  if (dashboard.currentView !== "settings") setView("settings");
  else renderSettings();
  let focused = false;
  const focusPanel = () => {
    const panel = nodeQuery(document, "[data-settings-backup]");
    if (!panel) return;
    focused = true;
...
const VIEWS = ["home", "cal", "todo", "notes", "habits", "stats", "llm-wiki", "pm-portfolio", "pm-kanban", "pm-gantt", "pm-team", "pm-pipeline", "dbm-instances", "dbm-schema", "dbm-queries", "dbm-backups", "settings", "system"];
    ...
    <section class="panel home-hero">
      ...
      <div class="home-activation">
        <div class="home-hero-actions">
          <button type="button" class="primary-btn" data-action="todo-add" title="새 할 일을 빠르게 추가합니다">+ 할 일</button>
          <button type="button" data-action="cal-add" data-date="${today}" title="오늘 날짜에 새 일정을 추가합니다">+ 일정</button>
          <button type="button" data-action="note-add" title="새 메모를 작성합니다">+ 메모</button>
          <button type="button" class="secondary-btn" data-action="nav-to" data-view="pm-portfolio" title="프로젝트 포트폴리오로 이동합니다">포트폴리오</button>
          <button type="button" class="secondary-btn" data-action="nav-to" data-view="dbm-instances" title="DB 카탈로그로 이동합니다">DB 카탈로그</button>
        </div>
...
function homeTileHTML(title, subtitle, viewName, body) {
  return html`
    <article class="panel home-tile">
      <div class="panel-head">
        <div><h2>${title}</h2><a href="${viewHref(viewName)}" data-action="nav-to" data-view="${viewName}" title="${title} 뷰로 이동합니다">전체 보기 ›</a></div>
...
function homeEmptyHTML(key, title, description, action, label) {
  return html`
    <div class="home-empty" data-home-empty="${key}">
      ...
      <button type="button" class="small-action" data-action="${action}" title="${label}">${label}</button>
    </div>
  `;
}
...
function homeExecutionTypeCounts(items) {
  ...
  let issueCount = 0;
  list.forEach((item) => {
    if (item.type === "todo") todoCount += 1;
    else if (item.type === "issue") issueCount += 1;
  });
  return { todoCount, issueCount };
}
...
function homeExecutionDueStateCounts(items) {
  ...
  let upcomingCount = 0;
  list.forEach((item) => {
    if (item.dueState === "overdue") overdueCount += 1;
    else if (item.dueState === "today") todayCount += 1;
    else if (item.dueState === "upcoming") upcomingCount += 1;
  });
...
function homeExecutionQueueModel({ today, weekEnd, openTodos, bucketFilter = "all" }) {
  const todoItems = openTodos
    .filter((todo) => todo.due && todo.due <= weekEnd)
    .map((todo) => {
...
function advanceHomeExecutionIssue(id, nextStatus) {
  ...
  // Snapshot the original slot (the card that sat just below this one) so undo
  // restores the exact position instead of dropping it at the bottom of the lane.
  const previousLane = sortedKanbanLaneIssues(issue.project, previousStatus);
  const previousIndex = previousLane.findIndex((it) => it.id === id);
  const restoreBeforeId = previousIndex >= 0 && previousIndex + 1 < previousLane.length
    ? previousLane[previousIndex + 1].id
    : null;
  ...
  showUndoToast(`이슈를 '${ISSUE_STATUS_LABELS[nextStatus]}'으로 이동했습니다`, () => {
    ...
    insertIssueIntoKanbanLane(current, previousStatus, restoreBeforeId ? { beforeId: restoreBeforeId } : { position: "bottom" });
    commit();
...
function homeFirstRunGuidanceModel({
  ...
  const firstRunSteps = [
    ...
    },
    {
      ...
      status: "ready",
      metric: "⌘K · Ctrl+K",
      detail: "커맨드 팔레트로 화면 이동과 레코드 검색을 하고, 입력한 문장을 그대로 새 할 일/메모로 만듭니다.",
      action: "open-palette",
      actionLabel: "팔레트 열기",
      viewName: "",
    },
    {
      ...
      status: "ready",
      metric: "local JSON",
      detail: "Settings의 데이터 백업 패널로 바로 이동해 JSON 내보내기, 가져오기 제한, 저장소 상태를 확인합니다.",
      action: "open-backup-panel",
      actionLabel: "백업 패널 열기",
      viewName: "settings",
    },
  ];
  ...
  const firstRunGuidedStartItems = [
    ...
    },
    {
      ...
      label: "안전망 확인",
      status: "ready",
      metric: "undo 8초 · 복구 30일",
      detail: "삭제는 undo 토스트와 Settings의 최근 삭제 복구함이 보호합니다. 실수해도 데이터가 바로 사라지지 않습니다.",
      action: "open-deleted-recovery",
    },
  ];
...
function homeTodayCommandContentHTML({ todaysEvents, overdueTodos, todayTodos, upcoming }) {
  ...
  const todayTodosHTML = todayTodoList.length
      ? html`<div class="home-today-todos">${todayTodoList.slice(0, 6).map((t) => raw(html`
        <div class="agenda-todo-row">
          ...
          <button type="button" class="agenda-todo-open" data-action="open-todo" data-todo-id="${t.id}" title="${t.title}: ${dueLabel(t.due).text}${t.done ? ' (완료됨)' : ''}">
  ...
  const upcomingHTML = upcoming.length
    ? html`<ul class="home-upcoming">${upcoming.map((e) => {
        ...
        return raw(html`<li>
          <button type="button" class="home-upcoming-open" data-action="open-event" data-event-id="${openId}" title="${e.title}: ${formatKoreanShort(e.date)} ${eventTimeLabel(e)} (${c.label || e.category})">
...
/* * View: Pipeline (제약 파이프라인 보드 — 자산 × 워크스트림 매트릭스) */
const pipelineViewHelpers = window.JooParkPipelineView && typeof window.JooParkPipelineView.create === "function"
  ? window.JooParkPipelineView.create({ html, raw, escapeHtml, matches, kpiCard, panelHead, searchEmptyState })
  : null;

function pipelineViewCall(name, payload) {
  return callModuleHelper(pipelineViewHelpers, "Pipeline view", name, [payload]);
}

function renderPipeline() {
  const view = refs.views["pm-pipeline"];
  if (!view) return;
  const pipeline = dashboard.pipeline || { assets: [], cells: {} };
  setHTML(view, pipelineViewCall("renderPipelineHTML", {
...
function openPipelineCellSheet(assetId, wsKey) {
  const pipeline = dashboard.pipeline;
  if (!pipeline || !pipelineViewHelpers) return;
  const asset = (Array.isArray(pipeline.assets) ? pipeline.assets : []).find((a) => a && a.id === assetId);
  const ws = (pipelineViewHelpers.WORKSTREAMS || []).find((w) => w.key === wsKey);
  if (!asset || !ws) return;
  const cell = pipeline.cells && typeof pipeline.cells === "object" ? pipeline.cells[`${assetId}:${wsKey}`] || null : null;
  openSheet(`${asset.id} · ${ws.label}`, "", null, { bodyHTML: pipelineViewCall("cellSheetBody", { asset, ws, cell }) });
}
...
const EVENT_CATS = {
  work:     { label: "업무", color: "var(--blue)" },
  meeting:  { label: "회의", color: "var(--violet)" },
  personal: { label: "개인", color: "var(--green)" },
...
function expandOccurrences(rangeStartISO, rangeEndISO) {
  ...
  for (const master of dashboard.events) {
    ...
    // Fast-forward the first step to just before the range start so the cap
    // bounds occurrences inside the range, not the age of the series — a
    // daily event started years ago must still surface in current ranges.
    let step = 0;
    if (master.date < rangeStartISO) {
      const gapDays = daysBetweenLocal(master.date, rangeStartISO);
      if (repeat === "daily") {
        step = Math.max(0, gapDays);
      } else if (repeat === "weekly") {
        step = Math.max(0, Math.floor(gapDays / 7));
      } else if (repeat === "monthly") {
        const [my, mm] = master.date.split("-").map(Number);
        const [ry, rm] = rangeStartISO.split("-").map(Number);
        const monthDiff = (ry - my) * 12 + (rm - mm);
        // Start one month early so day-clamping near month boundaries can
        // never skip the first in-range occurrence.
        step = Math.max(0, monthDiff - 1);
      }
    }
    const stepCap = step + MAX_STEPS;
    while (step < stepCap) {
      let occISO;
...
function normalizeAllData() {
  // ---- 파이프라인 (자산 × 워크스트림 매트릭스) ----
  // 객체형 슬라이스라 배열로 가정하는 코드와 충돌하지 않게 형을 강제하고,
  // 미지 자산을 참조하는 셀은 버린다(1D→2D 가정 방어).
  if (!dashboard.pipeline || typeof dashboard.pipeline !== "object" || Array.isArray(dashboard.pipeline)) {
    dashboard.pipeline = { assets: [], cells: {} };
  }
  if (!Array.isArray(dashboard.pipeline.assets)) dashboard.pipeline.assets = [];
  if (!dashboard.pipeline.cells || typeof dashboard.pipeline.cells !== "object" || Array.isArray(dashboard.pipeline.cells)) {
    dashboard.pipeline.cells = {};
  } else {
    const knownAssetIds = new Set(dashboard.pipeline.assets.map((a) => a && a.id));
    Object.keys(dashboard.pipeline.cells).forEach((key) => {
      if (!knownAssetIds.has(String(key).split(":")[0])) delete dashboard.pipeline.cells[key];
    });
  }
  // ---- 개인 데이터 (personal) ----
...
function updateNavCounts() {
  ...
  setText("navCountPipeline", (dashboard.pipeline && Array.isArray(dashboard.pipeline.assets)) ? dashboard.pipeline.assets.length : 0);
  // DB nav badges
...
function saveEventFromForm(id) {
  ...
  if (id) {
    ...
    if (ev) {
      // Exceptions are absolute dates of skipped occurrences, so they only stay
      // valid while the recurrence identity (rule type + anchor date) is unchanged.
      // Editing title/time/memo keeps the skip list; changing repeat or the anchor
      // date invalidates it (else old skips become phantom gaps on the new series).
      const recurrenceChanged = (ev.repeat || "none") !== repeat || (ev.date || "") !== date;
      const exceptions = (!recurrenceChanged && Array.isArray(ev.exceptions)) ? ev.exceptions : [];
...
function openTodoModal(arg) {
  ...
  const form = html`
    <form id="todoForm" class="modal-form">
      ...
      ${t && t.id ? raw(modalDeleteButtonHTML({ action: "delete-todo", dataAttr: "data-todo-id", dataValue: t.id, label: "이 할 일 삭제" })) : ""}
    </form>
  `;
  // Records without an id are palette quick-capture prefills — treat as new.
  openModal(t && t.id ? "할 일 편집" : "새 할 일", form, () => saveTodoFromForm(t && t.id ? t.id : null));
}
...
function rescheduleTodo(id, mode) {
  const t = todoById(id);
  if (!t) return;
  t.due = todoViewCall("rescheduleDue", t.due, mode, todayISO());
  commit();
  showToast(`마감일을 ${formatKoreanShort(t.due)}(으)로 옮겼습니다`, "info");
}

function rescheduleOverdueTodos() {
  const today = todayISO();
  const overdue = dashboard.todos.filter((t) => !t.done && t.due && t.due < today);
  if (!overdue.length) return;
  const prevDues = overdue.map((t) => ({ id: t.id, due: t.due }));
  overdue.forEach((t) => { t.due = today; });
  commit();
  showUndoToast(`기한 지난 할 일 ${overdue.length}건을 오늘로 옮겼습니다`, () => {
    prevDues.forEach((entry) => {
      const t = todoById(entry.id);
      if (t) t.due = entry.due;
    });
    commit();
  });
}
...
function openNoteModal(arg) {
  ...
  const form = html`
    <form id="noteForm" class="modal-form">
      ...
      ${raw(notesViewCall("noteModalModeToggleHTML"))}
      <label data-note-modal-edit>내용 <small class="field-hint">Markdown 지원</small>
        <textarea name="body" rows="6" maxlength="4000" placeholder="메모를 입력하세요  ·  **굵게**, - 목록, [링크](url), \`코드\`">${n ? n.body || "" : ""}</textarea>
      </label>
      <div class="note-body markdown-body note-modal-preview" data-note-modal-preview hidden></div>
      ...
      ${n && n.id ? raw(modalDeleteButtonHTML({ action: "delete-note", dataAttr: "data-note-id", dataValue: n.id, label: "이 메모 삭제" })) : ""}
    </form>
  `;
  // Records without an id are palette quick-capture prefills — treat as new.
  openModal(n && n.id ? "메모 편집" : "새 메모", form, () => saveNoteFromForm(n && n.id ? n.id : null));
  const noteForm = nodeQuery(document, "#noteForm");
  if (noteForm) {
    const modeBar = nodeQuery(noteForm, "[data-note-modal-mode-bar]");
    const editPane = nodeQuery(noteForm, "[data-note-modal-edit]");
    const previewPane = nodeQuery(noteForm, "[data-note-modal-preview]");
    const bodyInput = nodeQuery(noteForm, "textarea[name=body]");
    if (modeBar && editPane && previewPane && bodyInput) {
      modeBar.addEventListener("click", (event) => {
        const modeBtn = event.target.closest("[data-note-modal-mode]");
        if (!modeBtn) return;
        const preview = modeBtn.dataset.noteModalMode === "preview";
        // Re-render from the live textarea value on every switch so the
        // preview never shows stale content.
        if (preview) setHTML(previewPane, notesViewCall("noteModalPreviewHTML", bodyInput.value));
        previewPane.hidden = !preview;
        editPane.style.display = preview ? "none" : "";
        modeBar.querySelectorAll("[data-note-modal-mode]").forEach((button) => {
          const active = (button.dataset.noteModalMode === "preview") === preview;
          button.classList.toggle("is-active", active);
          button.setAttribute("aria-pressed", String(active));
        });
        if (!preview) bodyInput.focus();
      });
    }
  }
}
...
function habitStreak(habit) {
  ...
  // Current streak: count back while log[date] is true. Today is given grace —
  // an unchecked today doesn't break a run that ends yesterday, so a live streak
  // isn't shown as 0 every morning before the user checks in.
  let current = 0;
  let cursor = log[today] ? today : addDaysISO(today, -1);
  while (log[cursor]) {
    current += 1;
...
const VIEW_RENDERERS = {
  ...
  "llm-wiki": renderLlmWiki, "pm-portfolio": renderPortfolio, "pm-kanban": renderKanban, "pm-gantt": renderGantt, "pm-team": renderTeam, "pm-pipeline": renderPipeline,
...
// Route-change a11y: enabled only after the initial view is mounted so the very
// first paint never steals focus or fires a spurious announcement (WCAG 2.4.3 +
// SPA dynamic-content announcement). Toggled true right after the boot setView().
let routeNavReady = false;
...
function refreshViewPostRender(name, options = {}) {
  if (name === "settings" || name === "system") refreshStorageHealth({ render: true });
  scrollMainToTop();
  if (options.viewChanged) announceRouteChange(name);
}

// On a genuine client-side route change, move keyboard focus into the new view's
// heading and announce the view to assistive tech. SPAs swap content without a
// page load, so without this screen-reader users get no signal and keyboard focus
// is stranded on the (now stale) trigger. Guarded so it never runs on first paint,
// never disrupts active text entry, and yields to any explicit focus the caller sets.
function announceRouteChange(name) {
  if (!routeNavReady) return;
  const active = document.activeElement;
  const tag = active && active.tagName ? active.tagName.toLowerCase() : "";
  const typing = tag === "input" || tag === "textarea" || tag === "select" || (active && active.isContentEditable);
  const viewEl = refs.views && refs.views[name];
  const heading = viewEl ? viewEl.querySelector("h1, h2") : null;
  // Prefer moving focus to the new view's heading: it lands keyboard users in the
  // content AND makes the screen reader announce it, so a separate live-region
  // message would double-announce. Fall back to the live region only when we
  // can't move focus to a heading (mid-typing, or the view has no heading).
  if (!typing && heading) {
  ...
  if (!typing && viewEl) {
    ...
    try { viewEl.focus({ preventScroll: true }); } catch (_) { viewEl.focus(); }
  }
  const announcer = document.getElementById("routeAnnouncer");
  if (announcer) announcer.textContent = `${viewLabel(name)} 화면`;
}
...
function setView(name, options = {}) {
  ...
  renderCurrentView();
  refreshViewPostRender(name, { viewChanged: previous !== name });
}
...
function deleteProject(id) {
  ...
  `, () => {
    ...
    // 간트 작업 삭제 (삭제되는 작업 id를 먼저 모아 둔다)
    const removedTaskIds = new Set(dashboard.gantt.tasks.filter((t) => t.project === id).map((t) => t.id));
    dashboard.gantt.tasks = dashboard.gantt.tasks.filter((t) => t.project !== id);
    // 살아남은 작업의 deps에서 삭제된 작업 참조 제거 (deleteTask의 정리 계약과 일치)
    if (removedTaskIds.size) {
      dashboard.gantt.tasks.forEach((t) => {
        if (Array.isArray(t.deps)) t.deps = t.deps.filter((depId) => !removedTaskIds.has(depId));
      });
    }
    // 팀원 projects 목록에서 제거
...
const APP_SHELL_ACTION_HANDLERS = new Map([
  ...
  ["open-backup-panel", openBackupPanel],
...
const TODO_ACTION_HANDLERS = new Map([
  ...
  ["todo-reschedule", (target) => rescheduleTodo(target.dataset.todoId, target.dataset.dueMode)],
  ["todo-reschedule-overdue", rescheduleOverdueTodos],
...
const PIPELINE_ACTION_HANDLERS = new Map([
...
function setup() {
  ...
  routeNavReady = true; // initial view mounted; subsequent route changes manage focus + announce
