...
(function initJooParkImportGuards(global) {
  ...
  const IMPORT_RECORD_LIMITS = [
    ...
    { key: "pipelineAssets", label: "파이프라인 자산", max: 200, count: importPipelineAssetCount },
    { key: "pipelineCells", label: "파이프라인 셀", max: 2000, count: importPipelineCellCount },
  ...
  const COLLECTION_SCHEMAS = {
    ...
    projects: {
      ...
      fields: {
        ...
        benchmarkFocus: { type: "benchmarkObject" },
        workspaceBenchmark: { type: "benchmarkObject" },
        knowledgeBaseBenchmark: { type: "benchmarkObject" },
      },
    },
    issues: {
      label: "이슈",
  ...
  function importPipelineAssetCount(obj) {
    const pipeline = obj && obj.pipeline;
    return isPlainObject(pipeline) && Array.isArray(pipeline.assets)
      ? pipeline.assets.length
      : null;
  }

  function importPipelineCellCount(obj) {
    const pipeline = obj && obj.pipeline;
    return isPlainObject(pipeline) && isPlainObject(pipeline.cells)
      ? Object.keys(pipeline.cells).length
      : null;
  }
  ...
  // Adoption-candidate projects carry benchmark objects ({surface, flow, signals[],
  // rubric:[{axis,value,weight,score}]}) that are persisted and exported. Validate
  // them leniently (app-controlled, reseeded on load) so the app's own backup
  // round-trips instead of being fatally rejected as an unknown field.
  function validateBenchmarkObject(value, rule, path, violations) {
    if (value === undefined || value === null) return value;
    if (!isPlainObject(value)) {
      addViolation(violations, path, "객체여야 합니다", true);
      return {};
    }
    const numberOrZero = (raw) => {
      const n = Number(raw);
      return Number.isFinite(n) ? n : 0;
    };
    const rubricSource = Array.isArray(value.rubric) ? value.rubric : [];
    return {
      surface: validateString(typeof value.surface === "string" ? value.surface : "", { max: 200 }, `${path}.surface`, violations),
      flow: validateString(typeof value.flow === "string" ? value.flow : "", { max: 200 }, `${path}.flow`, violations),
      signals: validateStringArray(Array.isArray(value.signals) ? value.signals : [], { maxItems: 20, max: 200 }, `${path}.signals`, violations),
      rubric: rubricSource.slice(0, 20).map((item, index) => {
        const itemPath = `${path}.rubric[${index}]`;
        if (!isPlainObject(item)) {
          addViolation(violations, itemPath, "객체여야 합니다", true);
          return {};
        }
        return {
          axis: validateString(typeof item.axis === "string" ? item.axis : "", { max: 120 }, `${itemPath}.axis`, violations),
          value: validateString(typeof item.value === "string" ? item.value : "", { max: 300 }, `${itemPath}.value`, violations),
          weight: numberOrZero(item.weight),
  ...
  const FIELD_VALIDATORS = {
    ...
    benchmarkObject: validateBenchmarkObject,
  };
