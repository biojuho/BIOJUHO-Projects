...
class KDDFCrawler:
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    notices.append(
                        {"title": title, "url": urljoin(self.BASE_URL, link), "date": date_str, "source": "KDDF"}
                    )
                except Exception:
                    continue
    ...
    async def fetch_notice_detail(self, url: str) -> RFPDocument | None:
        """공고 상세 내용 가져오기"""
        if not CRAWLING_AVAILABLE:
            return None
...
# Singleton
_kddf_crawler: KDDFCrawler | None = None
