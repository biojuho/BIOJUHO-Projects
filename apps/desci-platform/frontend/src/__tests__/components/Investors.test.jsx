...
vi.mock('../../contexts/LocaleContext', () => {
  const EN = {
    'investors.directory': 'Investor Directory',
    'investors.titlePrefix': 'Investor',
    'investors.titleHighlight': 'Directory',
    'investors.subtitle': 'Browse {count} bio-focused investors.',
    'investors.searchPlaceholder': 'Search name, thesis, or keyword',
    'investors.filterCountry': 'Filter by country',
    'investors.allCountries': 'All countries',
    'investors.filterStage': 'Filter by stage',
    'investors.allStages': 'All stages',
    'investors.showingCount': 'Showing {shown} of {total}',
    'investors.emptyTitle': 'No investors match your filters',
    'investors.emptyDescription': 'Try adjusting your search or filters.',
    'investors.loadFailed': 'Could not load investor directory',
    'investors.websiteUnavailable': 'Website unavailable',
    'investors.emailUnavailable': 'Email unavailable',
  };
  const t = (key, vars) => {
    let s = EN[key] ?? (key.startsWith('investors.country.') ? key.split('.').pop() : key);
    if (vars) for (const [k, v] of Object.entries(vars)) s = s.replaceAll(`{${k}}`, String(v));
    return s;
  };
  return { useLocale: () => ({ locale: 'en-US', t }) };
});
...
describe('Investors page', () => {
  ...
  it('renders mailto and website links per card', async () => {
    ...
    const websiteAnchor = screen.getByTestId('investor-website-vc-test-001');
    expect(websiteAnchor).not.toBeNull();
    expect(websiteAnchor.getAttribute('href')).toMatch(/^https:\/\/acme\.example/);
