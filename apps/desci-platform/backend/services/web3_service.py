...
class Web3Service:
        ...
        try:
            ...
            if self.token_address:
                self.token_contract = self.w3.eth.contract(
                    address=Web3.to_checksum_address(self.token_address), abi=DESCI_TOKEN_ABI
                )

            if self.nft_address:
                self.nft_contract = self.w3.eth.contract(
                    address=Web3.to_checksum_address(self.nft_address), abi=RESEARCH_PAPER_NFT_ABI
                )
    ...
    async def get_balance(self, address: str) -> dict:
        ...
        try:
            ...
            return {"address": address, "balance": str(balance), "balance_wei": str(balance_wei), "symbol": "DSCI"}
        except Exception as e:
    ...
    async def distribute_custom_reward(self, user_address: str, amount: int, reason: str) -> dict:
        """커스텀 보상 지급"""
        ...
        try:
            ...
            return {"success": True, "tx_hash": tx_hash.hex(), "user": user_address, "amount": amount, "reason": reason}
    ...
    async def _send_reward_tx(self, function_name: str, user_address: str) -> dict:
        ...
        try:
            ...
            return {"success": True, "tx_hash": tx_hash.hex(), "user": user_address, "reward_type": function_name}
    ...
    async def mint_paper_nft(self, user_address: str, token_uri: str, consent_hash: str = None) -> dict:
        ...
        try:
            async with self.lock:
                ...
                tx = self.nft_contract.functions.mintPaper(checksum_addr, token_uri).build_transaction(
                    {
                        "from": self.account.address,
                        "nonce": self.w3.eth.get_transaction_count(self.account.address),
                        "gas": 200000,
