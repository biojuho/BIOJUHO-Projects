...
def test_release_handoff_provider_templates_split_unresolved_keys_by_target(tmp_path: Path) -> None:
    product_payload = _product_smoke_handoff_payload(
        [
            {
                "id": "stripe",
    ...
    env.pop("GITLEAKS_LICENSE")
    checks = deploy_readiness.run_checks(env, targets=("railway", "vercel", "github"))
    ...
    github = (output_dir / "github.env").read_text(encoding="utf-8")
    assert "# Provider: Railway" in railway
    assert "STRIPE_SECRET_KEY=" in railway
    assert "GROBID_URL=" in railway
    assert "VITE_API_BASE_URL=" not in railway
    assert "# Provider: Vercel" in vercel
    assert "VITE_API_BASE_URL=" in vercel
    assert "STRIPE_SECRET_KEY=" not in vercel
    assert "# Provider: GitHub" in github
    assert "GITLEAKS_LICENSE=" in github
    assert "not-a-real-test-private-key" not in railway + vercel + github


def test_release_handoff_cli_writes_provider_templates(tmp_path: Path, capsys) -> None:
    product_payload = _product_smoke_handoff_payload(
        [
            {
                "id": "auth",
    ...
    output = capsys.readouterr().out
    assert code == 1
    assert "[release-handoff] provider template written: railway=" in output
    assert (provider_dir / "railway.env").read_text(encoding="utf-8").startswith(
