...
(async () => {
  ...
  await runStep("workspace candidate portfolio search", async () => {
    ...
    assert(bookStackText.includes("documentation") && bookStackText.includes("Codeberg"), "BookStack candidate description did not render");
    ...
    assert(qs("[data-candidate-action]", bookStackCard).textContent.includes("스파이크"), "BookStack candidate action did not render spike recommendation");
