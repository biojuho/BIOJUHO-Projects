...
(async () => {
  ...
  await runStep("workspace candidate portfolio search", async () => {
    ...
    assert(outlineText.includes("Markdown") && outlineText.includes("Slack"), "Outline candidate description did not render");
    ...
    assert(qs("[data-candidate-action]", outlineCard).textContent.includes("리스크 리뷰"), "Outline candidate action did not render risk review");
