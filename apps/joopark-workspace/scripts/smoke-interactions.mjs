...
(async () => {
  ...
  await runStep("workspace candidate portfolio search", async () => {
    ...
    assert(/^[0-9a-f]{40}$/i.test(veritasCandidate.lastCommit || ""), "Veritas AutoResearch candidate commit was missing");
    assert(!Number.isNaN(Date.parse(veritasCandidate.pushedAt || "")), "Veritas AutoResearch candidate pushedAt was missing");
    const veritasShortCommit = veritasCandidate.lastCommit.slice(0, 8);
    ...
    fill("#globalSearch", veritasShortCommit);
    await waitFor(() => state.query === veritasShortCommit && document.querySelectorAll("#view-pm-portfolio .portfolio-card").length === 1, "Veritas commit search did not filter portfolio");
    ...
    assert(veritasText.includes("최신"), "Veritas candidate description did not render");
    const veritasCommit = qs("[data-candidate-commit]", veritasCard);
    assert(veritasCommit.dataset.candidateCommit === veritasShortCommit, "Veritas freshness commit did not render");
    assert(veritasCommit.dataset.candidatePushedAt === veritasCandidate.pushedAt, "Veritas pushedAt freshness marker did not render");
    ...
    fill("#globalSearch", "b3a1037b");
    await waitFor(() => state.query === "b3a1037b" && document.querySelectorAll("#view-pm-portfolio .portfolio-card").length === 1, "Leantime commit search did not filter portfolio");
    ...
    assert(leantimeCommit.dataset.candidateCommit === "b3a1037b", "Leantime freshness commit did not render");
    assert(leantimeCommit.dataset.candidatePushedAt === "2026-06-05T04:17:00Z", "Leantime pushedAt freshness marker did not render");
