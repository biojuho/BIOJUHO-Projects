...
(async () => {
  ...
  await runStep("workspace candidate portfolio search", async () => {
    ...
    assert(veritasCandidate.lastCommit === "04714cdc78e2997594cc2daad5a9403d2e4d2b20", "Veritas AutoResearch candidate commit was stale");
    assert(veritasCandidate.pushedAt === "2026-06-05T14:20:51Z", "Veritas AutoResearch candidate pushedAt was stale");
    ...
    fill("#globalSearch", "04714cdc");
    await waitFor(() => state.query === "04714cdc" && document.querySelectorAll("#view-pm-portfolio .portfolio-card").length === 1, "Veritas commit search did not filter portfolio");
    ...
    assert(veritasText.includes("v8.381 최신"), "Veritas candidate description did not render");
    const veritasCommit = qs("[data-candidate-commit]", veritasCard);
    assert(veritasCommit.dataset.candidateCommit === "04714cdc", "Veritas freshness commit did not render");
    assert(veritasCommit.dataset.candidatePushedAt === "2026-06-05T14:20:51Z", "Veritas pushedAt freshness marker did not render");
