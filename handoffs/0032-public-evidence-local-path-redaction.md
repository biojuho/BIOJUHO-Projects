...
## 배경
`node scripts/plan-workflow-ui-install.mjs`와 `node scripts/plan-publish-dispatch.mjs --repo biojuho/BIOJUHO-Projects` dry-run 출력 및 저장된 `data/*.json`, `dist/release/data/*.json`에서 `repositoryRoot`가 `project-root`로 노출된다. 두 data 파일은 package release에 포함되는 공개 산출물이므로 privacy leak이다.
...
- `rg -n "<local-home>|/Users/" data/workflow-ui-install-plan.json data/publish-dispatch-plan.json dist/release/data/workflow-ui-install-plan.json dist/release/data/publish-dispatch-plan.json`가 수정 후 hit 0
- `npm run test:unit`
- `npm run lint`
- `npm run build`
