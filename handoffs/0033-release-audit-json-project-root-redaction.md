...
## 배경
0032에서 공개 package evidence의 로컬 경로 노출을 제거했지만, release readiness audit의 직접 JSON 출력은 여전히 `projectRoot`를 `project-root`로 노출한다. 이 출력은 운영 자동화와 외부 handoff에 붙일 수 있는 machine-readable evidence라 같은 redaction 계약이 필요하다. 또한 검증 중 `console.log(JSON.stringify(payload))` 직후 `process.exit(exitCode)`가 호출되어 pipe stdout이 65536바이트 근처에서 잘리고 JSON이 파싱 불가능해지는 현상도 확인됐다.
...
## 수용 게이트
- `node scripts/audit-release-readiness.mjs --format=json | rg -n "<local-home>|/Users/"`
- `node scripts/audit-release-readiness.mjs --format=json | node -e '<parse projectRoot/projectRootRedacted>'`
- `node scripts/audit-release-readiness.mjs --format=markdown | rg -n "<local-home>|/Users/"`
