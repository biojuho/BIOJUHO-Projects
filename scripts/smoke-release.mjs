...
const interactionReleaseSmokeTimeoutMs = positiveMsOption(
  process.env.INTERACTION_RELEASE_SMOKE_TIMEOUT_MS,
  600000,
);
const interactionReleaseRuntimeTimeoutMs = positiveMsOption(
  process.env.INTERACTION_RELEASE_RUNTIME_TIMEOUT_MS || process.env.SMOKE_LONG_SCENARIO_TIMEOUT_MS,
  600000,
);
...
async function smokeReleaseHeaders(baseUrl) {
  ...
  const pipelineView = await fetchHeaderMap(`${baseUrl}/pipeline-view.js`);
  ...
  const headerChecks = {
    ...
    pipeline_view_cache_no_cache: pipelineView.headers["cache-control"] === "no-cache",
...
async function main() {
  ...
  try {
    ...
    const interactionRun = await runRetryableBrowserScript("scripts/smoke-interactions.mjs", {
      BASE_URL: baseUrl,
      SMOKE_PROGRESS: "1",
      SMOKE_LONG_SCENARIO_TIMEOUT_MS: String(interactionReleaseRuntimeTimeoutMs),
      SMOKE_RUNTIME_TIMEOUT_MS: String(Math.min(interactionReleaseRuntimeTimeoutMs, 150000)),
    }, interactionReleaseSmokeTimeoutMs);
