...
(async () => {
  ...
  await runStep("workspace candidate portfolio search", async () => {
    ...
    assert(veritasCandidate.lastCommit === "b0d4177dadce49f78f6978a2a36c777698ca9cb2", "Veritas AutoResearch candidate commit was stale");
    assert(veritasCandidate.pushedAt === "2026-06-05T13:51:51Z", "Veritas AutoResearch candidate pushedAt was stale");
    ...
    fill("#globalSearch", "b0d4177d");
    await waitFor(() => state.query === "b0d4177d" && document.querySelectorAll("#view-pm-portfolio .portfolio-card").length === 1, "Veritas commit search did not filter portfolio");
    ...
    assert(veritasText.includes("v8.374 최신"), "Veritas candidate description did not render");
    const veritasCommit = qs("[data-candidate-commit]", veritasCard);
    assert(veritasCommit.dataset.candidateCommit === "b0d4177d", "Veritas freshness commit did not render");
    assert(veritasCommit.dataset.candidatePushedAt === "2026-06-05T13:51:51Z", "Veritas pushedAt freshness marker did not render");
