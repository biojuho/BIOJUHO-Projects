...
(async () => {
	  ...
	  await runStep("home launch action surfaces current guard", async () => {
	    ...
	    await waitFor(() => {
	      ...
	      const shareProofAction = card &&
	        card.dataset.homeLaunchActionKey === "share-launch-proof" &&
	      ...
	      return card &&
	        ...
	        Number(card.dataset.homeLaunchCommandCount || "0") >= 1 &&
	    ...
	    const isShareProofAction = card.dataset.homeLaunchActionKey === "share-launch-proof";
	    ...
	    assert(text.includes(expectedLaunchCommandNeedle) && (text.includes("remoteWorkflowFilesReady=true") || text.includes("postPublishEvidenceReady=true")), "home launch action command and success condition did not render");
  ...
  await runStep("system publish readiness alignment", async () => {
    ...
    panel = qs("[data-system-publish-readiness]");
	    ...
	    await waitFor(() => outputQuality.dataset.outputQualityAuditSource === "data/output-quality-audit.json" &&
	      ...
	      outputQuality.dataset.outputQualityAuditWorkflowAuthPreflight === "true" &&
	      outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightUiVerified === "true" &&
	      outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightFields === "1" &&
	      ...
	      outputQuality.dataset.outputQualityAuditWorkflowUiInstallReceiptCommandCount === "8" &&
	      ...
	      Number(outputQuality.dataset.outputQualityAuditInstallPathCommandCount || 0) >= 10, "output quality audit state was not surfaced", 15000);
	    ...
	    if (outputQualityReadyForExternalClaim) {
	      ...
	      assert(outputQuality.dataset.outputQualityAuditHandoffVerifierSafeToDispatch === "true" && outputQuality.dataset.outputQualityAuditBlockerResolutionActionRequiredCount === "0" && outputQuality.dataset.outputQualityAuditBlockerResolutionDeferredCount === "0" && outputQuality.dataset.outputQualityAuditInstallPathCommandCount === "13", "output quality blocker resolution ready state was not surfaced");
	      ...
	      assert(qualityInstallPaths.length === 2 && qualityInstallPaths.some((item) => item.textContent.includes("CLI path after workflow scope") && item.textContent.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify")) && qualityInstallPaths.some((item) => item.textContent.includes("GitHub UI path") && item.textContent.includes("pbcopy < 'docs/github-pages-workflow.yml'")), "output quality ready launch install paths were not surfaced");
    ...
    assert(outputQuality.dataset.outputQualityAuditLaunchProofEvidenceReceipt === "true" && outputQuality.dataset.outputQualityAuditLaunchProofEvidenceFields === "6" && outputQuality.dataset.outputQualityAuditLaunchProofEvidenceCoverage === "1", "output quality launch proof evidence receipt state was not surfaced");
	    assert(outputQuality.dataset.outputQualityAuditHandoffVerifierArtifact === "true" && outputQuality.dataset.outputQualityAuditHandoffVerifierArtifactCoverage === "2" && outputQuality.dataset.outputQualityAuditHandoffVerifierSafeToDispatch === "false" && outputQuality.dataset.outputQualityAuditHandoffVerifierJsonPath === "data/launch-handoff-verification.json" && outputQuality.dataset.outputQualityAuditHandoffVerifierMarkdownPath === "data/launch-handoff-verification.md", "output quality handoff verifier artifact state was not surfaced");
		    ...
		    assert(outputQualityWorkflowUiInstallReceiptCommandCount >= 6 && outputQualityWorkflowUiInstallReceiptChecklistCount >= 6, "output quality workflow UI install receipt counts were stale");
    ...
    const outputQualityGoalStatuses = goalChecklistItems.map((item) => item.dataset.outputQualityAuditGoalStatus || "");
	    ...
	    const outputQualityPostInstallProofParserReady = (text) => (
	      ...
	      text.includes("Post-install proof parser: blocked (0 fields, coverage=0)") &&
	      text.includes("detected=0/0") &&
	      text.includes("falsePositiveGuard=false") &&
	      text.includes("not dispatch approval")
	    );
