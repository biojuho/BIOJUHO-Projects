...
(async () => {
  ...
  await runStep("home launch action surfaces current guard", async () => {
    ...
    await waitFor(() => {
      ...
      return card &&
        ...
        card.dataset.homeLaunchCommandCount === "7" &&
        card.dataset.homeLaunchWithheldCount === "2" &&
        text.includes("remoteWorkflowFilesReady=true") &&
        code === "gh auth refresh -h github.com -s workflow";
    ...
    assert(card.dataset.homeLaunchActionStatus === "action_required", "home launch action did not expose action_required status");
    assert(label === "Install workflows on the default branch", "home launch action label dataset did not render: " + label);
    assert(card.dataset.homeLaunchSafeToDispatch === "false", "home launch action should keep dispatch blocked");
    assert(card.dataset.homeLaunchReadyForExternalClaim === "false", "home launch action should block external launch claim");
    assert(card.dataset.homeLaunchTransitionSource === "generated_from_launch_execution_packet" && card.dataset.homeLaunchTransitionCurrentStage === "install_workflows" && card.dataset.homeLaunchTransitionNextStage === "verify_visibility" && card.dataset.homeLaunchTransitionPendingCount === "3" && card.dataset.homeLaunchTransitionGateCommand.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "home launch transition preview dataset was incomplete");
    assert(card.getAttribute("data-home-launch-install-matrix-source") === "generated_from_launch_execution_packet" && card.dataset.homeLaunchInstallMatrixPathCount === "2" && card.dataset.homeLaunchInstallMatrixSignalCount === "6" && card.dataset.homeLaunchInstallMatrixNextStage === "verify_visibility", "home launch install verification matrix dataset was incomplete");
    assert(card.getAttribute("data-home-remote-workflow-file-ledger-source") === "generated_from_remote_workflow_file_check" && card.dataset.homeRemoteWorkflowFileLedgerStatus === "remote_file_install_required" && card.dataset.homeRemoteWorkflowFileLedgerFileCount === "2" && card.dataset.homeRemoteWorkflowFileLedgerReadyCount === "0" && card.dataset.homeRemoteWorkflowFileLedgerMissingCount === "2", "home remote workflow file acceptance ledger dataset was incomplete");
    assert(card.getAttribute("data-home-launch-proof-ledger-source") === "generated_from_launch_execution_packet" && card.dataset.homeLaunchProofLedgerStatus === "proof_blocked_until_dispatch" && card.dataset.homeLaunchProofLedgerRequiredCount === "6" && card.dataset.homeLaunchProofLedgerReadyCount === "0" && card.dataset.homeLaunchProofLedgerPendingCount === "6" && card.dataset.homeLaunchProofLedgerCurrentGate === "capture_launch_proof", "home launch proof acceptance ledger dataset was incomplete");
    assert(text.includes("현재") && normalizedText.includes("install workflows on the default branch") && renderedLabel === label, "home launch action label did not render: " + text.slice(0, 240));
    assert(text.includes("remoteWorkflowFilesReady=true") && text.includes("gh auth refresh -h github.com -s workflow"), "home launch action command and success condition did not render");
    assert(text.includes("next transition") && text.includes("install_workflows -> verify_visibility") && text.includes("Verify workflow visibility") && text.includes("pending 3") && text.includes("dispatch withheld"), "home launch transition preview did not render");
    assert(text.includes("install verification matrix") && text.includes("2 paths -> verify_visibility") && text.includes("6 signals") && text.includes("remoteWorkflowVisibilityReady=true") && text.includes("dispatchReady=true") && text.includes("driftDispatchReady=true") && text.includes("allDispatchReady=true") && text.includes("verify-launch-handoff reports safeToDispatch=true"), "home launch install verification matrix did not render");
    assert(text.includes("remote workflow file acceptance ledger") && text.includes("0/2 files ready") && text.includes("remote_file_install_required") && text.includes("missing 2") && text.includes("remoteMatchesTemplate required"), "home remote workflow file acceptance ledger did not render");
    assert(text.includes("launch proof acceptance ledger") && text.includes("0/6 proofs ready") && text.includes("proof_blocked_until_dispatch") && text.includes("pending 6") && text.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --write"), "home launch proof acceptance ledger did not render");
    ...
    assert(qs("code", card).textContent === "gh auth refresh -h github.com -s workflow", "home launch action code did not expose first safe command");
    ...
    await waitFor(() => {
      ...
      return copied && statusText.includes("복사") &&
        ...
        window.__smokeClipboardText.includes("Active blocker: operator_auth_path") &&
    ...
    const homePostInstallIntakeCopyText = qs("[data-post-install-evidence-intake-text]", homePostInstallIntake).textContent;
	    ...
	    assert(homePostInstallIntake.dataset.homePostInstallEvidenceIntakeReady === "true" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakeStatus === "collect_post_install_proof" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakeProofComplete === "false" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakeCompletedCount === "0" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakePendingCount === "6" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakeFieldCount === "6" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakeCommandCount === "4" && homePostInstallIntake.dataset.homePostInstallEvidenceIntakeSignalCount === "8", "home post-install evidence intake dataset was incomplete");
	    ...
	    assert(homePostInstallIntake.dataset.postInstallEvidenceIntakeReady === "true" && homePostInstallIntake.dataset.postInstallEvidenceIntakeProofComplete === "false" && homePostInstallIntake.dataset.postInstallEvidenceIntakeFieldCount === "6" && homePostInstallIntake.dataset.postInstallQuickProofReady === "true" && homePostInstallIntake.dataset.postInstallQuickProofStepCount === "4" && homePostInstallIntake.dataset.postInstallQuickProofCoverage === "1" && homePostInstallIntake.dataset.postInstallQuickProofFieldMappingReady === "true" && homePostInstallIntake.dataset.postInstallQuickProofFieldMappingCoverage === "1" && homePostInstallIntake.dataset.postInstallQuickProofMappedFieldCount === "4" && homePostInstallIntake.dataset.postInstallQuickProofCompletedMappedFieldCount === "0", "home post-install evidence intake did not reuse copy selectors");
	    ...
	    assert(homePostInstallQuickProofMap.dataset.postInstallQuickProofFieldMappingReady === "true" && homePostInstallQuickProofMap.dataset.postInstallQuickProofFieldMappingCoverage === "1" && homePostInstallQuickProofMap.dataset.postInstallQuickProofMappedFieldCount === "4" && homePostInstallQuickProofMap.dataset.postInstallQuickProofCompletedMappedFieldCount === "0" && homePostInstallQuickProofMapItems.length === 4 && homePostInstallQuickProofMapItems[0].dataset.postInstallQuickProofFieldMapStep === "remote_file_parity" && homePostInstallQuickProofMapItems[0].dataset.postInstallQuickProofFieldMapField === "remote_parity_proof" && homePostInstallQuickProofMapItems[3].dataset.postInstallQuickProofFieldMapStep === "handoff_verifier" && homePostInstallQuickProofMapItems[3].dataset.postInstallQuickProofFieldMapField === "handoff_verifier_proof", "home post-install quick proof field map did not render");
	    assert(homePostInstallFields.length === 6 && homePostInstallFields.some((item) => item.dataset.homePostInstallEvidenceIntakeFieldKey === "remote_parity_proof" && item.textContent.includes("remoteWorkflowFilesReady=false")) && homePostInstallFields.some((item) => item.dataset.homePostInstallEvidenceIntakeFieldKey === "handoff_verifier_proof" && item.textContent.includes("safeToDispatch=false")), "home post-install evidence fields did not render");
    ...
    await waitFor(() => {
      ...
      return copied && statusText.includes("복사") &&
	        ...
	        window.__smokeClipboardText.includes("Quick proof field mapping: ready=true; mapped=4; completed=0/4; coverage=1") &&
    ...
    assert(homeClaimGuard.dataset.homeExternalClaimGuardReady === "false" && homeClaimGuard.dataset.homeExternalClaimGuardStatus === "blocked_external_claim" && homeClaimGuard.dataset.homeExternalClaimGuardBlockedCount === "3" && homeClaimGuard.dataset.homeExternalClaimGuardRequirementCount === "3" && homeClaimGuard.dataset.homeExternalClaimGuardCommandCount === "5" && homeClaimGuard.dataset.homeExternalClaimGuardNextProofKey === "workflow_installation" && homeClaimGuard.dataset.homeExternalClaimGuardNextProofCommand.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "home external claim guard dataset was incomplete");
    assert(homeClaimGuard.dataset.outputQualityAuditExternalClaimGuardStatus === "blocked_external_claim" && homeClaimGuard.dataset.outputQualityAuditExternalClaimGuardBlockedCount === "3", "home external claim guard did not reuse output quality copy selectors");
    assert(homeClaimGuardNextProof.dataset.homeExternalClaimGuardNextProofReady === "false" && homeClaimGuardNextProof.dataset.homeExternalClaimGuardNextProofKey === "workflow_installation" && homeClaimGuardNextProof.dataset.homeExternalClaimGuardNextProofStatus === "blocked" && homeClaimGuardNextProof.textContent.includes("next proof") && homeClaimGuardNextProof.textContent.includes("Workflow installation") && homeClaimGuardNextProof.textContent.includes("remoteWorkflowFilesReady=false") && homeClaimGuardNextProof.textContent.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "home external claim guard next proof shortcut was incomplete");
    assert(homeClaimGuardItems.length === 3 && homeClaimGuardItems.some((item) => item.dataset.homeExternalClaimGuardKey === "workflow_installation" && item.dataset.homeExternalClaimGuardItemStatus === "blocked") && homeClaimGuardItems.some((item) => item.dataset.homeExternalClaimGuardKey === "public_launch_proof") && homeClaimGuardItems.some((item) => item.dataset.homeExternalClaimGuardKey === "external_completion_claim"), "home external claim guard requirements did not render");
    assert(homeClaimGuardSignals.length === 6 && homeClaimGuardSignals.some((item) => item.textContent.includes("allDispatchReady=false")) && homeClaimGuardSignals.some((item) => item.textContent.includes("readyForExternalClaim=false")), "home external claim guard signals did not render");
    ...
    assert(homeClaimGuardText.includes("외부 완료 주장 차단") && homeClaimGuardText.includes("blocked_external_claim") && homeClaimGuardText.includes("Workflow installation") && homeClaimGuardText.includes("Public launch proof") && homeClaimGuardText.includes("External completion claim") && homeClaimGuardText.includes("Stop condition: do not claim readyForExternalClaim"), "home external claim guard text did not render");
    assert(homeClaimGuardCopyText.includes("JooPark External Completion Claim Guard") && homeClaimGuardCopyText.includes("Status: blocked_external_claim") && homeClaimGuardCopyText.includes("Blocked requirements: 3/3") && homeClaimGuardCopyText.includes("Next claim proof shortcut:") && homeClaimGuardCopyText.includes("requirement=workflow_installation") && homeClaimGuardCopyText.includes("command=node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "home external claim guard copy text was not ready");
    ...
    await waitFor(() => {
      ...
      return copied && statusText.includes("복사") &&
        ...
        window.__smokeClipboardText.includes("Status: blocked_external_claim") &&
        window.__smokeClipboardText.includes("Next claim proof shortcut:") &&
        window.__smokeClipboardText.includes("requirement=workflow_installation") &&
  ...
  await runStep("home launch blocker resolver exposes active unblock path", async () => {
    ...
    await waitFor(() => {
      ...
      return resolver &&
        ...
        resolver.dataset.homeLaunchBlockerResolverActive === "operator_auth_path" &&
        resolver.dataset.homeLaunchBlockerResolverStatus === "action_required" &&
        primary === "gh auth refresh -h github.com -s workflow";
    ...
    const workflowInstallShortcut = qs("[data-home-workflow-install-shortcut]", resolver);
    const workflowInstallShortcutPaths = Array.from(workflowInstallShortcut.querySelectorAll("[data-home-workflow-install-shortcut-path]"));
    const workflowInstallShortcutGuards = Array.from(workflowInstallShortcut.querySelectorAll("[data-home-workflow-install-shortcut-guard]"));
    const workflowInstallShortcutCommands = Array.from(workflowInstallShortcut.querySelectorAll("[data-home-workflow-install-shortcut-command]"));
	    ...
	    assert(resolver.dataset.homeLaunchBlockerResolverItemCount === "6" && resolver.dataset.homeLaunchBlockerResolverPassCount === "2" && resolver.dataset.homeLaunchBlockerResolverActionRequiredCount === "3" && resolver.dataset.homeLaunchBlockerResolverDeferredCount === "1" && resolver.dataset.homeLaunchBlockerResolverProofCommandCount === "6" && resolver.dataset.homeLaunchBlockerResolverFallbackCommandCount === "6" && resolver.dataset.homeLaunchBlockerResolverEvidenceGapCount === "3" && resolver.dataset.homeLaunchBlockerResolverRemoteFilesReady === "false" && resolver.dataset.homeLaunchBlockerResolverLaunchProofReady === "false" && resolver.dataset.homeLaunchBlockerResolverPostInstallProofComplete === "false", "home launch blocker resolver counters were incomplete");
	    ...
	    assert(items.length === 6 && items.some((item) => item.dataset.homeLaunchBlockerResolverItemKey === "operator_auth_path" && item.dataset.homeLaunchBlockerResolverItemStatus === "action_required") && items.some((item) => item.dataset.homeLaunchBlockerResolverItemKey === "remote_workflow_file_parity") && items.some((item) => item.dataset.homeLaunchBlockerResolverItemKey === "workflow_visibility") && items.some((item) => item.dataset.homeLaunchBlockerResolverItemKey === "dispatch_guard" && item.dataset.homeLaunchBlockerResolverItemStatus === "pass"), "home launch blocker resolver checklist items did not render");
	    assert(evidenceGapItems.length === 3 && evidenceGapItems.some((item) => item.dataset.homeLaunchBlockerEvidenceGapKey === "remote_workflow_files" && item.dataset.homeLaunchBlockerEvidenceGapReady === "false" && item.textContent.includes("0/2 files ready") && item.textContent.includes("missing=2")) && evidenceGapItems.some((item) => item.dataset.homeLaunchBlockerEvidenceGapKey === "launch_proof" && item.textContent.includes("0/6 proofs ready") && item.textContent.includes("capture_launch_proof")) && evidenceGapItems.some((item) => item.dataset.homeLaunchBlockerEvidenceGapKey === "post_install_intake" && item.textContent.includes("0/6 proof fields complete") && item.textContent.includes("proofComplete=false")), "home launch blocker resolver evidence gaps did not render");
    assert(workflowInstallShortcut.dataset.homeWorkflowInstallShortcutReady === "true" && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutStatus === "workflow_scope_blocked_use_ui_or_refresh" && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutPathCount === "2" && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutCommandCount === "14" && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutTargetCount === "2" && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutPrimaryPath === "github_ui" && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutPrimaryCommand.includes("pbcopy < 'docs/github-pages-workflow.yml'") && workflowInstallShortcut.dataset.homeWorkflowInstallShortcutVerifyCommand.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "home workflow install shortcut dataset was incomplete");
    assert(workflowInstallShortcutPaths.length === 2 && workflowInstallShortcutPaths.some((item) => item.dataset.homeWorkflowInstallShortcutPathKey === "cli_workflow_scope" && item.dataset.homeWorkflowInstallShortcutPathCommandCount === "8" && item.textContent.includes("install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify")) && workflowInstallShortcutPaths.some((item) => item.dataset.homeWorkflowInstallShortcutPathKey === "github_ui" && item.dataset.homeWorkflowInstallShortcutPathCommandCount === "6" && item.textContent.includes("pbcopy < 'docs/github-pages-workflow.yml'") && item.textContent.includes("github.com/biojuho/BIOJUHO-Projects/new/main")), "home workflow install shortcut paths did not render");
    assert(workflowInstallShortcutGuards.length === 2 && workflowInstallShortcutGuards.some((item) => item.textContent.includes("workflow_dispatch requires the workflow file on the repository default branch")) && workflowInstallShortcutGuards.some((item) => item.textContent.includes("workflow scope") && item.textContent.includes("workflowScopeInstallBlocked=true")), "home workflow install shortcut guards did not render");
    assert(workflowInstallShortcutCommands.some((item) => item.textContent.includes("gh auth refresh -h github.com -s workflow")) && workflowInstallShortcutCommands.some((item) => item.textContent.includes("pbcopy < 'docs/github-pages-workflow.yml'")) && qs("[data-home-workflow-install-shortcut-verify]", workflowInstallShortcut).textContent.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "home workflow install shortcut commands did not render");
	    assert(fallbackCommands.length === 6 && fallbackCommands[0].textContent.includes("pbcopy < 'docs/github-pages-workflow.yml'") && fallbackCommands[1].textContent.includes("github.com/biojuho/BIOJUHO-Projects/new/main") && fallbackCommands.some((item) => item.textContent.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write")), "home launch blocker resolver fallback commands did not render");
	    assert(text.includes("launch unblock resolver") && text.includes("Resolve workflow auth path") && text.includes("workflowScopeAvailable=false") && text.includes("workflowScopeInstallBlocked=true") && text.includes("evidence gap") && text.includes("Remote workflow files") && text.includes("Launch proof") && text.includes("Post-install intake") && text.includes("workflow install shortcut") && text.includes("default branch") && text.includes("GitHub UI fallback") && text.includes("Do not run gh workflow run"), "home launch blocker resolver visible text did not render");
	    assert(copyText.includes("JooPark Launch Blocker Resolver") && copyText.includes("activeItemKey=operator_auth_path") && copyText.includes("blockedSignal=workflowScopeAvailable=false; workflowScopeInstallBlocked=true") && copyText.includes("Primary proof command: gh auth refresh -h github.com -s workflow") && copyText.includes("Evidence gap:") && copyText.includes("remote_workflow_files: remote_file_install_required; ready=false; 0/2 files ready") && copyText.includes("launch_proof: proof_blocked_until_dispatch; ready=false; 0/6 proofs ready") && copyText.includes("post_install_intake: collect_post_install_proof; ready=false; 0/6 proof fields complete") && copyText.includes("Workflow install shortcut:") && copyText.includes("JooPark Workflow Install Shortcut") && copyText.includes("Target workflow files: 2") && copyText.includes("Install command count: 14") && copyText.includes("workflow_dispatch requires the workflow file on the repository default branch") && copyText.includes("Writing .github/workflows through the repository contents API requires workflow scope") && copyText.includes("GitHub UI fallback:") && copyText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && copyText.includes("Do not run gh workflow run"), "home launch blocker resolver copy text was not ready");
    ...
    await waitFor(() => {
      ...
      return copied && statusText.includes("복사") &&
	        ...
	        window.__smokeClipboardText.includes("activeItemKey=operator_auth_path") &&
	        window.__smokeClipboardText.includes("workflowScopeInstallBlocked=true") &&
        window.__smokeClipboardText.includes("JooPark Workflow Install Shortcut") &&
        window.__smokeClipboardText.includes("Install paths: 2") &&
  ...
  await runStep("settings operational handoff copy", async () => {
    ...
    assert(settingsLaunchRunbook.dataset.settingsLaunchRunbookReady === "true" && settingsLaunchRunbook.dataset.settingsLaunchRunbookStepCount === "7" && settingsLaunchRunbook.dataset.settingsLaunchRunbookSignalCount === "8" && settingsLaunchRunbook.dataset.settingsLaunchRunbookSafeToDispatch === "false" && settingsLaunchRunbook.dataset.settingsLaunchRunbookReadyForExternalClaim === "false" && settingsLaunchRunbook.dataset.settingsLaunchRunbookWithheldCount === "2" && settingsLaunchRunbook.dataset.settingsLaunchRunbookCurrentStage === "install_workflows" && settingsLaunchRunbook.dataset.settingsLaunchRunbookSource === "data/workflow-ui-install-plan.json" && settingsLaunchRunbook.dataset.settingsLaunchRunbookPacketSource === "data/launch-execution-packet.json", "settings launch runbook data attributes were incomplete");
    assert(settingsLaunchRunbookText.includes("GitHub UI install first, dispatch later") && settingsLaunchRunbookText.includes("Install workflows on the default branch") && settingsLaunchRunbookText.includes("remoteWorkflowFilesReady=true") && settingsLaunchRunbookText.includes("remoteWorkflowVisibilityReady=true") && settingsLaunchRunbookText.includes("safeToDispatch=true before gh workflow run") && settingsLaunchRunbookText.includes("Do not run gh workflow run"), "settings launch runbook copy was incomplete");
    const settingsLaunchTransition = qs("[data-settings-launch-transition-preview]", settingsLaunchRunbook);
    assert(settingsLaunchTransition.dataset.launchTransitionSource === "generated_from_launch_execution_packet" && settingsLaunchTransition.dataset.launchTransitionCurrentStage === "install_workflows" && settingsLaunchTransition.dataset.launchTransitionNextStage === "verify_visibility" && settingsLaunchTransition.dataset.launchTransitionReady === "false" && settingsLaunchTransition.dataset.launchTransitionPendingCount === "3" && settingsLaunchTransition.dataset.launchTransitionWithheldCount === "2", "settings launch transition preview dataset was incomplete");
    assert(settingsLaunchTransition.textContent.includes("Stage transition preview") && settingsLaunchTransition.textContent.includes("install_workflows -> verify_visibility") && settingsLaunchTransition.textContent.includes("conditional next stage") && settingsLaunchTransition.textContent.includes("remoteWorkflowFilesReady=true") && settingsLaunchTransition.textContent.includes("remoteWorkflowVisibilityReady=true") && qs("[data-launch-transition-gate-command]", settingsLaunchTransition).textContent.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "settings launch transition preview did not render");
    ...
    assert(settingsInstallMatrix.dataset.launchInstallVerificationSource === "generated_from_launch_execution_packet" && settingsInstallMatrix.dataset.launchInstallVerificationStatus === "install_verification_required" && settingsInstallMatrix.dataset.launchInstallVerificationPathCount === "2" && settingsInstallMatrix.dataset.launchInstallVerificationSignalCount === "6" && settingsInstallMatrix.dataset.launchInstallVerificationCommandCount === "4" && settingsInstallMatrix.dataset.launchInstallVerificationNextStage === "verify_visibility", "settings install verification matrix dataset was incomplete");
    assert(settingsInstallMatrix.textContent.includes("Workflow install verification matrix") && settingsInstallMatrix.textContent.includes("2 paths -> verify_visibility") && settingsInstallMatrix.textContent.includes("remoteWorkflowFilesReady=true") && settingsInstallMatrix.textContent.includes("remoteWorkflowVisibilityReady=true") && settingsInstallMatrix.textContent.includes("dispatchReady=true") && settingsInstallMatrix.textContent.includes("driftDispatchReady=true") && settingsInstallMatrix.textContent.includes("allDispatchReady=true") && settingsInstallMatrix.textContent.includes("verify-launch-handoff reports safeToDispatch=true") && qs("[data-settings-install-verification-handoff-command]", settingsInstallMatrix).textContent.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown"), "settings install verification matrix did not render");
    assert(settingsInstallMatrixRows.length === 2 && settingsInstallMatrixRows.some((row) => row.dataset.settingsInstallVerificationRowKey === "cli_workflow_scope" && row.dataset.settingsInstallVerificationRowStatus === "blocked_by_workflow_scope") && settingsInstallMatrixRows.some((row) => row.dataset.settingsInstallVerificationRowKey === "github_ui" && row.dataset.settingsInstallVerificationRowStatus === "ready_to_install") && settingsInstallMatrixSignals.length === 5 && settingsInstallMatrixSignals.some((signal) => signal.dataset.settingsInstallVerificationSignalKey === "workflow_visibility" && signal.dataset.settingsInstallVerificationSignalStatus === "action_required"), "settings install verification matrix rows or signals were incomplete");
    ...
    assert(settingsRemoteFileLedger.dataset.remoteWorkflowFileLedgerSource === "generated_from_remote_workflow_file_check" && settingsRemoteFileLedger.dataset.remoteWorkflowFileLedgerStatus === "remote_file_install_required" && settingsRemoteFileLedger.dataset.remoteWorkflowFileLedgerFileCount === "2" && settingsRemoteFileLedger.dataset.remoteWorkflowFileLedgerReadyCount === "0" && settingsRemoteFileLedger.dataset.remoteWorkflowFileLedgerMissingCount === "2" && settingsRemoteFileLedger.dataset.remoteWorkflowFileLedgerMismatchCount === "0", "settings remote workflow file ledger dataset was incomplete");
    assert(settingsRemoteFileLedger.textContent.includes("Remote workflow file acceptance ledger") && settingsRemoteFileLedger.textContent.includes("0/2 files ready") && settingsRemoteFileLedger.textContent.includes("remoteExists") && settingsRemoteFileLedger.textContent.includes("remoteMatchesTemplate") && qs("[data-settings-remote-workflow-file-ledger-verify-command]", settingsRemoteFileLedger).textContent.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write"), "settings remote workflow file ledger did not render");
    assert(settingsRemoteFileLedgerItems.length === 2 && settingsRemoteFileLedgerItems.some((item) => item.dataset.remoteWorkflowFileKey === "pages" && item.dataset.remoteWorkflowFileStatus === "missing_on_default_branch" && item.dataset.remoteWorkflowFileRemoteExists === "false") && settingsRemoteFileLedgerItems.some((item) => item.dataset.remoteWorkflowFileKey === "drift-watch" && item.dataset.remoteWorkflowFileStatus === "missing_on_default_branch" && item.dataset.remoteWorkflowFileRemoteMatches === "false"), "settings remote workflow file ledger rows were incomplete");
    ...
    assert(settingsProofLedger.dataset.launchProofLedgerSource === "generated_from_launch_execution_packet" && settingsProofLedger.dataset.launchProofLedgerStatus === "proof_blocked_until_dispatch" && settingsProofLedger.dataset.launchProofLedgerRequiredCount === "6" && settingsProofLedger.dataset.launchProofLedgerReadyCount === "0" && settingsProofLedger.dataset.launchProofLedgerPendingCount === "6" && settingsProofLedger.dataset.launchProofLedgerCurrentGate === "capture_launch_proof" && settingsProofLedger.dataset.launchProofLedgerDeferredUntil === "safeToDispatch=true", "settings launch proof ledger dataset was incomplete");
    assert(settingsProofLedger.textContent.includes("Launch proof acceptance ledger") && settingsProofLedger.textContent.includes("0/6 proofs ready") && settingsProofLedger.textContent.includes("Pages URL/status") && settingsProofLedger.textContent.includes("status/conclusion/url/headSha") && qs("[data-settings-launch-proof-ledger-capture-command]", settingsProofLedger).textContent.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --write"), "settings launch proof ledger did not render");
    assert(settingsProofLedgerItems.length === 6 && settingsProofLedgerItems.some((item) => item.dataset.launchProofAcceptanceKey === "pages_site_url" && item.dataset.launchProofAcceptanceStatus === "blocked_until_dispatch") && settingsProofLedgerItems.some((item) => item.dataset.launchProofAcceptanceKey === "public_claim_guard" && item.dataset.launchProofAcceptanceStatus === "guarded"), "settings launch proof ledger proof rows were incomplete");
    ...
    assert(settingsLaunchRunbookSignals.length === 8 && settingsLaunchRunbookSignals.some((signal) => signal.textContent.includes("allDispatchReady=true")) && settingsLaunchRunbookSignals.some((signal) => signal.textContent.includes("safeToDispatch=true before gh workflow run")) && settingsLaunchRunbook.querySelectorAll("[data-settings-launch-runbook-link]").length >= 2, "settings launch runbook signals or links were incomplete");
	    ...
	    assert(settingsPostInstallQuickProofMap.dataset.postInstallQuickProofFieldMappingReady === "true" && settingsPostInstallQuickProofMap.dataset.postInstallQuickProofFieldMappingCoverage === "1" && settingsPostInstallQuickProofMap.dataset.postInstallQuickProofMappedFieldCount === "4" && settingsPostInstallQuickProofMap.dataset.postInstallQuickProofCompletedMappedFieldCount === "0" && settingsPostInstallQuickProofMapItems.length === 4 && settingsPostInstallQuickProofMapItems[0].dataset.postInstallQuickProofFieldMapStep === "remote_file_parity" && settingsPostInstallQuickProofMapItems[0].dataset.postInstallQuickProofFieldMapField === "remote_parity_proof" && settingsPostInstallQuickProofMapItems[3].dataset.postInstallQuickProofFieldMapStep === "handoff_verifier" && settingsPostInstallQuickProofMapItems[3].dataset.postInstallQuickProofFieldMapField === "handoff_verifier_proof", "settings post-install quick proof field map was incomplete");
  ...
  await runStep("system publish readiness alignment", async () => {
    ...
    assert(workflowInstallCards.length === 2 && workflowInstallCards.every((card) => card.dataset.workflowUiInstallReady === "true" && card.dataset.workflowUiInstallTargetMatchesTemplate === "true" && card.dataset.workflowUiInstallFileNameField === card.dataset.workflowUiInstallTarget && card.dataset.workflowUiInstallCommitMessage), "workflow UI install plan cards were incomplete");
    ...
    const dispatchPlanReady = (node) => !!node &&
      ...
      node.dataset.publishDispatchRemoteVisible === "false" &&
      ...
      (node.dataset.publishDispatchWorkflowScopeSource || "").length > 0 &&
      ...
      node.dataset.publishDispatchReady === "false" &&
      node.dataset.publishDispatchDriftReady === "false" &&
      node.dataset.publishDispatchAllReady === "false" &&
    ...
    assert(dispatchCards.length === 2 && dispatchCards.every((card) => card.dataset.publishDispatchWorkflowReady === "false" && card.dataset.publishDispatchWorkflowTargetExists === "true" && card.dataset.publishDispatchWorkflowTargetMatchesTemplate === "true"), "publish dispatch workflow cards were incomplete");
    ...
    assert(authPreflight.dataset.publishDispatchAuthPreflightAvailable === "false" && authPreflight.dataset.publishDispatchAuthPreflightInstallBlocked === "true" && Number(authPreflight.dataset.publishDispatchAuthPreflightScopeCount || "0") >= 3 && authPreflight.dataset.publishDispatchAuthPreflightSource.length > 0 && authPreflightText.includes("Auth preflight") && authPreflightText.includes("auth preflight only") && authPreflightText.includes("workflowScopeAvailable=false") && authPreflightText.includes("workflowScopeInstallBlocked=true") && authPreflightText.includes("workflowScope.scopes=gist, read:org, repo") && authPreflightText.includes("Missing scope=workflow") && authPreflightText.includes("workflowScopeRefreshCommand=gh auth refresh -h github.com -s workflow") && authPreflightText.includes("workflowScopeRefreshClipboardCommand=gh auth refresh -h github.com -s workflow --clipboard") && authPreflightText.includes("workflowScopeRecheckCommand=node scripts/plan-publish-dispatch.mjs --live --repo biojuho/BIOJUHO-Projects") && authPreflightText.includes("interactiveApprovalRequired=true") && authPreflightText.includes("terminalWaitRequired=true") && authPreflightText.includes("Token scopes still omit workflow after the refresh attempt") && authPreflightText.includes("does not install workflow files") && authPreflightText.includes("keep dispatch withheld until allDispatchReady: true"), "publish dispatch auth preflight card was not surfaced");
    ...
    assert(workflowScopePacket.dataset.publishDispatchWorkflowScopePacketReady === "true" && workflowScopePacketText.includes("JooPark Workflow Scope Refresh Packet") && workflowScopePacketText.includes("workflowScope.scopes: gist, read:org, repo") && workflowScopePacketText.includes("workflowScopeAvailable: false") && workflowScopePacketText.includes("workflowScopeInstallBlocked: true") && workflowScopePacketText.includes("Missing scope: workflow") && workflowScopePacketText.includes("gh auth refresh -h github.com -s workflow") && workflowScopePacketText.includes("gh auth refresh -h github.com -s workflow --clipboard") && workflowScopePacketText.includes("Interactive approval required: true") && workflowScopePacketText.includes("Terminal wait required: true") && workflowScopePacketText.includes("Incomplete approval signal: Token scopes still omit workflow after the refresh attempt") && workflowScopePacketText.includes("node scripts/plan-publish-dispatch.mjs --live --repo biojuho/BIOJUHO-Projects") && workflowScopePacketText.includes("Do not run gh workflow run until remoteWorkflowFilesReady: true"), "publish workflow scope refresh packet was not copy-ready");
    window.__smokeClipboardText = "";
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Workflow Scope Refresh Packet") && window.__smokeClipboardText.includes("workflowScope.scopes: gist, read:org, repo") && window.__smokeClipboardText.includes("Missing scope: workflow") && window.__smokeClipboardText.includes("gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("gh auth refresh -h github.com -s workflow --clipboard") && window.__smokeClipboardText.includes("Interactive approval required: true") && window.__smokeClipboardText.includes("Terminal wait required: true") && window.__smokeClipboardText.includes("Do not run gh workflow run until remoteWorkflowFilesReady: true"), "publish workflow scope packet copy text did not reach clipboard");
    ...
    assert(withheldDispatchCommands?.dataset.publishDispatchWithheldDispatchCount === "2" && withheldDispatchCommandsText.includes("withheldDispatchCommands") && withheldDispatchCommandsText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && withheldDispatchCommandsText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-drift-watch.yml -f mode=advisory"), "publish dispatch withheld command list was incomplete");
    assert(dispatchText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && dispatchText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-drift-watch.yml -f mode=advisory") && dispatchText.includes("nextVerificationCommand") && dispatchText.includes("suggestedCommands") && dispatchText.includes("suggestedDispatchCommands") && dispatchText.includes("withheldDispatchCommands"), "publish dispatch plan commands were incomplete");
    ...
    const remoteFileCards = Array.from(remoteFileCheck.querySelectorAll("[data-remote-workflow-file-card]"));
    const remoteFileText = remoteFileCheck.textContent || "";
    assert(remoteFileCheck.dataset.remoteWorkflowFileSource === "data/remote-workflow-file-check.json" && remoteFileCheck.dataset.remoteWorkflowFileLoaded === "true" && remoteFileCheck.dataset.remoteWorkflowFileChecked === "true" && remoteFileCheck.dataset.remoteWorkflowFileReady === "false" && remoteFileCheck.dataset.remoteWorkflowFileRepoReady === "true" && remoteFileCheck.dataset.remoteWorkflowFileCheckCount === "2" && Number(remoteFileCheck.dataset.remoteWorkflowFileBlockerCount || "0") >= 2 && remoteFileCheck.dataset.remoteWorkflowFileNextCommand.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write") && remoteFileCheck.dataset.remoteWorkflowFileWorkflowScopeAvailable === "false" && remoteFileCheck.dataset.remoteWorkflowFileWorkflowScopeInstallBlocked === "true" && remoteFileCheck.dataset.remoteWorkflowFileWorkflowScopeApprovalStatus === "approval_required" && remoteFileCheck.dataset.remoteWorkflowFileWorkflowScopeApprovalUrl === "https://github.com/login/device", "remote workflow file check state was not surfaced");
    assert(qs("[data-remote-workflow-file-state-label]", remoteFileCheck).textContent.includes("action required"), "remote workflow file check did not label blocked state");
    assert(remoteFileCards.length === 2 && remoteFileCards.every((card) => card.dataset.remoteWorkflowFileExists === "false" && card.dataset.remoteWorkflowFileMatchesTemplate === "false"), "remote workflow file cards were incomplete");
    ...
    assert(remoteFileText.includes("data/remote-workflow-file-check.json") && remoteFileText.includes("GitHub Contents API") && remoteFileText.includes("defaultBranch") && remoteFileText.includes("repoEvidenceReady") && remoteFileText.includes("remoteWorkflowFilesChecked") && remoteFileText.includes("remoteWorkflowFilesReady") && remoteFileText.includes("workflowScopeAvailable") && remoteFileText.includes("workflowScopeInstallBlocked") && remoteFileText.includes("workflow scope preflight") && remoteFileText.includes("approvalUrl=https://github.com/login/device") && remoteFileText.includes("Device-code approval handoff") && remoteFileText.includes("one-time device code") && remoteFileText.includes("gh auth refresh -h github.com -s workflow --clipboard") && remoteFileText.includes("interactiveApprovalRequired=true") && remoteFileText.includes("terminalWaitRequired=true") && remoteFileText.includes("Token scopes still omit workflow after the refresh attempt") && remoteFileText.includes("GitHub UI new-file links") && remoteFileText.includes("templateSha256") && remoteFileText.includes("remoteSha256") && remoteFileText.includes("remoteExists") && remoteFileText.includes("remoteMatchesTemplate") && remoteFileText.includes("not found on default branch") && remoteFileText.includes("remote workflow file is not installed on main") && remoteFileText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && remoteFileText.includes("githubNewFileUrl") && remoteFileText.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write"), "remote workflow file check blockers were not surfaced");
    ...
    assert(remoteInstallPacket.dataset.remoteWorkflowInstallPacketReady === "true" && remoteInstallPacketText.includes("JooPark Remote Workflow Install Packet") && remoteInstallPacketText.includes("Status: action required - install workflow files on the default branch") && remoteInstallPacketText.includes("Repo: biojuho/BIOJUHO-Projects") && remoteInstallPacketText.includes("Default branch: main") && remoteInstallPacketText.includes("workflowScopeAvailable: false") && remoteInstallPacketText.includes("workflowScopeInstallBlocked: true") && remoteInstallPacketText.includes("Workflow scope preflight:") && remoteInstallPacketText.includes("refresh with clipboard: gh auth refresh -h github.com -s workflow --clipboard") && remoteInstallPacketText.includes("approvalUrl: https://github.com/login/device") && remoteInstallPacketText.includes("interactive approval required: true") && remoteInstallPacketText.includes("terminal wait required: true") && remoteInstallPacketText.includes("incomplete approval signal: Token scopes still omit workflow after the refresh attempt") && remoteInstallPacketText.includes("one-time device code policy") && remoteInstallPacketText.includes("GitHub UI fallback") && remoteInstallPacketText.includes("Manual workflow dispatch requires workflow_dispatch") && remoteInstallPacketText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && remoteInstallPacketText.includes("open 'https://github.com/biojuho/BIOJUHO-Projects/new/main?filename=.github%2Fworkflows%2Fjoopark-pages.yml'") && remoteInstallPacketText.includes("pbcopy < 'docs/github-drift-watch-workflow.yml'") && remoteInstallPacketText.includes("Post-install verification checklist:") && remoteInstallPacketText.includes("remoteWorkflowFilesChecked: true") && remoteInstallPacketText.includes("pages remoteExists: true and remoteMatchesTemplate: true") && remoteInstallPacketText.includes("drift-watch remoteExists: true and remoteMatchesTemplate: true") && remoteInstallPacketText.includes("remoteWorkflowVisibilityReady: true") && remoteInstallPacketText.includes("allDispatchReady: true") && remoteInstallPacketText.includes("remote workflow file is not installed on main") && remoteInstallPacketText.includes("remoteWorkflowFilesReady: true and allDispatchReady: true"), "remote workflow install packet was not copy-ready");
    window.__smokeClipboardText = "";
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Remote Workflow Install Packet") && window.__smokeClipboardText.includes("Workflow scope preflight:") && window.__smokeClipboardText.includes("workflowScopeInstallBlocked: true") && window.__smokeClipboardText.includes("refresh with clipboard: gh auth refresh -h github.com -s workflow --clipboard") && window.__smokeClipboardText.includes("approvalUrl: https://github.com/login/device") && window.__smokeClipboardText.includes("interactive approval required: true") && window.__smokeClipboardText.includes("terminal wait required: true") && window.__smokeClipboardText.includes("incomplete approval signal: Token scopes still omit workflow after the refresh attempt") && window.__smokeClipboardText.includes("one-time device code policy") && window.__smokeClipboardText.includes("Install steps:") && window.__smokeClipboardText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && window.__smokeClipboardText.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write") && window.__smokeClipboardText.includes("Post-install verification checklist:") && window.__smokeClipboardText.includes("remoteWorkflowFilesReady: true") && window.__smokeClipboardText.includes("remoteWorkflowVisibilityReady: true") && window.__smokeClipboardText.includes("allDispatchReady: true") && window.__smokeClipboardText.includes("Do not run gh workflow run until remoteWorkflowFilesReady: true and allDispatchReady: true"), "remote workflow install packet copy text did not reach clipboard");
    remoteWorkflowFileCheckPanelOk = true;
    const launchPacketReady = (node) => !!node &&
      node.dataset.launchExecutionSource === "data/launch-execution-packet.json" &&
      node.dataset.launchExecutionLoaded === "true" &&
      node.dataset.launchExecutionReadyToDispatch === "false" &&
      node.dataset.launchExecutionProofReady === "false" &&
      node.dataset.launchExecutionExternalReady === "false" &&
      node.dataset.launchExecutionStageCount === "5" &&
      node.dataset.launchExecutionComparisonCount === "2" &&
      node.dataset.launchExecutionAuthPreflightStatus === "action_required" &&
      node.dataset.launchExecutionAuthPreflightChecked === "true" &&
      node.dataset.launchExecutionAuthWorkflowScopeAvailable === "false" &&
      node.dataset.launchExecutionAuthWorkflowScopeInstallBlocked === "true" &&
      node.dataset.launchExecutionAuthScopeCount === "3" &&
      node.dataset.launchExecutionAuthMissingScopes === "workflow" &&
      node.dataset.launchExecutionAuthApprovalStatus === "approval_required" &&
      node.dataset.launchExecutionAuthApprovalUrl === "https://github.com/login/device" &&
      node.dataset.launchExecutionPostAuthCheckpointStatus === "action_required" &&
      ...
      node.dataset.launchExecutionPostAuthCheckpointTriggerCommand === "gh auth refresh -h github.com -s workflow" &&
      node.dataset.launchExecutionPostAuthCheckpointVerifyCommand === "node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown" &&
      node.dataset.launchExecutionPostAuthCheckpointInstallCommand === "node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify" &&
      ...
      Number(node.dataset.launchExecutionCurrentActionCommandCount || "0") >= 5 &&
      ...
      node.dataset.launchExecutionCurrentActionAcceptancePassCount === "2" &&
      node.dataset.launchExecutionCurrentActionAcceptancePendingCount === "3" &&
      ...
      node.dataset.launchExecutionTransitionPendingCount === "3" &&
      node.dataset.launchExecutionTransitionPassCount === "2" &&
      ...
      node.dataset.launchExecutionBlockerResolutionActive === "operator_auth_path" &&
	      node.dataset.launchExecutionBlockerResolutionItemCount === "6" &&
	      node.dataset.launchExecutionBlockerResolutionActionRequiredCount === "3" &&
	      ...
	      node.dataset.launchPostInstallEvidenceIntakeCompletedCount === "0" &&
	      ...
	      node.dataset.launchPostInstallQuickProofCompletedMappedFieldCount === "0" &&
	      node.dataset.launchPostInstallQuickProofFinalCommand.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown") &&
      ...
      node.dataset.remoteWorkflowFileLedgerReadyCount === "0" &&
      node.dataset.remoteWorkflowFileLedgerMissingCount === "2" &&
      ...
      node.dataset.launchProofLedgerReadyCount === "0" &&
      node.dataset.launchProofLedgerPendingCount === "6" &&
    ...
    assert(launchOperatorOnePage.dataset.launchOperatorOnePageSource === "generated_from_launch_execution_packet" && launchOperatorOnePage.dataset.launchOperatorOnePageReady === "true" && launchOperatorOnePage.dataset.launchOperatorOnePageStatus === "action_required" && launchOperatorOnePage.dataset.launchOperatorOnePageStage === "install_workflows" && launchOperatorOnePage.dataset.launchOperatorOnePageActive === "operator_auth_path" && launchOperatorOnePage.dataset.launchOperatorOnePageSectionCount === "8" && Number(launchOperatorOnePage.dataset.launchOperatorOnePageCommandCount || "0") >= 12 && launchOperatorOnePage.dataset.launchOperatorOnePageProofCommandCount === "4" && Number(launchOperatorOnePage.dataset.launchOperatorOnePageSuccessSignalCount || "0") >= 8 && Number(launchOperatorOnePage.dataset.launchOperatorOnePageForbiddenCommandCount || "0") >= 3, "launch operator one-page handoff dataset was incomplete");
    assert(launchOperatorOnePage.textContent.includes("Operator one-page handoff") && launchOperatorOnePage.textContent.includes("operator_auth_path") && launchOperatorOnePage.textContent.includes("8") && launchOperatorOnePageText.includes("JooPark Launch Operator One-Page Handoff") && launchOperatorOnePageText.includes("Goal for this pass:") && launchOperatorOnePageText.includes("Install workflows on the default branch") && launchOperatorOnePageText.includes("Active blocker: operator_auth_path") && launchOperatorOnePageText.includes("Do first:") && launchOperatorOnePageText.includes("gh auth refresh -h github.com -s workflow") && launchOperatorOnePageText.includes("If CLI workflow scope is still blocked, use GitHub UI fallback:") && launchOperatorOnePageText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && launchOperatorOnePageText.includes("Prove after install:") && launchOperatorOnePageText.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write") && launchOperatorOnePageText.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown") && launchOperatorOnePageText.includes("Success signals:") && launchOperatorSuccessSignals.every((signal) => launchOperatorOnePageText.includes(signal)) && launchOperatorOnePageText.includes("Evidence fields to fill:") && launchOperatorOnePageText.includes("Handoff verifier proof") && launchOperatorOnePageText.includes("Do not run or claim yet:") && launchOperatorOnePageText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && launchOperatorOnePageText.includes("Do not claim readyForExternalClaim=true") && launchOperatorOnePageText.includes("External baseline:") && launchOperatorOnePageText.includes("workflow_dispatch must exist on the default branch"), "launch operator one-page handoff was not copy-ready");
    ...
    assert(launchAuthPreflight.textContent.includes("Auth preflight") && launchAuthPreflight.textContent.includes("workflowScopeAvailable") && launchAuthPreflight.textContent.includes("false") && launchAuthPreflight.textContent.includes("workflowScopeInstallBlocked") && launchAuthPreflight.textContent.includes("gist, read:org, repo") && launchAuthPreflight.textContent.includes("missingScopes") && launchAuthPreflight.textContent.includes("workflow") && launchAuthPreflight.textContent.includes("approval_required") && launchAuthPreflight.textContent.includes("https://github.com/login/device") && launchAuthPreflight.textContent.includes("Do not store, log, or paste the one-time device code") && launchAuthPreflight.textContent.includes("gh auth refresh -h github.com -s workflow"), "launch execution auth preflight was not surfaced");
    ...
    assert(blockerResolution.dataset.launchBlockerResolutionSource === "generated_from_launch_execution_packet" && blockerResolution.dataset.launchBlockerResolutionStatus === "action_required" && blockerResolution.dataset.launchBlockerResolutionActive === "operator_auth_path" && blockerResolution.dataset.launchBlockerResolutionItemCount === "6" && blockerResolution.dataset.launchBlockerResolutionPassCount === "2" && blockerResolution.dataset.launchBlockerResolutionActionRequiredCount === "3" && blockerResolution.dataset.launchBlockerResolutionDeferredCount === "1" && blockerResolution.dataset.launchBlockerResolutionProofCommandCount === "6" && blockerResolution.dataset.launchBlockerResolutionGuard.includes("every action_required item") && blockerResolution.dataset.launchBlockerResolutionGuard.includes("verify-launch-handoff reports safeToDispatch=true"), "launch blocker resolution checklist dataset was incomplete");
    assert(blockerResolution.textContent.includes("Blocker resolution checklist") && blockerResolution.textContent.includes("proof commands") && blockerResolution.textContent.includes("Expected: workflowScopeAvailable=true") && blockerResolution.textContent.includes("Stop: If workflowScopeInstallBlocked=true remains after recheck") && blockerResolution.textContent.includes("Do not run gh workflow run until every action_required item") && blockerResolution.textContent.includes("verify-launch-handoff reports safeToDispatch=true") && blockerResolution.textContent.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --write"), "launch blocker resolution checklist did not render");
    assert(blockerResolutionItems.length === 6 && blockerResolutionItems.some((item) => item.dataset.launchBlockerResolutionKey === "operator_auth_path" && item.dataset.launchBlockerResolutionStatus === "action_required" && item.textContent.includes("gh auth refresh -h github.com -s workflow")) && blockerResolutionItems.some((item) => item.dataset.launchBlockerResolutionKey === "remote_workflow_file_parity" && item.dataset.launchBlockerResolutionStatus === "action_required" && item.textContent.includes("remoteWorkflowFilesReady=true")) && blockerResolutionItems.some((item) => item.dataset.launchBlockerResolutionKey === "workflow_visibility" && item.textContent.includes("gh workflow list --repo biojuho/BIOJUHO-Projects")) && blockerResolutionItems.some((item) => item.dataset.launchBlockerResolutionKey === "dispatch_guard" && item.dataset.launchBlockerResolutionStatus === "pass" && item.textContent.includes("safeToDispatch=true before gh workflow run")) && blockerResolutionItems.some((item) => item.dataset.launchBlockerResolutionKey === "launch_proof_capture" && item.dataset.launchBlockerResolutionStatus === "deferred_until_dispatch"), "launch blocker resolution checklist rows were incomplete");
    ...
    const launchInstallMatrixSignals = Array.from(launchInstallMatrix.querySelectorAll("[data-launch-install-verification-signal]"));
	    ...
	    assert(launchInstallMatrixRows.length === 2 && launchInstallMatrixRows.some((row) => row.dataset.launchInstallVerificationRowKey === "cli_workflow_scope" && row.dataset.launchInstallVerificationRowStatus === "blocked_by_workflow_scope") && launchInstallMatrixRows.some((row) => row.dataset.launchInstallVerificationRowKey === "github_ui" && row.dataset.launchInstallVerificationRowStatus === "ready_to_install") && launchInstallMatrixSignals.length === 5 && launchInstallMatrixSignals.some((signal) => signal.dataset.launchInstallVerificationSignalKey === "workflow_visibility" && signal.dataset.launchInstallVerificationSignalStatus === "action_required") && launchInstallMatrixSignals.some((signal) => signal.dataset.launchInstallVerificationSignalKey === "dispatch_guard" && signal.dataset.launchInstallVerificationSignalStatus === "pass"), "launch install verification matrix rows or signals were incomplete");
    ...
    assert(remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerSource === "generated_from_remote_workflow_file_check" && remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerStatus === "remote_file_install_required" && remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerFileCount === "2" && remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerReadyCount === "0" && remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerMissingCount === "2" && remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerMismatchCount === "0" && remoteWorkflowFileLedger.dataset.remoteWorkflowFileLedgerVerifyCommand.includes("check-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write"), "remote workflow file acceptance ledger dataset was incomplete");
    assert(remoteWorkflowFileLedger.textContent.includes("Remote workflow file acceptance ledger") && remoteWorkflowFileLedger.textContent.includes("0/2 files ready") && remoteWorkflowFileLedger.textContent.includes("remote_file_install_required") && remoteWorkflowFileLedger.textContent.includes("templateSha256") && remoteWorkflowFileLedger.textContent.includes("remoteSha256") && remoteWorkflowFileLedger.textContent.includes("remoteExists=false") && remoteWorkflowFileLedger.textContent.includes("remoteMatchesTemplate=false"), "remote workflow file acceptance ledger did not render");
    assert(remoteWorkflowFileLedgerItems.length === 2 && remoteWorkflowFileLedgerItems.some((item) => item.dataset.remoteWorkflowFileKey === "pages" && item.dataset.remoteWorkflowFileStatus === "missing_on_default_branch" && item.textContent.includes("joopark-pages.yml")) && remoteWorkflowFileLedgerItems.some((item) => item.dataset.remoteWorkflowFileKey === "drift-watch" && item.dataset.remoteWorkflowFileStatus === "missing_on_default_branch" && item.textContent.includes("joopark-drift-watch.yml")), "remote workflow file acceptance ledger rows were incomplete");
    ...
    assert(launchProofLedger.dataset.launchProofLedgerSource === "generated_from_launch_execution_packet" && launchProofLedger.dataset.launchProofLedgerStatus === "proof_blocked_until_dispatch" && launchProofLedger.dataset.launchProofLedgerRequiredCount === "6" && launchProofLedger.dataset.launchProofLedgerReadyCount === "0" && launchProofLedger.dataset.launchProofLedgerPendingCount === "6" && launchProofLedger.dataset.launchProofLedgerCurrentGate === "capture_launch_proof" && launchProofLedger.dataset.launchProofLedgerDeferredUntil === "safeToDispatch=true" && launchProofLedger.dataset.launchProofLedgerCaptureCommand.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --write"), "launch proof acceptance ledger dataset was incomplete");
    assert(launchProofLedger.textContent.includes("Launch proof acceptance ledger") && launchProofLedger.textContent.includes("0/6 proofs ready") && launchProofLedger.textContent.includes("Pages html_url/status") && launchProofLedger.textContent.includes("status/conclusion/url/headSha") && launchProofLedger.textContent.includes("readyForExternalClaim") && launchProofLedger.textContent.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --write"), "launch proof acceptance ledger did not render");
    assert(launchProofLedgerItems.length === 6 && launchProofLedgerItems.some((item) => item.dataset.launchProofAcceptanceKey === "pages_site_url" && item.dataset.launchProofAcceptanceStatus === "blocked_until_dispatch") && launchProofLedgerItems.some((item) => item.dataset.launchProofAcceptanceKey === "pages_workflow_run" && item.textContent.includes("joopark-pages.yml")) && launchProofLedgerItems.some((item) => item.dataset.launchProofAcceptanceKey === "drift_workflow_run" && item.textContent.includes("joopark-drift-watch.yml")) && launchProofLedgerItems.some((item) => item.dataset.launchProofAcceptanceKey === "public_claim_guard" && item.dataset.launchProofAcceptanceStatus === "guarded"), "launch proof acceptance ledger rows were incomplete");
    ...
    assert(launchAcceptanceItems.length === 5 && launchAcceptanceItems.some((item) => item.dataset.launchExecutionAcceptanceKey === "operator_auth_path" && item.dataset.launchExecutionAcceptanceStatus === "action_required") && launchAcceptanceItems.some((item) => item.dataset.launchExecutionAcceptanceKey === "local_template_parity" && item.dataset.launchExecutionAcceptanceStatus === "pass") && launchAcceptanceItems.some((item) => item.dataset.launchExecutionAcceptanceKey === "remote_workflow_file_parity" && item.textContent.includes("remoteWorkflowFilesReady=false")) && launchAcceptanceItems.some((item) => item.dataset.launchExecutionAcceptanceKey === "workflow_visibility" && item.textContent.includes("remoteWorkflowVisibilityReady=false")) && launchAcceptanceItems.some((item) => item.dataset.launchExecutionAcceptanceKey === "dispatch_guard" && item.dataset.launchExecutionAcceptanceStatus === "pass" && item.textContent.includes("withheldCommands=2")), "launch execution acceptance checklist was not surfaced");
    const defaultBranchProof = qs("[data-launch-current-default-branch-proof]", launchCurrentAction);
    assert(defaultBranchProof.dataset.launchCurrentDefaultBranchProofReady === "true" && defaultBranchProof.dataset.launchCurrentDefaultBranchProofFileCount === "2" && defaultBranchProof.dataset.launchCurrentDefaultBranchProofRequirementCount === "4" && defaultBranchProof.textContent.includes("Default-branch requirement proof") && defaultBranchProof.textContent.includes("GitHub manual workflow dispatch docs") && defaultBranchProof.textContent.includes("GitHub REST repository contents API") && defaultBranchProof.textContent.includes("manually-running-a-workflow") && defaultBranchProof.textContent.includes("repos/contents#get-repository-content") && defaultBranchProof.textContent.includes("workflow_dispatch exists on the default branch") && defaultBranchProof.textContent.includes("match each local template SHA-256") && defaultBranchProof.textContent.includes("gh workflow list --repo biojuho/BIOJUHO-Projects"), "launch current action default-branch proof was not surfaced");
    const currentActionSurfaceOk = launchCurrentAction.textContent.includes("Install workflows on the default branch") &&
      ...
      launchCurrentActionText.includes("workflowScopeAvailable: false") &&
      launchCurrentActionText.includes("workflowScopeInstallBlocked: true") &&
      launchCurrentActionText.includes("scopes: gist, read:org, repo") &&
      launchCurrentActionText.includes("missingScopes: workflow") &&
      launchCurrentActionText.includes("approval: approval_required") &&
      ...
      launchCurrentActionText.includes("interactiveApprovalRequired: true") &&
      launchCurrentActionText.includes("terminalWaitRequired: true") &&
      ...
      launchCurrentActionText.includes("Acceptance checklist: 2/5 pass; pending=3") &&
      launchCurrentActionText.includes("Operator auth path: action_required") &&
      launchCurrentActionText.includes("Remote workflow file parity: action_required") &&
      launchCurrentActionText.includes("Workflow visibility: action_required") &&
      ...
      launchCurrentActionText.includes("Create the files on the default branch with the exact copied template contents") &&
      launchCurrentActionText.includes("gh auth refresh -h github.com -s workflow") &&
      launchCurrentActionText.includes("Do not run yet:") &&
      launchCurrentActionText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml");
    ...
    const launchPacketCopyReady = launchPacketText.includes("JooPark Launch Execution Packet") &&
      ...
      launchPacketText.includes("open 'https://github.com/biojuho/BIOJUHO-Projects/new/main?filename=.github%2Fworkflows%2Fjoopark-pages.yml'") &&
      ...
      launchPacketText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") &&
    ...
    assert(launchPacketCopyReady, "launch execution packet was not copy-ready");
    window.__smokeClipboardText = "";
    ...
    assert(launchReadinessRefresh.dataset.launchReadinessRefreshLoaded === "true" &&
      ...
      launchReadinessRefresh.dataset.launchReadinessRefreshWorkflowScopeAvailable === "false" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshWorkflowScopeInstallBlocked === "true" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshRemoteFilesReady === "false" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshRemoteVisible === "false" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshAllDispatchReady === "false" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshSafeToDispatch === "false" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshReadyForExternalClaim === "false" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshWithheldCount === "2" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshSuggestedDispatchCount === "0" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshNextAction === "refresh_workflow_scope_or_use_github_ui" &&
      launchReadinessRefresh.dataset.launchReadinessRefreshNextCommand === "gh auth refresh -h github.com -s workflow" &&
      ...
      launchReadinessRefresh.dataset.launchReadinessRefreshSourceArtifactCount === "6", "launch readiness refresh dataset was incomplete");
    assert(launchReadinessRefresh.textContent.includes("Launch readiness refresh") &&
      launchReadinessRefresh.textContent.includes("6 commands") &&
      launchReadinessRefresh.textContent.includes("freshness") &&
      launchReadinessRefresh.textContent.includes("workflow_ui_install_plan") &&
      launchReadinessRefresh.textContent.includes("keep_b") &&
      launchReadinessRefresh.textContent.includes("npm run verify ->") &&
      launchReadinessRefresh.textContent.includes("11 sources") &&
      launchReadinessRefresh.textContent.includes("workflowScopeInstallBlocked=true") &&
      launchReadinessRefresh.textContent.includes("remoteWorkflowFilesReady=false") &&
      launchReadinessRefresh.textContent.includes("safeToDispatch=false") &&
      launchReadinessRefresh.textContent.includes("readyForExternalClaim=false") &&
      launchReadinessRefresh.textContent.includes("every action_required refresh checklist item has passed") &&
      launchReadinessRefresh.textContent.includes("verify-launch-handoff reports safeToDispatch=true") &&
      launchReadinessRefresh.textContent.includes("gh auth refresh -h github.com -s workflow") &&
      launchReadinessRefresh.textContent.includes("npm run refresh:launch-readiness"), "launch readiness refresh panel did not render");
    ...
    assert(qs("[data-launch-readiness-refresh-receipt-text]", launchReadinessReceipt).textContent.includes("JooPark Launch Readiness Refresh Receipt") &&
      ...
      qs("[data-launch-readiness-refresh-receipt-text]", launchReadinessReceipt).textContent.includes("safeToDispatch: false") &&
      qs("[data-launch-readiness-refresh-receipt-text]", launchReadinessReceipt).textContent.includes("readyForExternalClaim: false") &&
      qs("[data-launch-readiness-refresh-receipt-text]", launchReadinessReceipt).textContent.includes("suggestedDispatchCommandCount: 0") &&
      qs("[data-launch-readiness-refresh-receipt-text]", launchReadinessReceipt).textContent.includes("withheldDispatchCommandCount: 2") &&
      qs("[data-launch-readiness-refresh-receipt-text]", launchReadinessReceipt).textContent.includes("gh auth refresh -h github.com -s workflow"), "launch readiness refresh receipt text was incomplete");
    window.__smokeClipboardText = "";
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Launch Readiness Refresh Receipt") &&
      ...
      window.__smokeClipboardText.includes("safeToDispatch: false") &&
      window.__smokeClipboardText.includes("readyForExternalClaim: false") &&
      window.__smokeClipboardText.includes("suggestedDispatchCommandCount: 0") &&
      window.__smokeClipboardText.includes("withheldDispatchCommandCount: 2") &&
      window.__smokeClipboardText.includes("Do not run gh workflow run") &&
      window.__smokeClipboardText.includes("workflowScopeInstallBlocked: true") &&
      window.__smokeClipboardText.includes("remoteWorkflowFilesReady: false") &&
      window.__smokeClipboardText.includes("gh auth refresh -h github.com -s workflow"), "launch readiness refresh receipt copy text did not reach clipboard");
    ...
    if (verifyWorkspaceSummaryLoaded) {
      assert(verifyWorkspaceSummary.dataset.verifyWorkspaceSummarySource === "autoresearch-results/verify-workspace-summary.json" &&
        ...
        verifyWorkspaceSummary.dataset.verifyWorkspaceSummarySafeToDispatch === "false" &&
        verifyWorkspaceSummary.dataset.verifyWorkspaceSummaryReadyForExternalClaim === "false", "verify workspace summary dataset was incomplete");
    } else {
    ...
    if (verifyWorkspaceSummaryLoaded) {
      assert(verifyWorkspaceReceipt.dataset.verifyWorkspaceSummaryReceiptCopyReady === "true" &&
        ...
        verifyWorkspaceReceiptText.includes("readyForExternalClaim: false"), "verify workspace summary receipt text was incomplete");
      window.__smokeClipboardText = "";
      ...
      await waitFor(() => window.__smokeClipboardText.includes("JooPark Verify Workspace Summary Receipt") &&
        ...
        window.__smokeClipboardText.includes("readyForExternalClaim: false"), "verify workspace summary receipt copy text did not reach clipboard");
    } else {
    ...
    assert(releaseGateCache.textContent.includes("Release gate cache") &&
      (releaseGateCache.textContent.includes("cached pass") || releaseGateCache.textContent.includes("repair required")) &&
      releaseGateCache.textContent.includes("packaged browser gate cache") &&
      releaseGateCache.textContent.includes("cachedEvidenceStatus") &&
      releaseGateCache.textContent.includes("cachedResultStatus") &&
      releaseGateCache.textContent.includes("contextMatched=false") &&
      releaseGateCache.textContent.includes("context_mismatch") &&
      releaseGateCache.textContent.includes("not_run") &&
      releaseGateCache.textContent.includes("npm test") &&
      releaseGateCache.textContent.includes("node scripts/audit-release-readiness.mjs --format=summary"), "release gate cache panel did not render");
    ...
    assert(attestationIntake.dataset.pagesAttestationProofIntakeReady === "false" &&
      ...
      attestationIntake.dataset.pagesAttestationProofIntakeRemoteWorkflowVisible === "false" &&
      attestationIntake.dataset.pagesAttestationProofIntakeAllDispatchReady === "false" &&
    ...
    assert(evidence.dataset.publishEvidenceMode === "live" && evidence.dataset.publishEvidenceRepoReady === "true" && evidence.dataset.publishEvidenceEvidenceRepo === "biojuho/BIOJUHO-Projects" && evidence.dataset.publishEvidenceRepoResolution === "source_repo" && evidence.dataset.publishEvidenceRepoPlaceholderResolved === "false" && evidence.dataset.publishEvidencePagesReady === "false" && evidence.dataset.publishEvidenceWorkflowsReady === "false" && evidence.dataset.publishEvidenceLaunchProofReady === "false", "publish evidence live action-required readiness state was not surfaced");
    assert(evidence.dataset.publishEvidenceNextAction === "install_workflows" && evidence.dataset.publishEvidenceNextCommand.includes("gh auth refresh -h github.com -s workflow"), "publish evidence next action was not surfaced");
    assert(evidence.dataset.publishEvidenceImmediateAction === "install_workflows" && evidence.dataset.publishEvidenceImmediateActionStatus === "action_required" && evidence.dataset.publishEvidenceImmediateActionSource === "data/launch-execution-packet.json" && evidence.dataset.publishEvidenceImmediateCommand.includes("gh auth refresh -h github.com -s workflow") && evidence.dataset.publishEvidenceImmediateCommandCount === "7" && evidence.dataset.publishEvidenceImmediateWithheldCommandCount === "2" && evidence.dataset.publishEvidenceInstallPathsReady === "true" && evidence.dataset.publishEvidenceInstallPathCount === "2" && evidence.dataset.publishEvidenceInstallPathCommandCount === "14" && evidence.dataset.publishEvidenceDeferredAction === "capture-live-evidence" && evidence.dataset.publishEvidenceDeferredCommand.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown"), "publish evidence immediate next action was not surfaced");
    ...
    assert(publishEvidenceStateLabel.includes("action required") || publishEvidenceStateLabel.includes("dry-run evidence"), "publish evidence state label did not distinguish blocked live evidence");
    const nextActionCardText = qs("[data-publish-evidence-next-action-card]", evidence).textContent;
    assert(nextActionCardText.includes("Immediate action") && nextActionCardText.includes("Install workflows on the default branch") && nextActionCardText.includes("Success condition: remoteWorkflowFilesReady=true") && nextActionCardText.includes("gh auth refresh -h github.com -s workflow") && nextActionCardText.includes("Choose one install path") && nextActionCardText.includes("CLI path after workflow scope") && nextActionCardText.includes("GitHub UI path") && nextActionCardText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && nextActionCardText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && nextActionCardText.includes("Deferred evidence capture") && nextActionCardText.includes("Capture live publish evidence") && nextActionCardText.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown"), "publish evidence next action card did not render immediate and deferred actions");
    ...
    assert(commandGuardText.includes("Dispatch command guard") && commandGuardText.includes("Do not run until allDispatchReady: true") && commandGuardText.includes("withheld-until-all-dispatch-ready"), "publish evidence dispatch command guard did not render");
    ...
    await waitFor(() => {
      ...
      return evidence.dataset.publishEvidenceShareUpdateReady === "true" && shareUpdateText.includes("JooPark Publish Evidence Update") && shareUpdateText.includes("Status: action required") && shareUpdateText.includes("Repo: biojuho/BIOJUHO-Projects") && shareUpdateText.includes(expectedEvidenceRepoLine) && shareUpdateText.includes(expectedRepoResolutionLine) && !shareUpdateText.includes("\\nRepo: OWNER/REPO") && shareUpdateText.includes("Suggested repo: biojuho/BIOJUHO-Projects") && shareUpdateText.includes("postPublishEvidenceReady: false") && shareUpdateText.includes("Dispatch guard: withheld (withheld-until-all-dispatch-ready)") && shareUpdateText.includes("Suggested commands safe: true; suggested dispatch: 0; withheld dispatch: 2") && shareUpdateText.includes("Do not run dispatch until allDispatchReady: true.") && shareUpdateText.includes("Withheld dispatch commands:") && shareUpdateText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && shareUpdateText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-drift-watch.yml -f mode=advisory") && shareUpdateText.includes("Immediate action: Install workflows on the default branch [action_required]") && shareUpdateText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && shareUpdateText.includes("Immediate command count: 7") && shareUpdateText.includes("Immediate success condition: remoteWorkflowFilesReady=true") && shareUpdateText.includes("Choose one install path:") && shareUpdateText.includes("Launch install path options: pass (2 paths, 14 commands; CLI path after workflow scope | GitHub UI path)") && shareUpdateText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && shareUpdateText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && shareUpdateText.includes("Deferred evidence capture: Capture live publish evidence") && shareUpdateText.includes("Deferred command: node scripts/capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown");
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Publish Evidence Update") && window.__smokeClipboardText.includes("Status: action required") && window.__smokeClipboardText.includes("Repo: biojuho/BIOJUHO-Projects") && window.__smokeClipboardText.includes(expectedEvidenceRepoLine) && !window.__smokeClipboardText.includes("\\nRepo: OWNER/REPO") && window.__smokeClipboardText.includes("Deferred evidence capture: Capture live publish evidence") && window.__smokeClipboardText.includes("Dispatch guard: withheld (withheld-until-all-dispatch-ready)") && window.__smokeClipboardText.includes("Do not run dispatch until allDispatchReady: true.") && window.__smokeClipboardText.includes("Withheld dispatch commands:") && window.__smokeClipboardText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && window.__smokeClipboardText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-drift-watch.yml -f mode=advisory") && window.__smokeClipboardText.includes("Immediate action: Install workflows on the default branch [action_required]") && window.__smokeClipboardText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("Choose one install path:") && window.__smokeClipboardText.includes("CLI path after workflow scope") && window.__smokeClipboardText.includes("GitHub UI path") && window.__smokeClipboardText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && window.__smokeClipboardText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && window.__smokeClipboardText.includes("Deferred command: node scripts/capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown"), "publish evidence share update copy text did not reach clipboard");
    ...
    await waitFor(() => {
      ...
      return evidence.dataset.publishEvidenceLaunchAnnouncementReady === "true" && launchAnnouncementText.includes("JooPark Public Launch Announcement") && launchAnnouncementText.includes("Status: not ready for public posting") && launchAnnouncementText.includes("Repo: biojuho/BIOJUHO-Projects") && launchAnnouncementText.includes(expectedEvidenceRepoLine) && !launchAnnouncementText.includes("\\nRepo: OWNER/REPO") && launchAnnouncementText.includes("Dispatch gate:") && launchAnnouncementText.includes("Dispatch guard: withheld (withheld-until-all-dispatch-ready)") && launchAnnouncementText.includes("Suggested commands safe: true; suggested dispatch: 0; withheld dispatch: 2") && launchAnnouncementText.includes("Do not post or dispatch until allDispatchReady: true and postPublishEvidenceReady: true.") && launchAnnouncementText.includes("Immediate action: Install workflows on the default branch [action_required]") && launchAnnouncementText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && launchAnnouncementText.includes("Choose one install path:") && launchAnnouncementText.includes("CLI path after workflow scope") && launchAnnouncementText.includes("GitHub UI path") && launchAnnouncementText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && launchAnnouncementText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && launchAnnouncementText.includes("Deferred evidence capture: Capture live publish evidence") && !launchAnnouncementText.includes("gh workflow run --repo") && launchAnnouncementText.includes("Do not post a public launch announcement") && launchAnnouncementText.includes("repoEvidenceReady, evidenceFresh, and postPublishEvidenceReady are all true");
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Public Launch Announcement") && window.__smokeClipboardText.includes("Status: not ready for public posting") && window.__smokeClipboardText.includes("Repo: biojuho/BIOJUHO-Projects") && window.__smokeClipboardText.includes(expectedEvidenceRepoLine) && !window.__smokeClipboardText.includes("\\nRepo: OWNER/REPO") && window.__smokeClipboardText.includes("Deferred evidence capture: Capture live publish evidence") && window.__smokeClipboardText.includes("Dispatch gate:") && window.__smokeClipboardText.includes("Dispatch guard: withheld (withheld-until-all-dispatch-ready)") && window.__smokeClipboardText.includes("Do not post or dispatch until allDispatchReady: true and postPublishEvidenceReady: true.") && window.__smokeClipboardText.includes("Immediate action: Install workflows on the default branch [action_required]") && window.__smokeClipboardText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("Choose one install path:") && window.__smokeClipboardText.includes("CLI path after workflow scope") && window.__smokeClipboardText.includes("GitHub UI path") && window.__smokeClipboardText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && window.__smokeClipboardText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && window.__smokeClipboardText.includes("Deferred evidence capture: Capture live publish evidence") && !window.__smokeClipboardText.includes("gh workflow run --repo") && window.__smokeClipboardText.includes("Do not post a public launch announcement until repoEvidenceReady, evidenceFresh, and postPublishEvidenceReady are all true."), "publish launch announcement copy text did not reach clipboard");
    ...
    await waitFor(() => {
      ...
      return evidence.dataset.publishEvidencePostLaunchReceiptReady === "true" && postLaunchReceiptText.includes("JooPark Post-Launch Verification Receipt") && postLaunchReceiptText.includes("Status: not ready to archive") && postLaunchReceiptText.includes("Repo: biojuho/BIOJUHO-Projects") && postLaunchReceiptText.includes(expectedEvidenceRepoLine) && !postLaunchReceiptText.includes("\\nRepo: OWNER/REPO") && postLaunchReceiptText.includes("Verification checklist:") && postLaunchReceiptText.includes(expectedRepoEvidenceReadyLine) && postLaunchReceiptText.includes("pagesEvidenceReady: false") && postLaunchReceiptText.includes("workflowEvidenceReady: false") && postLaunchReceiptText.includes("postPublishEvidenceReady: false") && postLaunchReceiptText.includes("publishDispatchReady: false") && postLaunchReceiptText.includes("dispatchSuggestionStatus: withheld-until-all-dispatch-ready") && postLaunchReceiptText.includes("Dispatch guard: withheld (withheld-until-all-dispatch-ready)") && postLaunchReceiptText.includes("Suggested commands safe: true; suggested dispatch: 0; withheld dispatch: 2") && postLaunchReceiptText.includes("Do not run dispatch until allDispatchReady: true.") && postLaunchReceiptText.includes("Withheld dispatch commands:") && postLaunchReceiptText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && postLaunchReceiptText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-drift-watch.yml -f mode=advisory") && postLaunchReceiptText.includes("Immediate action: Install workflows on the default branch [action_required]") && postLaunchReceiptText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && postLaunchReceiptText.includes("Choose one install path:") && postLaunchReceiptText.includes("CLI path after workflow scope") && postLaunchReceiptText.includes("GitHub UI path") && postLaunchReceiptText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && postLaunchReceiptText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && postLaunchReceiptText.includes("Deferred evidence capture: Capture live publish evidence") && postLaunchReceiptText.includes("Do not archive this as post-launch verification");
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Post-Launch Verification Receipt") && window.__smokeClipboardText.includes("Status: not ready to archive") && window.__smokeClipboardText.includes("Repo: biojuho/BIOJUHO-Projects") && window.__smokeClipboardText.includes(expectedEvidenceRepoLine) && !window.__smokeClipboardText.includes("\\nRepo: OWNER/REPO") && window.__smokeClipboardText.includes("Deferred evidence capture: Capture live publish evidence") && window.__smokeClipboardText.includes("publishDispatchReady: false") && window.__smokeClipboardText.includes("dispatchSuggestionStatus: withheld-until-all-dispatch-ready") && window.__smokeClipboardText.includes("Dispatch guard: withheld (withheld-until-all-dispatch-ready)") && window.__smokeClipboardText.includes("Do not run dispatch until allDispatchReady: true.") && window.__smokeClipboardText.includes("Withheld dispatch commands:") && window.__smokeClipboardText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-pages.yml") && window.__smokeClipboardText.includes("gh workflow run --repo biojuho/BIOJUHO-Projects joopark-drift-watch.yml -f mode=advisory") && window.__smokeClipboardText.includes("Immediate action: Install workflows on the default branch [action_required]") && window.__smokeClipboardText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("Choose one install path:") && window.__smokeClipboardText.includes("CLI path after workflow scope") && window.__smokeClipboardText.includes("GitHub UI path") && window.__smokeClipboardText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && window.__smokeClipboardText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && window.__smokeClipboardText.includes("Deferred evidence capture: Capture live publish evidence") && window.__smokeClipboardText.includes("Do not archive this as post-launch verification until repoEvidenceReady, pagesEvidenceReady, workflowEvidenceReady, evidenceFresh, and postPublishEvidenceReady are all true."), "publish post-launch receipt copy text did not reach clipboard");
    ...
    await waitFor(() => {
        ...
        evidence.dataset.publishEvidenceLaunchProofFieldCoverage === "1" &&
	        ...
	        launchProofReceiptText.includes("Status: blocked until live proof") &&
	        ...
	        launchProofReceiptText.includes("Stop condition: do not post public launch copy, archive proof, or claim readyForExternalClaim until all six evidence fields are live, fresh, linked, and successful.");
    ...
    await waitFor(() => launchProofReceipt.dataset.publishLaunchProofReceiptCopied === "true" && qs("[data-publish-evidence-launch-proof-receipt-copy-status]", launchProofReceipt).textContent.includes("복사"), "publish launch proof receipt copy did not report success");
	    await waitFor(() => window.__smokeClipboardText.includes("JooPark Launch Proof Evidence Receipt") && window.__smokeClipboardText.includes("Status: blocked until live proof") && window.__smokeClipboardText.includes("Repo: biojuho/BIOJUHO-Projects") && window.__smokeClipboardText.includes(expectedEvidenceRepoLine) && launchProofEvidenceLabels.every((label) => window.__smokeClipboardText.includes(label)) && window.__smokeClipboardText.includes("Required proof commands:") && window.__smokeClipboardText.includes("Next proof actions:") && window.__smokeClipboardText.includes("nextAction=Run gh api repos/biojuho/BIOJUHO-Projects/pages") && window.__smokeClipboardText.includes("nextAction=Run node scripts/capture-output-quality-audit.mjs --write") && window.__smokeClipboardText.includes("Acceptance criteria:") && window.__smokeClipboardText.includes("Stop condition: do not post public launch copy, archive proof, or claim readyForExternalClaim until all six evidence fields are live, fresh, linked, and successful."), "publish launch proof receipt copy text did not reach clipboard");
    ...
    panel = qs("[data-system-publish-readiness]");
	    const outputQuality = qs("[data-system-output-quality-audit]", panel);
	    await waitFor(() => outputQuality.dataset.outputQualityAuditSource === "data/output-quality-audit.json" && outputQuality.dataset.outputQualityAuditLoaded === "true" && outputQuality.dataset.outputQualityAuditReleaseReady === "true" && outputQuality.dataset.outputQualityAuditExternalReady === "false" && outputQuality.dataset.outputQualityAuditLaunchPacketExternalReady === "false" && outputQuality.dataset.outputQualityAuditSourceEvidenceFresh === "true" && outputQuality.dataset.outputQualityAuditSourceEvidenceCount === "7" && outputQuality.dataset.outputQualityAuditSourceEvidenceStaleCount === "0" && outputQuality.dataset.outputQualityAuditArtifactRubricStatus === "pass" && outputQuality.dataset.outputQualityAuditArtifactRubricScore === "100" && outputQuality.dataset.outputQualityAuditArtifactRubricMaxScore === "100" && outputQuality.dataset.outputQualityAuditArtifactRubricPassingScore === "90" && outputQuality.dataset.outputQualityAuditArtifactRubricItemCount === "5" && outputQuality.dataset.outputQualityAuditCriteriaCount === "6" && outputQuality.dataset.outputQualityAuditCompletionCount === "8" && outputQuality.dataset.outputQualityAuditCompletionReady === "false" && Number(outputQuality.dataset.outputQualityAuditCompletionBlockedCount || 0) >= 3 && outputQuality.dataset.outputQualityAuditComparisonCount === "5" && outputQuality.dataset.outputQualityAuditWorkflowAuthPreflight === "true" && outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightUiVerified === "true" && outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightFields === "1" && outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightAvailable === "false" && outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightInstallBlocked === "true" && outputQuality.dataset.outputQualityAuditWorkflowAuthPreflightScopeCount === "3" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpoint === "true" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointCommandCount === "5" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointExpectedCount === "6" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointBlockedCount === "4" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointRecheckCount === "5" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointSourceArtifactCount === "4" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointDispatchApproval === "false" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointVerificationOnly === "true" && outputQuality.dataset.outputQualityAuditLaunchPostAuthCheckpointVerifyCommand === "node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown" && outputQuality.dataset.outputQualityAuditWorkflowUiInstallReceipt === "true" && outputQuality.dataset.outputQualityAuditWorkflowUiInstallReceiptCommandCount === "8" && outputQuality.dataset.outputQualityAuditWorkflowUiInstallReceiptChecklistCount === "6" && outputQuality.dataset.outputQualityAuditWorkflowUiInstallReceiptVerifyCommand === "node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown" && outputQuality.dataset.outputQualityAuditWorkflowUiInstallPastePacket === "true" && outputQuality.dataset.outputQualityAuditWorkflowUiInstallPastePacketCoverage === "1" && outputQuality.dataset.outputQualityAuditLaunchAcceptanceTotal === "5" && outputQuality.dataset.outputQualityAuditLaunchAcceptancePass === "2" && outputQuality.dataset.outputQualityAuditLaunchAcceptancePending === "3" && outputQuality.dataset.outputQualityAuditLaunchAcceptanceStage === "install_workflows" && outputQuality.dataset.outputQualityAuditBlockerResolution === "true" && outputQuality.dataset.outputQualityAuditBlockerResolutionActive === "operator_auth_path" && outputQuality.dataset.outputQualityAuditBlockerResolutionItemCount === "6" && outputQuality.dataset.outputQualityAuditBlockerResolutionActionRequiredCount === "3" && outputQuality.dataset.outputQualityAuditBlockerResolutionDeferredCount === "1" && outputQuality.dataset.outputQualityAuditBlockerResolutionProofCommandCount === "6" && outputQuality.dataset.outputQualityAuditInstallPathsReady === "true" && outputQuality.dataset.outputQualityAuditInstallPathCount === "2" && outputQuality.dataset.outputQualityAuditInstallPathCommandCount === "14", "output quality audit state was not surfaced", 15000);
    ...
    assert(outputQuality.dataset.outputQualityAuditVariantStatus === "pass" && outputQuality.dataset.outputQualityAuditVariantDecision === "keep_b" && outputQuality.dataset.outputQualityAuditVariantSelected === "copy_ready_evidence_receipt" && outputQuality.dataset.outputQualityAuditVariantScore === "6" && outputQuality.dataset.outputQualityAuditVariantBaselineScore === "2" && outputQuality.dataset.outputQualityAuditVariantMaxScore === "6" && outputQuality.dataset.outputQualityAuditVariantCount === "2" && outputQuality.dataset.outputQualityAuditVariantCriteriaCount === "4", "output quality variant comparison state was not surfaced");
    assert(outputQuality.dataset.outputQualityAuditLaunchProofEvidenceReceipt === "true" && outputQuality.dataset.outputQualityAuditLaunchProofEvidenceFields === "6" && outputQuality.dataset.outputQualityAuditLaunchProofEvidenceCoverage === "1", "output quality launch proof evidence receipt state was not surfaced");
	    assert(outputQuality.dataset.outputQualityAuditHandoffVerifierArtifact === "true" && outputQuality.dataset.outputQualityAuditHandoffVerifierArtifactCoverage === "2" && outputQuality.dataset.outputQualityAuditHandoffVerifierSafeToDispatch === "false" && outputQuality.dataset.outputQualityAuditHandoffVerifierJsonPath === "data/launch-handoff-verification.json" && outputQuality.dataset.outputQualityAuditHandoffVerifierMarkdownPath === "data/launch-handoff-verification.md", "output quality handoff verifier artifact state was not surfaced");
		    assert(outputQuality.dataset.outputQualityAuditSnapshotStatus === "pass" && outputQuality.dataset.outputQualityAuditReviewReady === "true" && outputQuality.dataset.outputQualityAuditIssueDecisionSummary === "true" && outputQuality.dataset.outputQualityAuditIssueDecisionSummaryFields === "6" && outputQuality.dataset.outputQualityAuditCommentNoteDecisionSummary === "true" && outputQuality.dataset.outputQualityAuditCommentNoteDecisionSummaryFields === "6" && outputQuality.dataset.outputQualityAuditRepairActionPlan === "true" && outputQuality.dataset.outputQualityAuditRepairActionPlanFields === "6" && outputQuality.dataset.outputQualityAuditSubmissionCloseoutSummary === "true" && outputQuality.dataset.outputQualityAuditSubmissionCloseoutSummaryFields === "6" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntake === "true" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeSource === "generated_from_launch_execution_packet" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeStatus === "collect_post_install_proof" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeFields === "6" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeCoverage === "1" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeCompletedCount === "0" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeProofComplete === "false" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeCommandCount === "4" && outputQuality.dataset.outputQualityAuditPostInstallEvidenceIntakeSignalCount === "8" && outputQuality.dataset.outputQualityAuditTrackerFormPayloadCount === "11" && outputQuality.dataset.outputQualityAuditTrackerFormPayloadChecksums === "true" && outputQuality.dataset.outputQualityAuditPublishEvidenceCommandGuard === "true" && outputQuality.dataset.outputQualityAuditPublishEvidenceImmediateAction === "true" && outputQuality.dataset.outputQualityAuditPublishEvidenceImmediateActionKey === "install_workflows" && outputQuality.dataset.outputQualityAuditPublishEvidenceSuggestedDispatchCount === "0" && outputQuality.dataset.outputQualityAuditPublishEvidenceWithheldDispatchCount === "2", "output quality audit snapshot state was not surfaced");
	    assert(outputQuality.dataset.outputQualityAuditNextActionReady === "true" && outputQuality.dataset.outputQualityAuditNextActionKey === "install_workflows" && outputQuality.dataset.outputQualityAuditNextActionStatus === "action_required" && outputQuality.dataset.outputQualityAuditNextActionSource === "data/launch-execution-packet.json" && outputQuality.dataset.outputQualityAuditNextActionCommand === "gh auth refresh -h github.com -s workflow" && outputQuality.dataset.outputQualityAuditNextActionDeferredKey === "capture-live-evidence" && outputQuality.dataset.outputQualityAuditNextActionDeferredCommand === "node scripts/capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown", "output quality structured next action state was not surfaced");
	    const qualityNextAction = qs("[data-output-quality-audit-next-action]", outputQuality);
	    assert(qualityNextAction.dataset.outputQualityAuditNextActionReady === "true" && qualityNextAction.textContent.includes("Structured next action") && qualityNextAction.textContent.includes("Install workflows on the default branch") && qualityNextAction.textContent.includes("remoteWorkflowFilesReady=true") && qualityNextAction.textContent.includes("gh auth refresh -h github.com -s workflow") && qualityNextAction.textContent.includes("Deferred: Capture live publish evidence") && qualityNextAction.textContent.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown"), "output quality structured next action card was not surfaced");
	    assert(outputQuality.dataset.outputQualityAuditExternalClaimGuardReady === "false" && outputQuality.dataset.outputQualityAuditExternalClaimGuardStatus === "blocked_external_claim" && outputQuality.dataset.outputQualityAuditExternalClaimGuardBlockedCount === "3" && outputQuality.dataset.outputQualityAuditExternalClaimGuardRequirementCount === "3" && outputQuality.dataset.outputQualityAuditExternalClaimGuardCommandCount === "5", "output quality external claim guard state was not surfaced");
    ...
    assert(outputQualityAuditBlockerResolutionSnapshot.dataset.outputQualityAuditBlockerResolutionGuard.includes("every action_required item has passed") && outputQualityAuditBlockerResolutionSnapshot.dataset.outputQualityAuditBlockerResolutionGuard.includes("verify-launch-handoff reports safeToDispatch=true"), "output quality blocker resolution guard was not surfaced");
	    assert(qualitySnapshot.length === 19 && qualitySnapshot.some((item) => item.textContent.includes("Review package") && item.textContent.includes("Final quality 6/6")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "review-decision-summaries" && item.textContent.includes("issue 6 fields") && item.textContent.includes("comment/note 6 fields") && item.textContent.includes("coverage 1")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "review-repair-action-plan" && item.textContent.includes("6 fields") && item.textContent.includes("coverage 1")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "submission-closeout-summary" && item.textContent.includes("6 fields") && item.textContent.includes("coverage 1")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "post-install-evidence-intake" && item.textContent.includes("6 fields") && item.textContent.includes("coverage 1") && item.textContent.includes("completed 0/6") && item.textContent.includes("proofComplete false") && item.textContent.includes("commands 4")) && qualitySnapshot.some((item) => item.textContent.includes("Tracker form payloads") && item.textContent.includes("11 fields") && item.textContent.includes("checksums ready")) && qualitySnapshot.some((item) => item.textContent.includes("Runtime issues") && item.textContent.includes("console 0")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "workflow-auth-preflight" && item.textContent.includes("Workflow auth preflight") && item.textContent.includes("ui verified") && item.textContent.includes("workflowScopeAvailable=false") && item.textContent.includes("workflowScopeInstallBlocked=true") && item.textContent.includes("missing workflow") && item.textContent.includes("scopes gist, read:org, repo")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "launch-post-auth-checkpoint" && item.textContent.includes("Launch post-auth checkpoint") && item.textContent.includes("5 commands") && item.textContent.includes("expected 6") && item.textContent.includes("blocked 4") && item.textContent.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown") && item.textContent.includes("every action_required post-auth checkpoint item has passed")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "workflow-ui-install-receipt" && item.textContent.includes("Workflow UI paste packet") && item.textContent.includes("workflowUiInstallPastePacketCoverage=1") && item.textContent.includes("8 commands") && item.textContent.includes("checklist 6") && item.textContent.includes("verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "launch-acceptance-checklist" && item.textContent.includes("2/5 pass") && item.textContent.includes("3 pending") && item.textContent.includes("install_workflows")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "blocker-resolution-checklist" && item.dataset.outputQualityAuditBlockerResolutionGuard.includes("every action_required item has passed") && item.dataset.outputQualityAuditBlockerResolutionGuard.includes("verify-launch-handoff reports safeToDispatch=true") && item.textContent.includes("Blocker resolution checklist") && item.textContent.includes("active operator_auth_path") && item.textContent.includes("2/6 pass") && item.textContent.includes("actionRequired 3") && item.textContent.includes("proofCommands 6") && item.textContent.includes("guard Do not run gh workflow run until every action_required item has passed") && item.textContent.includes("verify-launch-handoff reports safeToDispatch=true")) && qualitySnapshot.some((item) => item.dataset.outputQualityAuditSnapshotKey === "launch-install-path-options" && item.textContent.includes("2 paths") && item.textContent.includes("14 commands") && item.textContent.includes("CLI path after workflow scope") && item.textContent.includes("GitHub UI path") && item.textContent.includes("install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify")) && qualitySnapshot.some((item) => item.textContent.includes("Publish evidence command guard") && item.textContent.includes("0 suggested dispatch") && item.textContent.includes("2 withheld dispatch")) && qualitySnapshot.some((item) => item.textContent.includes("Publish evidence immediate action") && item.textContent.includes("install_workflows") && item.textContent.includes("deferred capture-live-evidence")), "output quality audit snapshot was not surfaced");
    ...
    assert(artifactRubric.textContent.includes("Artifact quality rubric") && artifactRubric.textContent.includes("Score 100/100") && artifactRubric.textContent.includes("pass threshold 90") && artifactRubric.textContent.includes("GitHub Issue Forms required inputs") && artifactRubricItems.length === 5 && artifactRubricItems.every((item) => item.dataset.outputQualityAuditArtifactRubricStatus === "pass") && artifactRubricItems.some((item) => item.dataset.outputQualityAuditArtifactRubricKey === "required_form_fit" && item.textContent.includes("11 field payloads") && item.textContent.includes("checksums ready")) && artifactRubricItems.some((item) => item.dataset.outputQualityAuditArtifactRubricKey === "copy_ready_completeness" && item.textContent.includes("workflowUiInstallPastePacket=true")) && artifactRubricItems.some((item) => item.dataset.outputQualityAuditArtifactRubricKey === "safety_guardrails" && item.textContent.includes("withheldDispatchCommands=2") && item.textContent.includes("readyForExternalClaim=false")), "output quality artifact rubric was not surfaced");
    assert(artifactRubricItems.some((item) => item.dataset.outputQualityAuditArtifactRubricKey === "copy_ready_completeness" && item.textContent.includes("externalClaimGuard=true") && item.textContent.includes("handoffVerifierArtifact=true") && item.textContent.includes("mainBridgePlan=true")), "output quality artifact rubric did not include external claim guard, handoff verifier, and main bridge readiness");
    ...
    assert(qualityInstallPaths.length === 2 && qualityInstallPaths.some((item) => item.textContent.includes("CLI path after workflow scope") && item.textContent.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify")) && qualityInstallPaths.some((item) => item.textContent.includes("GitHub UI path") && item.textContent.includes("pbcopy < 'docs/github-pages-workflow.yml'") && item.textContent.includes("open 'https://github.com/biojuho/BIOJUHO-Projects/new/main?filename=.github%2Fworkflows%2Fjoopark-pages.yml'")), "output quality launch install paths were not surfaced");
    ...
    assert(completionAuditItems.length === 8 && completionAuditItems.some((item) => item.dataset.outputQualityAuditCompletionKey === "release_quality_gate" && item.dataset.outputQualityAuditCompletionStatus === "pass") && completionAuditItems.some((item) => item.dataset.outputQualityAuditCompletionKey === "source_evidence_freshness" && item.dataset.outputQualityAuditCompletionStatus === "pass" && item.textContent.includes("staleSources=0")) && completionAuditItems.some((item) => item.dataset.outputQualityAuditCompletionKey === "workflow_installation" && item.dataset.outputQualityAuditCompletionStatus === "blocked" && item.textContent.includes("remoteWorkflowFilesReady=false") && item.textContent.includes("remoteWorkflowVisibilityReady=false")) && completionAuditItems.some((item) => item.dataset.outputQualityAuditCompletionKey === "public_launch_proof" && item.dataset.outputQualityAuditCompletionStatus === "blocked" && item.textContent.includes("postPublishEvidenceReady=false")) && completionAuditItems.some((item) => item.dataset.outputQualityAuditCompletionKey === "external_completion_claim" && item.textContent.includes("launchPacketReadyForExternalClaim=false") && item.textContent.includes("readyForExternalClaim=false")), "output quality completion audit checklist was not surfaced");
    ...
    assert(outputQuality.dataset.outputQualityAuditGoalReady === "true" && outputQuality.dataset.outputQualityAuditGoalBlockedCount === "0" && goalChecklistItems.length === 7 && goalChecklistItems.every((item) => item.dataset.outputQualityAuditGoalStatus === "pass") && goalChecklistItems.some((item) => item.dataset.outputQualityAuditGoalKey === "result_quality_diagnosis" && item.textContent.includes("artifactQualityRubric")) && externalComparisonGoalReady && autoresearchGoalReady, "output quality prompt-to-artifact checklist was not surfaced");
    ...
    await waitFor(() => {
      ...
      return qualityReceiptText.includes("JooPark Final Output Quality Audit Receipt") && qualityReceiptText.includes("Status: release quality ready; public launch proof blocked") && qualityReceiptText.includes("Repo: biojuho/BIOJUHO-Projects") && qualityReceiptText.includes(expectedOutputQualityEvidenceRepoLine) && qualityReceiptText.includes(expectedOutputQualityRepoResolutionLine) && !qualityReceiptText.includes("\\nRepo: OWNER/REPO") && qualityReceiptText.includes("Latest gate: npm run verify ->") && qualityReceiptText.includes("0 fail, 0 not_run, 0 blocked") && qualityReceiptText.includes("Workflow scope refresh command: gh auth refresh -h github.com -s workflow") && qualityReceiptText.includes("Remote workflow files ready: false") && qualityReceiptText.includes("Launch packet readyForExternalClaim: false") && qualityReceiptText.includes("Source evidence freshness:") && qualityReceiptText.includes("sourceEvidenceFresh=true; staleSources=0; sources=7") && qualityReceiptText.includes("remote workflow file check: fresh") && qualityReceiptText.includes("launch execution packet: fresh") && qualityReceiptText.includes("launch handoff verification: fresh") && qualityReceiptText.includes("main bridge plan: fresh") && qualityReceiptText.includes("Output readiness snapshot:") && qualityReceiptText.includes("Review package final quality: 6/6") && qualityReceiptText.includes("First-run guided start: pass (3 items, coverage=1)") && qualityReceiptText.includes("Review package decision brief: pass (6 fields, coverage=1)") && qualityReceiptText.includes("Review issue decision summary: pass (6 fields, coverage=1)") && qualityReceiptText.includes("Review comment/note decision summary: pass (6 fields, coverage=1)") && qualityReceiptText.includes("Tracker form payloads: pass (11 fields, checksums ready)") && qualityReceiptText.includes("Runtime issues: console 0, network 0, layout 0") && qualityReceiptText.includes("Workflow auth preflight: pass (uiVerified=true, workflowScopeAvailable=false, workflowScopeInstallBlocked=true, missing=workflow, scopes=gist, read:org, repo)") && qualityReceiptText.includes(outputQualityLaunchPostAuthCheckpointExpectedSummary) && qualityReceiptText.includes("Workflow UI paste packet: pass (workflowUiInstallPastePacketCoverage=1, 8 commands, checklist=6, verify=node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown)") && qualityReceiptText.includes("Launch handoff verifier artifact: pass") && qualityReceiptText.includes("Main PR bridge plan: pass") && qualityReceiptText.includes("Operator one-page handoff: pass") && qualityReceiptText.includes("successSignals=8") && qualityReceiptText.includes("Post-install evidence intake: pass (6 fields, coverage=1)") && qualityReceiptText.includes("Post-install proof parser: pass (6 fields, coverage=1)") && qualityReceiptText.includes("Launch acceptance checklist: 2/5 pass, pending=3, stage=install_workflows") && qualityReceiptText.includes("Launch install path options: pass (2 paths, 14 commands; CLI path after workflow scope | GitHub UI path)") && qualityReceiptText.includes("Artifact quality rubric:") && qualityReceiptText.includes("artifactQualityRubric=pass; totalScore=100/100; passingScore=90") && qualityReceiptText.includes("Required form fit: pass (20/20)") && qualityReceiptText.includes("Copy-ready completeness") && qualityReceiptText.includes("Safety guardrails: pass (20/20)") && qualityReceiptText.includes("GitHub Issue Forms required inputs") && qualityReceiptText.includes("Launch install path options:") && qualityReceiptText.includes("CLI path after workflow scope") && qualityReceiptText.includes("GitHub UI path") && qualityReceiptText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && qualityReceiptText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && qualityReceiptText.includes("Publish evidence command guard: pass (7 safe suggestions, 0 suggested dispatch, 2 withheld dispatch)") && qualityReceiptText.includes("Publish evidence immediate action: pass (install_workflows from data/launch-execution-packet.json, deferred capture-live-evidence)") && qualityReceiptText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && qualityReceiptText.includes("Deferred evidence capture: Capture live publish evidence") && qualityReceiptText.includes("Remote workflow file check: pages: remote workflow file is not installed on main") && qualityReceiptText.includes("Quality criteria:") && qualityReceiptText.includes("Prompt-to-artifact checklist:") && qualityReceiptText.includes("goalCompletionAudit=output_quality_goal_covered") && qualityReceiptText.includes("Result quality diagnosis: pass") && qualityReceiptText.includes("External output comparison: pass") && qualityReceiptText.includes("AutoResearch usage: pass") && qualityReceiptText.includes("Completion audit:") && qualityReceiptText.includes("completionAuditReady=false") && qualityReceiptText.includes("Source evidence freshness: pass") && qualityReceiptText.includes("Workflow installation: blocked") && qualityReceiptText.includes("remoteWorkflowFilesReady=false") && qualityReceiptText.includes("Public launch proof: blocked") && qualityReceiptText.includes("postPublishEvidenceReady=false") && qualityReceiptText.includes("External completion claim: blocked") && qualityReceiptText.includes("launchPacketReadyForExternalClaim=false") && qualityReceiptText.includes("readyForExternalClaim=false") && qualityReceiptText.includes("External comparison:") && qualityReceiptText.includes("GitHub issue forms validation") && qualityReceiptText.includes("GitHub Actions job summaries") && qualityReceiptText.includes("GitHub Releases") && qualityReceiptText.includes("Linear issue templates") && qualityReceiptText.includes("Jira required fields") && qualityReceiptText.includes("Public launch proof: blocked") && qualityReceiptText.includes("Do not present it as public launch completion");
	    ...
	    assert(qualityReceiptReadyText.includes("Workflow UI paste packet: pass (workflowUiInstallPastePacketCoverage=1, 8 commands, checklist=6, verify=node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown)") && qualityReceiptReadyText.includes("guard=Do not run gh workflow run until every post-install evidence field has been filled") && qualityReceiptReadyText.includes("remoteWorkflowFilesReady=true") && qualityReceiptReadyText.includes("verify-launch-handoff reports safeToDispatch=true."), "output quality workflow UI paste packet guard was not copy-ready");
    assert(qualityReceiptReadyText.includes("External claim closeout packet:") && qualityReceiptReadyText.includes("default branch workflow_dispatch") && qualityReceiptReadyText.includes("Required proof fields:") && qualityReceiptReadyText.includes("workflow run summary") && qualityReceiptReadyText.includes("Release-note archive claim") && qualityReceiptReadyText.includes("Allowed claim after proof:") && qualityReceiptReadyText.includes("Forbidden until proof:"), "output quality external claim closeout packet was not copy-ready");
	    assert(qualityReceiptReadyText.includes("Output variant comparison:") && qualityReceiptReadyText.includes("status=pass; decision=keep_b; selected=copy_ready_evidence_receipt; score=6/6; baseline=2/6") && qualityReceiptReadyText.includes("A: generic generated summary: rejected") && qualityReceiptReadyText.includes("B: copy-ready evidence receipt: selected") && qualityReceiptReadyText.includes("Copy-ready field payloads: winner=copy_ready_evidence_receipt") && qualityReceiptReadyText.includes("Launch safety: winner=copy_ready_evidence_receipt"), "output quality output variant comparison was not copy-ready");
	    ...
	    assert(qualityReceiptReadyText.includes("Post-install proof parser: pass (6 fields, coverage=1)") && qualityReceiptReadyText.includes("detected=6/6") && qualityReceiptReadyText.includes("falsePositiveGuard=true") && qualityReceiptReadyText.includes("not dispatch approval"), "output quality post-install proof parser was not copy-ready");
	    ...
	    assert(qualityReceiptReadyText.includes("Post-install quick proof field mapping: pass (0/4 mapped fields complete, coverage=1)"), "output quality post-install quick proof field mapping was not copy-ready");
	    assert(qualityReceiptReadyText.includes("Post-install evidence intake:") && qualityReceiptReadyText.includes("source=generated_from_launch_execution_packet; status=collect_post_install_proof; proofComplete=false; completed=0/6; commands=4; signals=8") && qualityReceiptReadyText.includes("quickProofReady=true; steps=4; coverage=1") && qualityReceiptReadyText.includes("quickProofFieldMappingReady=true; mapped=4; completed=0/4; coverage=1") && qualityReceiptReadyText.includes("quick proof 1 remote_file_parity: evidence_required") && qualityReceiptReadyText.includes("quick proof 4 handoff_verifier: evidence_required") && qualityReceiptReadyText.includes("quick proof field 1 remote_file_parity -> remote_parity_proof") && qualityReceiptReadyText.includes("quick proof field 4 handoff_verifier -> handoff_verifier_proof") && qualityReceiptReadyText.includes("remote_parity_proof: evidence_required") && qualityReceiptReadyText.includes("currentValue=remoteWorkflowFilesReady=false") && qualityReceiptReadyText.includes("handoff_verifier_proof: evidence_required") && qualityReceiptReadyText.includes("Stop condition: do not run gh workflow run"), "output quality post-install evidence intake ledger was not copy-ready");
    ...
    assert(qualityReceiptReadyText.includes("Blocker resolution checklist: pass (active=operator_auth_path, 2/6 pass, actionRequired=3, deferred=1, proofCommands=6, guard=Do not run gh workflow run until every action_required item has passed and verify-launch-handoff reports safeToDispatch=true.)") && qualityReceiptReadyText.includes("Blocker resolution checklist:") && qualityReceiptReadyText.includes("proofCommand=gh auth refresh -h github.com -s workflow") && qualityReceiptReadyText.includes("expectedValue=remoteWorkflowFilesReady=true") && qualityReceiptReadyText.includes("stopCondition=If safeToDispatch=false"), "output quality blocker resolution checklist was not copy-ready");
    ...
    assert(qualityReceiptReadyText.includes("Remote workflow file acceptance ledger: pass (0/2 files ready, missing=2, mismatch=0, status=remote_file_install_required)") && qualityReceiptReadyText.includes("Publish JooPark Pages: missing_on_default_branch") && qualityReceiptReadyText.includes("Watch JooPark Candidate Drift: missing_on_default_branch") && qualityReceiptReadyText.includes("remoteExists=false") && qualityReceiptReadyText.includes("remoteMatchesTemplate=false"), "output quality remote workflow file acceptance ledger was not copy-ready");
    assert(qualityReceiptReadyText.includes("Launch proof acceptance ledger: pass (0/6 ready, pending=6, gate=capture_launch_proof, status=proof_blocked_until_dispatch)") && qualityReceiptReadyText.includes("Pages site URL/status: blocked_until_dispatch") && qualityReceiptReadyText.includes("Pages workflow run: blocked_until_dispatch") && qualityReceiptReadyText.includes("Drift Watch workflow run: blocked_until_dispatch") && qualityReceiptReadyText.includes("Public claim guard: guarded") && qualityReceiptReadyText.includes("capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --write"), "output quality launch proof acceptance ledger was not copy-ready");
    assert(qualityReceiptReadyText.includes("External completion claim guard:") && qualityReceiptReadyText.includes("status=blocked_external_claim; ready=false; blocked=3/3") && qualityReceiptReadyText.includes("signal readyForExternalClaim=false") && qualityReceiptReadyText.includes("proofCommand node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown") && qualityReceiptReadyText.includes("Stop condition: do not claim readyForExternalClaim"), "output quality external claim guard receipt section was not copy-ready");
    window.__smokeClipboardText = "";
    ...
    await waitFor(() => window.__smokeClipboardText.includes("JooPark Final Output Quality Audit Receipt") && window.__smokeClipboardText.includes("release quality ready; public launch proof blocked") && window.__smokeClipboardText.includes("Repo: biojuho/BIOJUHO-Projects") && window.__smokeClipboardText.includes(expectedOutputQualityEvidenceRepoLine) && !window.__smokeClipboardText.includes("\\nRepo: OWNER/REPO") && window.__smokeClipboardText.includes("Workflow scope refresh command: gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("Remote workflow files ready: false") && window.__smokeClipboardText.includes("Launch packet readyForExternalClaim: false") && window.__smokeClipboardText.includes("Source evidence freshness:") && window.__smokeClipboardText.includes("sourceEvidenceFresh=true; staleSources=0; sources=7") && window.__smokeClipboardText.includes("remote workflow file check: fresh") && window.__smokeClipboardText.includes("launch handoff verification: fresh") && window.__smokeClipboardText.includes("main bridge plan: fresh") && window.__smokeClipboardText.includes("Output readiness snapshot:") && window.__smokeClipboardText.includes("First-run guided start: pass (3 items, coverage=1)") && window.__smokeClipboardText.includes("Review package decision brief: pass (6 fields, coverage=1)") && window.__smokeClipboardText.includes("Review issue decision summary: pass (6 fields, coverage=1)") && window.__smokeClipboardText.includes("Review comment/note decision summary: pass (6 fields, coverage=1)") && window.__smokeClipboardText.includes("Tracker form payloads: pass (11 fields, checksums ready)") && window.__smokeClipboardText.includes("Runtime issues: console 0, network 0, layout 0") && window.__smokeClipboardText.includes("Workflow auth preflight: pass (uiVerified=true, workflowScopeAvailable=false, workflowScopeInstallBlocked=true, missing=workflow, scopes=gist, read:org, repo)") && window.__smokeClipboardText.includes(outputQualityLaunchPostAuthCheckpointExpectedSummary) && window.__smokeClipboardText.includes("Workflow UI paste packet: pass (workflowUiInstallPastePacketCoverage=1, 8 commands, checklist=6, verify=node scripts/verify-launch-handoff.mjs --repo biojuho/BIOJUHO-Projects --write --markdown)") && window.__smokeClipboardText.includes("Launch handoff verifier artifact: pass") && window.__smokeClipboardText.includes("Main PR bridge plan: pass") && window.__smokeClipboardText.includes("Operator one-page handoff: pass") && window.__smokeClipboardText.includes("successSignals=8") && window.__smokeClipboardText.includes("Launch acceptance checklist: 2/5 pass, pending=3, stage=install_workflows") && window.__smokeClipboardText.includes("Launch install path options: pass (2 paths, 14 commands; CLI path after workflow scope | GitHub UI path)") && window.__smokeClipboardText.includes("Artifact quality rubric:") && window.__smokeClipboardText.includes("artifactQualityRubric=pass; totalScore=100/100; passingScore=90") && window.__smokeClipboardText.includes("Required form fit: pass (20/20)") && window.__smokeClipboardText.includes("Copy-ready completeness: pass (20/20)") && window.__smokeClipboardText.includes("Safety guardrails: pass (20/20)") && window.__smokeClipboardText.includes("CLI path after workflow scope") && window.__smokeClipboardText.includes("GitHub UI path") && window.__smokeClipboardText.includes("node scripts/install-remote-workflow-files.mjs --repo biojuho/BIOJUHO-Projects --write --verify") && window.__smokeClipboardText.includes("pbcopy < 'docs/github-pages-workflow.yml'") && window.__smokeClipboardText.includes("Publish evidence command guard: pass (7 safe suggestions, 0 suggested dispatch, 2 withheld dispatch)") && window.__smokeClipboardText.includes("Publish evidence immediate action: pass (install_workflows from data/launch-execution-packet.json, deferred capture-live-evidence)") && window.__smokeClipboardText.includes("Immediate command: gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("Deferred evidence capture: Capture live publish evidence") && window.__smokeClipboardText.includes("Remote workflow file check: pages: remote workflow file is not installed on main") && window.__smokeClipboardText.includes("Prompt-to-artifact checklist:") && window.__smokeClipboardText.includes("goalCompletionAudit=output_quality_goal_covered") && window.__smokeClipboardText.includes("External output comparison: pass") && window.__smokeClipboardText.includes("Completion audit:") && window.__smokeClipboardText.includes("Source evidence freshness: pass") && window.__smokeClipboardText.includes("Workflow installation: blocked") && window.__smokeClipboardText.includes("Public launch proof: blocked") && window.__smokeClipboardText.includes("launchPacketReadyForExternalClaim=false") && window.__smokeClipboardText.includes("readyForExternalClaim=false") && window.__smokeClipboardText.includes("External comparison:") && window.__smokeClipboardText.includes("GitHub issue forms validation") && window.__smokeClipboardText.includes("GitHub Actions job summaries") && window.__smokeClipboardText.includes("GitHub Releases") && window.__smokeClipboardText.includes("Linear issue templates") && window.__smokeClipboardText.includes("Jira required fields") && window.__smokeClipboardText.includes("Public launch proof: blocked") && window.__smokeClipboardText.includes("node scripts/capture-publish-evidence.mjs --live --repo biojuho/BIOJUHO-Projects --markdown"), "output quality audit receipt copy text did not reach clipboard");
	    ...
	    assert(window.__smokeClipboardText.includes("guard=Do not run gh workflow run until every post-install evidence field has been filled") && window.__smokeClipboardText.includes("verify-launch-handoff reports safeToDispatch=true."), "output quality workflow UI paste packet guard copy did not reach clipboard");
		    ...
		    assert(window.__smokeClipboardText.includes("Output variant comparison:") && window.__smokeClipboardText.includes("decision=keep_b; selected=copy_ready_evidence_receipt") && window.__smokeClipboardText.includes("A: generic generated summary: rejected") && window.__smokeClipboardText.includes("B: copy-ready evidence receipt: selected"), "output quality output variant comparison copy did not reach clipboard");
	    ...
	    assert(window.__smokeClipboardText.includes("Post-install evidence intake: pass (6 fields, coverage=1)") && window.__smokeClipboardText.includes("Post-install proof parser: pass (6 fields, coverage=1)"), "output quality post-install evidence intake copy did not reach clipboard");
	    assert(window.__smokeClipboardText.includes("Post-install proof parser: pass (6 fields, coverage=1)") && window.__smokeClipboardText.includes("detected=6/6") && window.__smokeClipboardText.includes("falsePositiveGuard=true") && window.__smokeClipboardText.includes("not dispatch approval"), "output quality post-install proof parser copy did not reach clipboard");
	    assert(window.__smokeClipboardText.includes("Post-install quick proof: pass (4 steps, coverage=1)") && window.__smokeClipboardText.includes("quickProofReady=true; steps=4; coverage=1") && window.__smokeClipboardText.includes("Post-install quick proof field mapping: pass (0/4 mapped fields complete, coverage=1)") && window.__smokeClipboardText.includes("quickProofFieldMappingReady=true; mapped=4; completed=0/4; coverage=1"), "output quality post-install quick proof copy did not reach clipboard");
	    assert(window.__smokeClipboardText.includes("source=generated_from_launch_execution_packet; status=collect_post_install_proof; proofComplete=false; completed=0/6; commands=4; signals=8") && window.__smokeClipboardText.includes("quick proof 1 remote_file_parity: evidence_required") && window.__smokeClipboardText.includes("quick proof 4 handoff_verifier: evidence_required") && window.__smokeClipboardText.includes("quick proof field 1 remote_file_parity -> remote_parity_proof") && window.__smokeClipboardText.includes("quick proof field 4 handoff_verifier -> handoff_verifier_proof") && window.__smokeClipboardText.includes("remote_parity_proof: evidence_required") && window.__smokeClipboardText.includes("handoff_verifier_proof: evidence_required"), "output quality post-install evidence intake ledger copy did not reach clipboard");
    ...
    assert(window.__smokeClipboardText.includes("Blocker resolution checklist: pass (active=operator_auth_path, 2/6 pass, actionRequired=3, deferred=1, proofCommands=6, guard=Do not run gh workflow run until every action_required item has passed and verify-launch-handoff reports safeToDispatch=true.)") && window.__smokeClipboardText.includes("proofCommand=gh auth refresh -h github.com -s workflow") && window.__smokeClipboardText.includes("stopCondition=If safeToDispatch=false"), "output quality blocker resolution checklist copy did not reach clipboard");
    ...
    assert(window.__smokeClipboardText.includes("Remote workflow file acceptance ledger: pass (0/2 files ready, missing=2, mismatch=0, status=remote_file_install_required)") && window.__smokeClipboardText.includes("Publish JooPark Pages: missing_on_default_branch") && window.__smokeClipboardText.includes("remoteMatchesTemplate=false"), "output quality remote workflow file acceptance ledger copy did not reach clipboard");
    assert(window.__smokeClipboardText.includes("Launch proof acceptance ledger: pass (0/6 ready, pending=6, gate=capture_launch_proof, status=proof_blocked_until_dispatch)") && window.__smokeClipboardText.includes("Pages site URL/status: blocked_until_dispatch") && window.__smokeClipboardText.includes("Public claim guard: guarded"), "output quality launch proof acceptance ledger copy did not reach clipboard");
    assert(window.__smokeClipboardText.includes("External completion claim guard:") && window.__smokeClipboardText.includes("status=blocked_external_claim; ready=false; blocked=3/3") && window.__smokeClipboardText.includes("signal readyForExternalClaim=false") && window.__smokeClipboardText.includes("Stop condition: do not claim readyForExternalClaim"), "output quality external claim guard receipt copy did not reach clipboard");
  ...
  await runStep("command palette opens created note record", async () => {
    ...
    assert(helpTrigger.getAttribute("aria-expanded") === "true" && help.dataset.globalHelpAccessReady === "true" && help.dataset.globalHelpAccessCoverage === "1" && help.dataset.globalHelpAccessActionCount === "4" && help.dataset.globalHelpCurrentView === "todo" && help.dataset.globalHelpSearchMode === "view" && help.dataset.globalHelpSafeToDispatch === "false" && help.dataset.globalHelpReadyForExternalClaim === "false" && help.dataset.globalHelpConsistentHelp === "wcag-3.2.6" && help.dataset.globalHelpStatusRole === "status", "global help access dataset was incomplete");
    assert(helpStatus.getAttribute("role") === "status" && helpStatus.getAttribute("aria-live") === "polite" && helpStatus.textContent.includes("할 일") && helpStatus.textContent.includes("safeToDispatch=false") && helpStatus.textContent.includes("readyForExternalClaim=false"), "global help status message was not programmatically exposed");
