...
(async () => {
  ...
  let pipelineViewModuleOk = false;
  ...
  let llmWikiTodoNoteSourceFilterOk = false;
	  let llmWikiTodoNoteSourceSummaryOk = false;
	  let llmWikiTodoNoteSourcePaletteOk = false;
	  let commandPalettePersonalSourceCountsOk = false;
	  let commandPaletteKanbanSourceCountsOk = false;
  ...
  await runStep("workspace candidate portfolio search", async () => {
    ...
    const architectureQueue = dashboard.projects
      .filter((project) => project.sourceKind === "adoption-candidate" && projectCandidateAction(project)?.key === "architecture")
      .sort(compareCandidatePriorityThenName);
    const topArchitectureCandidate = architectureQueue[0];
    ...
    assert(topArchitectureCandidate && actionSummary.innerText.includes(topArchitectureCandidate.name), "architecture action summary top candidate did not render");
    ...
    assert(qs("[data-candidate-action]", planeCard).textContent.includes(projectCandidateAction(planeCandidate).label), "Plane candidate action did not render current recommendation");
    ...
    assert(qs("[data-candidate-action]", appFlowyCard).textContent.includes(projectCandidateAction(appFlowyCandidate).label), "AppFlowy candidate action did not render current recommendation");
    ...
    assert(qs("[data-candidate-action]", affineCard).textContent.includes(projectCandidateAction(affineCandidate).label), "AFFiNE candidate action did not render current recommendation");
    ...
    assert(qs("[data-candidate-action]", outlineCard).textContent.includes(projectCandidateAction(outlineCandidate).label), "Outline candidate action did not render current recommendation");
    ...
    assert(qs("[data-candidate-action]", bookStackCard).textContent.includes(projectCandidateAction(bookStackCandidate).label), "BookStack candidate action did not render current recommendation");
    ...
    assert(qs("[data-candidate-action]", wikiJsCard).textContent.includes(projectCandidateAction(wikiJsCandidate).label), "Wiki.js candidate action did not render current recommendation");
  ...
  await runStep("pipeline matrix view", async () => {
    await nav("pm-pipeline");
    assert(window.JooParkPipelineView && window.JooParkPipelineView.version === "joopark-pipeline-view/v1" && typeof window.JooParkPipelineView.create === "function", "pipeline view runtime module was not loaded");
    pipelineViewModuleOk = true;
    assert(!!document.querySelector('#view-pm-pipeline .pipe-matrix[role="table"]'), "pipeline matrix did not render");
    assert(document.querySelectorAll('#view-pm-pipeline [data-action="open-pipeline-cell"]').length > 0, "pipeline cell buttons did not render");
  });
  ...
  await runStep("llm wiki article actions create local drafts", async () => {
    ...
    let todoSourceSummary = qs('#view-todo [data-todo-source-summary]');
    assert(todoSourceSummary.dataset.todoSourceSummaryFilter === "wiki" && todoSourceSummary.dataset.todoSourceSummaryCount === "1" && todoSourceSummary.textContent.includes("LLM Wiki") && todoSourceSummary.textContent.includes("1건") && todoSourceSummary.textContent.includes("전체 출처 보기"), "llm wiki todo source summary did not render active scope");
    click('[data-action="todo-source-filter"][data-todo-source-filter="all"]', todoSourceSummary);
    ...
    todoSourceSummary = qs('#view-todo [data-todo-source-summary]');
    assert(todoSourceSummary.dataset.todoSourceSummaryFilter === "wiki" && todoSourceSummary.dataset.todoSourceSummaryCount === "1", "llm wiki todo source summary did not restore after reapplying source scope");
    ...
    let noteSourceSummary = qs('#view-notes [data-note-source-summary]');
    assert(noteSourceSummary.dataset.noteSourceSummaryFilter === "wiki" && noteSourceSummary.dataset.noteSourceSummaryCount === "1" && noteSourceSummary.textContent.includes("LLM Wiki") && noteSourceSummary.textContent.includes("1건") && noteSourceSummary.textContent.includes("전체 출처 보기"), "llm wiki note source summary did not render active scope");
    click('[data-action="note-source-filter"][data-note-source-filter="all"]', noteSourceSummary);
    ...
    noteSourceSummary = qs('#view-notes [data-note-source-summary]');
    assert(noteSourceSummary.dataset.noteSourceSummaryFilter === "wiki" && noteSourceSummary.dataset.noteSourceSummaryCount === "1", "llm wiki note source summary did not restore after reapplying source scope");
    llmWikiTodoNoteSourceFilterOk = true;
    llmWikiTodoNoteSourceSummaryOk = true;
    click('[data-action="open-palette"]');
    ...
    const todoWikiSourceCommand = qsa("#paletteResults .pal-item").find((item) => item.textContent.includes("할 일: LLM Wiki 출처 보기"));
    assert(todoWikiSourceCommand.textContent.includes("source filter") && todoWikiSourceCommand.textContent.includes("1건"), "todo source filter palette command did not expose result count");
    todoWikiSourceCommand.click();
    ...
    const noteWikiSourceCommand = qsa("#paletteResults .pal-item").find((item) => item.textContent.includes("메모: LLM Wiki 출처 보기"));
    assert(noteWikiSourceCommand.textContent.includes("source filter") && noteWikiSourceCommand.textContent.includes("1건"), "note source filter palette command did not expose result count");
    commandPalettePersonalSourceCountsOk = true;
    noteWikiSourceCommand.click();
    ...
    await waitFor(() => qs('#view-pm-kanban [data-kanban-source-filterbar]').dataset.kanbanSourceFilterCurrent === "all", "llm wiki source filter did not reset to all");
	    ...
	    const kanbanWikiSourceCommand = qsa("#paletteResults .pal-item").find((item) => item.textContent.includes("Kanban: LLM Wiki 출처 보기"));
	    assert(kanbanWikiSourceCommand.textContent.includes("source filter") && kanbanWikiSourceCommand.textContent.includes("1건"), "kanban source filter palette command did not expose result count");
	    commandPaletteKanbanSourceCountsOk = true;
	    kanbanWikiSourceCommand.click();
  ...
  const persistedChecks = {
    ...
    pipelineViewModule: pipelineViewModuleOk,
    ...
    llmWikiTodoNoteModalSource: llmWikiTodoNoteModalSourceOk,
	    ...
	    llmWikiTodoNoteSourceSummary: llmWikiTodoNoteSourceSummaryOk,
	    commandPalettePersonalSourceCounts: commandPalettePersonalSourceCountsOk,
	    commandPaletteKanbanSourceCounts: commandPaletteKanbanSourceCountsOk,
  ...
  await runStep("settings reset all workspace data", async () => {
    ...
    click('[data-home-first-run-step-key="find_anything"] [data-action="open-palette"]', firstRun);
    await waitFor(() => document.querySelector("#palette.open"), "home first-run palette action did not open command palette");
    qs("#paletteInput").dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true, cancelable: true }));
    await waitFor(() => !document.querySelector("#palette.open"), "home first-run palette did not close after Escape");
    click('[data-home-first-run-step-key="protect_data"] [data-action="open-backup-panel"]', qs("#view-home [data-home-first-run-guidance]"));
    ...
    await waitFor(() => !!document.querySelector("#view-settings [data-settings-backup]"), "home first-run backup deep link did not reach the backup panel");
    await nav("home");
