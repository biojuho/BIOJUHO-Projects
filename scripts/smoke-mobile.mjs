  ...
  try {
    ...
    paletteMobileReport = await evaluate(pageClient, `(() => {
      ...
      return (async () => {
        ...
        // No-match queries now render the 바로 만들기 quick-capture fallback
        // (2 items) instead of a dead-end empty status.
        input.value = "zzzz-no-match-mobile";
        input.dispatchEvent(new Event("input", { bubbles: true }));
        await waitFor(() => /바로 만들기/.test(results.textContent), "palette mobile quick-capture fallback did not appear");
        await nextFrame();
        const statusRect = rect(status);
        const emptyResultsRect = rect(results);
        const emptyHintRect = rect(hint);
        const fallbackItems = Array.from(results.querySelectorAll(".pal-item"));
        if (fallbackItems.length !== 2) issues.push("palette_quick_capture_item_count " + fallbackItems.length);
        const fallbackRects = fallbackItems.map(rect);
        const fallbackMinHeight = fallbackRects.length ? Math.min(...fallbackRects.map((r) => r.height)) : 0;
        if (fallbackMinHeight && fallbackMinHeight < 40) issues.push("palette_quick_capture_touch_target_too_short " + fallbackMinHeight);
        if (fallbackRects.some((r) => !withinViewport(r))) issues.push("palette_quick_capture_outside_viewport");
        ...
        const noResultsText = (results.textContent || "").replace(/\\s+/g, " ").trim().slice(0, 160);
