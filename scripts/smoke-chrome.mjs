...
async function main() {
  ...
  try {
    ...
    pageClient.on("Runtime.consoleAPICalled", (params) => {
      ...
      const text = (params.args || []).map(formatArg).filter(Boolean).join(" ");
      console.error(`BROWSER CONSOLE ${params.type.toUpperCase()}:`, text);
      consoleIssues.push({
        route: currentRoute,
        type: params.type,
        text,
    ...
    pageClient.on("Runtime.exceptionThrown", (params) => {
      const desc = params.exceptionDetails?.exception?.description || params.exceptionDetails?.text || "uncaught exception";
      console.error("BROWSER EXCEPTION:", desc, JSON.stringify(params.exceptionDetails, null, 2));
      consoleIssues.push({
        route: currentRoute,
        type: "exception",
        text: desc,
