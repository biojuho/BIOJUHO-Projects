(function (root) {
  ...
  function createSystemStatusView(deps) {
    ...
    function systemMetricHTML(label, value, tip) {
      return html`<div ${tip ? raw(`title="${tip}"`) : ""}><dt>${label}</dt><dd>${value}</dd></div>`;
    }

    function systemMetricsHTML(items) {
      return listOf(items).map((item) => systemMetricHTML(item.label, item.value, item.tip)).join("");
    }
    ...
    function systemKpisHTML(model) {
      ...
      const kpis = [
        { title: "라우트", value: "#system", unit: "", color: "#2387ff", badge: "#", delta: `${model.routeCount}개 화면`, tip: "현재 시스템 상태 화면" },
        { title: "저장소", value: storageView.statusLabel, unit: "", color: storageColor(storageView.tone), badge: storageBadge(storageView.tone), delta: formatBytes(storageView.localBytes), tip: "로컬 스토리지 상태" },
        { title: "후보 스냅샷", value: `${model.sourceBacked.length}/${model.adoptionCandidates.length}`, unit: "개", color: "#22d3ee", badge: "SRC", delta: `asset ${snapshotHealth.loadedCount || 0}/${snapshotHealth.sourceCount || 0} · 벤치 ${model.benchmarkFocused.length}개`, tip: "소스 근거가 있는 후보 수" },
        { title: "알림", value: String(model.alertCount), unit: "건", color: model.alertCount > 0 ? "#f7a928" : "#17d983", badge: "OPS", delta: `전체 ${model.alerts.length}건`, tip: "운영 알림 수" },
      ];
      return html`
    ...
    function operationalSurfaceHTML(model) {
      ...
      const metrics = [
        { label: "프로젝트", value: `${model.projects.length}개`, tip: "등록된 프로젝트 수" },
        { label: "소스 근거", value: `${model.sourceBacked.length}개`, tip: "소스 코드와 연결된 후보 수" },
        { label: "벤치 포커스", value: `${model.benchmarkFocused.length}개`, tip: "벤치마크 대상 프로젝트 수" },
        { label: "벤치 분포", value: `PM ${model.pmBenchmarkCount} · Workspace ${model.workspaceBenchmarkCount} · KB/IA ${model.knowledgeBaseBenchmarkCount}`, tip: "벤치마크 유형별 분포" },
        { label: "이슈", value: `${listOf(dashboard.issues).length}개`, tip: "전체 이슈 수" },
        { label: "DB 카탈로그 정상", value: `${model.healthyDbCount}/${listOf(dashboard.dbInstances).length}`, tip: "정상 상태의 DB 인스턴스 비율" },
        { label: "테이블", value: `${model.totalTables}개`, tip: "등록된 DB 테이블 수" },
        { label: "질의 경고", value: `${model.slowQueries.length}건`, tip: "느린 쿼리 경고 수" },
        { label: "대기 마이그", value: `${model.pendingMigrations.length}건`, tip: "대기 중인 마이그레이션 수" },
      ];
      return html`
    ...
    function projectSnapshotHealthHTML(health) {
      ...
      const metrics = [
        { label: "loaded sources", value: `${loadedCount}/${sourceCount}`, tip: "로드된 소스 파일 수" },
        { label: "merged projects", value: projectCount, tip: "병합된 프로젝트 수" },
        { label: "applied", value: source.applied ? "true" : "false", tip: "소스 스냅샷 적용 여부" },
        { label: "reason", value: source.appliedReason || "not checked", tip: "적용 또는 미적용 사유" },
      ];
    ...
    function githubProjectDiscoveryHTML(source) {
      ...
      const metrics = [
        { label: "local git", value: `${model.localRepos} repos`, tip: "로컬에서 발견된 Git 저장소 수" },
        { label: "GitHub owner", value: `${model.owner} · ${model.githubRepos} repos`, tip: "GitHub 소유자와 저장소 수" },
        { label: "ranked", value: `${model.rankedProjects} projects`, tip: "순위가 매겨진 프로젝트 수" },
        { label: "dirty local", value: `${model.dirtyRepos} repos`, tip: "변경사항이 있는 로컬 저장소 수" },
        { label: "pushedAt coverage", value: `${model.githubReposWithPushedAt}/${model.githubRepos}`, tip: "pushedAt 정보가 있는 GitHub 저장소 비율" },
        { label: `${model.recentWindowDays}d fresh`, value: `${model.freshGithubRepos} repos`, tip: `${model.recentWindowDays}일 이내에 업데이트된 GitHub 저장소 수` },
        { label: "scan replay", value: model.reproducibilityReady ? `${model.localScanMaxDepth} depth` : "needs review", tip: "스캔 재현성 상태" },
        { label: "private metadata", value: model.privateMetadataReady ? `${model.privateGithubRepos + model.privateLocalRemotes} redacted` : "needs review", tip: "개인 메타데이터 제거 상태" },
        { label: "private rows", value: model.privateRowExposure === 0 ? `${model.privateRankedRowsRedacted} redacted` : "needs review", tip: "개인 행 노출 상태" },
        { label: "release target", value: yesNo(model.releaseTargetReady), tip: "배포 대상 준비 상태" },
        { label: "path privacy", value: model.publicSafe ? model.localPathMode : "needs review", tip: "경로 프라이버시 상태" },
      ];
      return html`
        <section class="panel" role="status" aria-live="polite" data-system-github-project-discovery data-github-project-discovery-source="${model.source}" data-github-project-discovery-loaded="${model.loaded ? "true" : "false"}" data-github-project-discovery-public-safe="${model.publicSafe ? "true" : "false"}" data-github-project-discovery-local-count="${model.localRepos}" data-github-project-discovery-github-count="${model.githubRepos}" data-github-project-discovery-ranked-count="${model.rankedProjects}" data-github-project-discovery-release-target-ready="${model.releaseTargetReady ? "true" : "false"}" data-github-project-discovery-ab-decision="${model.abDecision}" data-github-project-discovery-local-path-mode="${model.localPathMode}" data-github-project-discovery-freshness-ready="${model.freshnessReady ? "true" : "false"}" data-github-project-discovery-reproducible="${model.reproducibilityReady ? "true" : "false"}" data-github-project-discovery-private-redacted="${model.privateMetadataReady ? "true" : "false"}" data-github-project-discovery-private-count="${model.privateGithubRepos}" data-github-project-discovery-private-local-remote-count="${model.privateLocalRemotes}" data-github-project-discovery-private-rows-redacted="${model.privateRankedRowsRedacted}" data-github-project-discovery-private-exposure="${model.privateMetadataExposure}" data-github-project-discovery-private-row-exposure="${model.privateRowExposure}" data-github-project-discovery-local-scan-depth="${model.localScanMaxDepth}" data-github-project-discovery-ignored-count="${model.localScanIgnoredCount}" data-github-project-discovery-pushed-count="${model.githubReposWithPushedAt}" data-github-project-discovery-fresh-count="${model.freshGithubRepos}" data-github-project-discovery-source-field-count="${model.sourceFieldCount}" data-github-project-discovery-tone="${tone}">
          ...
          <ul class="settings-info" data-github-project-discovery-list>
            ${raw(model.projects.map((project) => html`
              <li data-github-project-discovery-row data-github-project-discovery-project="${project.nameWithOwner || ""}" data-github-project-discovery-relation="${project.relation || ""}" data-github-project-discovery-local="${project.localCheckout ? "true" : "false"}" data-github-project-discovery-pushed-at="${project.pushedAt || ""}" data-github-project-discovery-stars="${finiteNumber(project.stargazerCount, 0)}" title="${project.nameWithOwner || 'unknown'}: ${project.relation || 'unknown'} - ${project.localCheckout ? '로컬' : '원격'} - pushed ${project.pushedAt || 'unknown'} - stars ${finiteNumber(project.stargazerCount, 0)}">
    ...
    function pwaRuntimeHTML(model) {
      ...
      const metrics = [
        { label: "secure context", value: yesNo(runtime.secureContext || runtime.localHostContext), tip: "HTTPS 또는 로컬 호스트 컨텍스트 여부" },
        { label: "service worker", value: serviceWorkerRuntimeLabel(runtime), tip: "서비스 워커 등록 및 활성화 상태" },
        { label: "app shell cache", value: appShellCacheRuntimeLabel(runtime), tip: "오프라인 앱 셸 캐시 준비 상태" },
        { label: "manifest", value: yesNo(runtime.manifestLinked), tip: "웹 앱 매니페스트 연결 여부" },
        { label: "standalone", value: yesNo(runtime.standalone), tip: "독립 실행 모드(홈 화면 설치) 여부" },
        { label: "network", value: runtime.online === false ? "offline" : "online", tip: "네트워크 연결 상태" },
      ];
      return html`
    ...
    function opsRuntimeHTML(model) {
      ...
      const metrics = [
        { label: "loaded lazy files", value: `${loadedCount}/${totalCount}`, tip: "지연 로드된 파일 수 / 전체 파일 수" },
        { label: "ready groups", value: `${readyGroups}/${groupStats.length}`, tip: "준비된 런타임 그룹 수 / 전체 그룹 수" },
        { label: "pending", value: pending.length, tip: "대기 중인 런타임 그룹 수" },
        { label: "failed", value: failed.length, tip: "실패한 런타임 그룹 수" },
      ];
      return html`
        <section class="panel ops-runtime-diagnostics" role="status" aria-live="polite" data-system-ops-runtime data-ops-runtime-status="${statusLabel}" data-ops-runtime-version="${runtime.version || "missing"}" data-ops-runtime-loaded-count="${loadedCount}" data-ops-runtime-total-count="${totalCount}" data-ops-runtime-ready-group-count="${readyGroups}" data-ops-runtime-group-count="${groupStats.length}" data-ops-runtime-pending-count="${pending.length}" data-ops-runtime-failed-count="${failed.length}" data-ops-runtime-last-group="${lastGroup.group || ""}" data-ops-runtime-last-status="${lastGroup.lastStatus || "none"}">
          ...
          ${failed.length ? raw(html`
            <ul class="settings-info" data-ops-runtime-failures>
              ${failed.map((item) => raw(html`<li data-ops-runtime-failure data-ops-runtime-failure-path="${item.path || ""}" title="실패: ${item.path || 'unknown'} - ${item.error || 'failed'}"><strong>${item.path || "unknown"}</strong> · ${item.error || "failed"}</li>`))}
            </ul>
          `) : ""}
        </section>
      `;
      ...
      return html`
        <section class="panel publish-readiness" data-system-publish-readiness data-system-publish-blockers="${blockerCount}">
          ...
          <div class="publish-readiness-actions" data-system-publish-handoff>
            <button type="button" class="portfolio-export-download portfolio-export-copy" data-action="copy-system-publish-handoff" data-system-publish-handoff-copy data-system-publish-handoff-text="${model.publishUnblockHandoff}" title="배포 차단 해제 핸드오프를 클립보드에 복사합니다">publish unblock handoff 복사</button>
