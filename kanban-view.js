(function (root) {
  ...
  const SOURCE_FILTER_OPTIONS = [
    { key: "all", label: "전체", tip: "모든 출처의 이슈 표시" },
    { key: "wiki", label: "LLM Wiki", tip: "LLM Wiki에서 파생된 이슈" },
    { key: "db", label: "DB Catalog", tip: "DB 카탈로그에서 파생된 이슈" },
    { key: "review", label: "Review", tip: "리뷰 프로세스에서 생성된 이슈" },
    { key: "workspace-review", label: "Workspace", tip: "워크스페이스 리뷰 이슈" },
    { key: "kb-ia-review", label: "KB/IA", tip: "지식 베이스/정보 구조 리뷰 이슈" },
    { key: "benchmark-review", label: "PM Bench", tip: "벤치마크 리뷰 이슈" },
    { key: "source", label: "Source", tip: "소스 코드 관련 이슈" },
  ];
  const STATUS_COLORS = {
    todo: "var(--muted)",
  ...
  function createKanbanView(deps) {
    ...
    function sourceFilterHTML(model) {
      return html`
        <div class="kanban-source-filterbar" role="group" aria-label="칸반 출처 필터" data-kanban-source-filterbar data-kanban-source-filter-current="${model.sourceFilter}">
          ${raw(SOURCE_FILTER_OPTIONS.map((option) => html`
            <button type="button" class="kanban-source-chip ${raw(model.sourceFilter === option.key ? "is-active" : "")}" data-action="filter-kanban-source" data-kanban-source-filter="${option.key}" aria-pressed="${model.sourceFilter === option.key}" title="${option.tip}">
    ...
    function densityToggleHTML(model) {
      ...
      return html`
        <div class="kanban-density-toggle" role="group" aria-label="칸반 카드 밀도" data-kanban-density-toggle data-kanban-density-current="${model.density}">
