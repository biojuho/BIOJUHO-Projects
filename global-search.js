(function (root) {
  ...
  function createGlobalSearch(deps) {
    ...
    function setShellAffordance(shell, inert) {
      ...
      shell.title = inert ? SEARCH_INERT_HINT : "현재 화면 검색 (키 입력) · 명령 팔레트 (⌘K)";
    }
    ...
    function virtualizedOverflowCount(view) {
      // Virtualized lists render only the first N matches, so add back the
      // rows the virtual notes report as not rendered to keep the count honest.
      let overflow = 0;
      view.querySelectorAll("[data-todo-virtual-total],[data-kanban-virtual-total]").forEach((note) => {
        const total = Number(note.dataset.todoVirtualTotal || note.dataset.kanbanVirtualTotal);
        const rendered = Number(note.dataset.todoVirtualRendered || note.dataset.kanbanVirtualRendered);
        if (Number.isFinite(total) && Number.isFinite(rendered) && total > rendered) overflow += total - rendered;
      });
      return overflow;
    }
    ...
    function status() {
      ...
      const resultCount = view.querySelectorAll("[data-search-result]").length + virtualizedOverflowCount(view);
