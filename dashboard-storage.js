  ...
  function normalizeDashboardRecord(record, fallback = {}) {
    ...
    Object.keys(input).forEach((key) => {
      ...
      if (typeof value === "string") normalized[key] = cleanString(value, 360);
        ...
        // Arrays of objects (e.g. externalResearchSources: [{id,title,url,...}])
        // must keep their structure — cleanStringArray would flatten each object
        // to the string "[object Object]", destroying evidence-source metadata.
        normalized[key] = value.some(isObject)
          ? value.slice(0, 50).map(stableValue)
          : cleanStringArray(value, 20, 180);
      }
