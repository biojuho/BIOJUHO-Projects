...
from .vibe_tools import read_file_tool, write_code_tool, run_test_tool
    ...
    def specialized_node(state: AgentState):
        ...
        try:
            ...
            return {
                ...
                "code_variants": [{"variant_id": vid, "code": result["messages"][-1].content, "confidence": confidence}]
            }
            ...
            return {
                "errors": [error_info],
                "messages": [HumanMessage(content=f"Worker {vid} failed: {str(e)}")]
            }
...
def create_reflection_subgraph(chat_model, tools: list):
    ...
    def generate_node(state: AgentState):
        ...
        result = chat_model.invoke([SystemMessage(content="You are a Vibe Coder refining code."), HumanMessage(content=prompt)])
    ...
    def evaluate_reflect_node(state: AgentState):
        ...
        try:
            if hasattr(chat_model, "with_structured_output"):
                ...
                response = chat_model.invoke([SystemMessage(content="Output ONLY valid JSON"), HumanMessage(content=eval_prompt)])
                resp_text = response.content.split("```json")[-1].split("```")[0].strip() if "```json" in response.content else response.content
                eval_data = json.loads(resp_text)
                eval_result = EvaluationResult(**eval_data)
        except Exception as e:
             # Fallback if parsing fails
             eval_result = EvaluationResult(
                 correctness_score=0.5, vibe_alignment_score=0.5, code_quality_score=0.5, security_score=0.5,
                 overall_feedback=f"평가 파싱 에러: {str(e)}", confidence=0.5
             )
        ...
        return {
            ...
            "messages": [HumanMessage(content=f"평가: Conf={weighted_conf:.2f}, Vibe={eval_result.vibe_alignment_score:.2f}")]
        }
    ...
    def intent_analyzer(state: AgentState):
        ...
        try:
            if hasattr(chat_model, "invoke"):
                ...
                response = chat_model.invoke([SystemMessage(content="You are a domain classifier."), HumanMessage(content=prompt)])
                response_text = response.content
            else:
                response_text = chat_model([{"role": "system", "content": "You are a domain classifier."}, {"role": "user", "content": prompt}], tools=[])
...
def build_vibe_graph(chat_model):
    ...
    allowed_targets = [f"{domain}_worker" for domain in DOMAIN_PROMPTS.keys()]
    ...
    workflow.add_conditional_edges("aggregator", route_aggregator, {"reflection_worker": "reflection_worker", "supervisor": "supervisor", "planner": "planner"})
