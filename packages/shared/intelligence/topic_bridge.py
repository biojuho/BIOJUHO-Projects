...
_DAILYNEWS_DB = _ROOT / "automation" / "DailyNews" / "data" / "pipeline_state.db"
...
class TopicBridge:
    ...
    def _get_cache(self) -> dict[str, Any]:
        ...
        if self._cache is None or self._cache_at is None or (now - self._cache_at).total_seconds() > self._cache_ttl:
    ...
    def overlap_analysis(self, gdt_keywords: list[str]) -> dict[str, Any]:
        ...
        overlap = [kw for kw in gdt_keywords if any(dn_kw in kw.lower() or kw.lower() in dn_kw for dn_kw in dn_kw_set)]
